...
(function () {
  ...
  /** Read one form field. A field showing a translation must save the wording
   *  the record was written in, not the translation the reader was shown. */
  function fv(e){ return e ? (window.I18N && I18N.fieldValue ? I18N.fieldValue(e) : e.value) : ''; }

  // ---- permissions: the same rules as the desktop app ------------------------
  // Status gates what a user may do at all (Viewer = read-only); row-level
  // ownership gates edits (only the creator, or an Admin); a Countries-affiliated
  // user who Leads exactly one country logs activities only there.
  function mStatus() { return S.user ? (lkKey('user_status', S.user.status_id, S.user.status) || 'user') : null; }
  function mIsAdmin() { return mStatus() === 'admin'; }
  function mCanWrite() { return !!S.user && mStatus() !== 'viewer'; }
  function mOwns(table, row) {
    if (!S.user || !row) return false;
    if (mIsAdmin()) return true;
    var owner = table === 'measurement' ? row.reported_by_id : row.created_by;
    return owner != null && +owner === +S.user.id;
  }
  function mCountryLock() {
    var aff = S.user && IDX && IDX.affiliationById ? IDX.affiliationById[S.user.affiliation_id] : null;
    if (!aff || aff.key !== 'country') return null;
    var isos = (IDX.countryIsosByLead || {})[S.user.id] || [];
    return isos.length === 1 ? isos[0] : null;
  }
  function mCanReport(rawProject) {
    if (!mCanWrite()) return false;
    var lock = mCountryLock();
    return !lock || !rawProject || rawProject.country_iso3 === lock;
  }

  /** A record's searchable text: what it says, plus what the reader sees.
   *  `fold` does the same to the query, so a search in Arabic finds a record
   *  stored in English. */
  function trHay(txt){ return window.I18N && I18N.hay ? I18N.hay(txt) : String(txt == null ? '' : txt).toLowerCase(); }
  ...
  /* ----------------------------------------------------------------- app state */
  var S = { plan: null, user: null, view: 'home', proj: { q: '', region: '', status: '', page: 1 }, kpi: { q: '', region: '', status: '', n: 60 } };
  var IDX;                    // DB._idx
  var TODAY;                  // derived "now"
  var INDS = [], INDBYID = {}, PROJECTS = [], PROJBYID = {}, ACTS = [], ACTBYID = {};
  var USEOFMEAS = {};   // measurement id -> the KPI use it belongs to
  function distinctKpis(uses) { var seen = {}; (uses || []).forEach(function (u) { seen[u.kpiId] = 1; }); return Object.keys(seen).length; }
  ...
  // A KPI USE is what performance is measured on (same model as the desktop):
  // a KPI adopted by a project - its baseline/target cascade from the KPI's
  // plan-level values unless the project set its own - a project's own KPI, or
  // activities logged with no project in one country (no target). `use.raw` is
  // the KPI definition with the use's effective baseline/target folded in.
  function computeStatus(use) {
    ...
    if (b == null || t == null || +t === +b)
      return { progress: null, performance: null, code: 'nodata', value: latest ? indicatorValue(ind, ms) : null, latest: latest, series: ms };
    if (!latest)
  ...
  function enrich() {
    ...
    // 1) KPI uses of the active plan - a KPI belongs to no country; it meets one
    //    through a project that adopts it, or an activity logged somewhere
    INDS = []; INDBYID = {};
    var byProjKpi = {}, free = {}, seq = 0;
    function mkUse(def, kind, proj, iso, link) {
      var res = def.result_id != null ? IDX.resultById[def.result_id] : null;
      var co = iso ? IDX.countryByIso[iso] : null;
      var raw = {}; for (var k in def) raw[k] = def[k];
      if (kind === 'free') { raw.baseline_value = null; raw.target_value = null; }
      else if (link) ['baseline_value', 'baseline_date', 'target_value', 'target_date'].forEach(function (c) { if (link[c] != null) raw[c] = link[c]; });
      var r = {
        id: ++seq, kpiId: def.id, kind: kind, raw: raw, name: def.name, code: def.code, unit: kpiUnit(def),
        secondary: kind === 'secondary', level: res ? res.level : (kind === 'secondary' ? 'secondary' : 'output'),
        result: res, sdg: res ? res.sdg : null, iso: iso || null, project: proj || null,
        region: proj ? proj.region : (co ? co.region : null),
        target: raw.target_value, baseline: raw.baseline_value, series: []
      };
    ...
    DB.tables.project.forEach(function (p) {
      ...
      (IDX.projectKpiByProject[p.id] || []).forEach(function (pk) {
        var def = IDX.indicatorById[pk.indicator_id]; if (!def || def.secondary) return;
        byProjKpi[p.id + '|' + def.id] = mkUse(def, 'project', p, p.country_iso3, pk);
      });
      (IDX.secondaryByProject[p.id] || []).forEach(function (def) {
        byProjKpi[p.id + '|' + def.id] = mkUse(def, 'secondary', p, p.country_iso3, null);
      });
    });
    USEOFMEAS = {};
    DB.tables.measurement.slice().sort(function (a, b) { return a.date < b.date ? -1 : (a.date > b.date ? 1 : a.id - b.id); }).forEach(function (m) {
      var def = IDX.indicatorById[m.indicator_id]; if (!def || indicatorPlanId(def) !== S.plan) return;
      var u;
      if (m.project_id != null) {
        u = byProjKpi[m.project_id + '|' + def.id];
        if (!u) {
          var pj = IDX.projectById[m.project_id]; if (!pj || pj.plan_id !== S.plan) return;
          u = byProjKpi[m.project_id + '|' + def.id] = mkUse(def, 'free', pj, pj.country_iso3, null);
        }
      } else {
        if (def.secondary) return;
        var key = def.id + '|' + (m.country_iso3 || '');
        u = free[key] || (free[key] = mkUse(def, 'free', null, m.country_iso3, null));
      }
      u.series.push(m); USEOFMEAS[m.id] = u;
    });
    INDS.forEach(function (r) {
      var st = computeStatus(r);
      r.progress = st.progress; r.performance = st.performance;
      r.ratio = st.performance; r.status = st.code;          // performance is the authoritative RAG
      r.value = st.value; r.latest = st.latest;
      r.updated = st.latest ? st.latest.date : null; r.updatedMs = st.latest ? Date.parse(st.latest.date) : null;
    });
    ...
    DB.tables.project.forEach(function (p) {
      if (p.plan_id !== S.plan) return;
      var inds = [];
      (IDX.projectKpiByProject[p.id] || []).forEach(function (pk) { var i = byProjKpi[p.id + '|' + pk.indicator_id]; if (i && i.kind === 'project' && inds.indexOf(i) < 0) inds.push(i); });
      (IDX.secondaryByProject[p.id] || []).forEach(function (sec) { var i = byProjKpi[p.id + '|' + sec.id]; if (i && inds.indexOf(i) < 0) inds.push(i); });
      var ratios = [], progs = [];
    ...
    (DB.tables.measurement || []).forEach(function (m) {
      var ind = USEOFMEAS[m.id]; if (!ind) return;
      var proj = m.project_id != null ? PROJBYID[m.project_id] : null;
      var mco = m.country_iso3 ? IDX.countryByIso[m.country_iso3] : null;
      ACTS.push({
        id: m.id, m: m, ind: ind, proj: proj,
        iso: proj ? proj.iso : (m.country_iso3 || ind.iso), region: proj ? proj.region : (mco ? mco.region : ind.region),
  ...
  function computeStats() {
    ...
    STAT = {
      projects: PROJECTS.length, activities: ACTS.length, kpis: (function () { var seen = {}; INDS.forEach(function (u) { seen[u.kpiId] = 1; }); return Object.keys(seen).length; })(),
  ...
  /** How a KPI is LABELLED everywhere: code first, then name, always together -
   *  the code is part of the name, not a separate field to strand elsewhere. */
  function kpiLabelHTML(i) {
    if (!i) return '<span class="kpi-name">–</span>';
    return (i.code ? '<span class="kpi-code">' + esc(i.code) + '</span> ' : '')
         + '<span class="kpi-name">' + esc(i.name) + '</span>';
  }
  function kpiLabelText(i) {
    if (!i) return '–';
    return (i.code ? i.code + ' · ' : '') + i.name;
  }
  /** The activity's own free text, behind a tap/hover icon: the KPI names the
   *  row, this note only describes one report of it. */
  function actNote(narrative) {
  ...
  /* ============================================================= VIEWS ===== */
  // The third tab lists KPIs, not activities: a KPI is what is being measured, an
  // activity is one report of it. The activity log lives inside a KPI (openKpi).
  var VIEW_TITLE = { home: 'Home', projects: 'Projects', kpis: 'KPIs', insights: 'Insights', profile: 'Profile' };
  ...
  function render() {
    ...
    else if (S.view === 'kpis') { scr.innerHTML = viewKpis(); wireKpis(); }
  ...
  function viewHome() {
    ...
    var tiles = [
      ...
      { k: 'KPIs tracked', v: fmtInt(STAT.kpis), sub: 'indicators', nav: 'kpis' },
      { k: 'Activities', v: compact(STAT.activities), sub: 'reports logged', nav: 'kpis' },
    ...
    return ''
      ...
      +   '<div class="hero-greet">' + esc(greet) + ', <b>' + esc(name) + '</b>.</div>'
      ...
      // Recently REPORTED KPIs, not a feed of narratives: the home screen answers
      // "what moved", and what moves is a KPI. Tapping one opens its activities.
      + '<div class="sec-h">Recently reported KPIs</div>'
      + '<div class="card list">' + (recentKpis(5).map(kpiRow).join('') || '<div class="empty">Nothing reported yet.</div>') + '</div>'
      + '<button class="btn ghost block" onclick="__go(\'kpis\')">See all KPIs</button>'
  ...
  function filteredProjects() {
    var q = fold(S.proj.q.trim());
    return PROJECTS.filter(function (p) {
      if (S.proj.region && p.region !== S.proj.region) return false;
      if (S.proj.status && p.status !== S.proj.status) return false;
      if (q) {
        var hay = [trHay(p.name), trHay(p.country ? p.country.name : ''), trHay(p.donor ? p.donor.name : ''), fold(p.donor ? p.donor.short_name : ''), fold(p.code || '')].join(' ');
  ...
  /* ---------------------------------------------------------------- KPIs */
  /** How many activities have been logged against a KPI. */
  function actCountOf(i) { return (i.series || []).length; }
  /** KPIs with the most recent report first - "what moved lately". */
  function recentKpis(n) {
    return INDS.filter(function (i) { return i.updatedMs; })
      .sort(function (a, b) { return b.updatedMs - a.updatedMs; }).slice(0, n || 5);
  }
  /** A KPI row — the unit every list is built from. Tapping it opens the KPI
   *  sheet, which is where that KPI's own activities are read. */
  function kpiRow(i) {
    var n = actCountOf(i);
    // The code leads the name in the row's own title - the two are one label,
    // never split across lines - so the sub-line is free to carry the numbers.
    // a KPI is measured per project (or per country when logged with none), so
    // the row names where - otherwise the same KPI repeats indistinguishably
    var where = i.project ? (i.project.code || i.project.name) : (i.iso && IDX.countryByIso[i.iso] ? IDX.countryByIso[i.iso].name : '');
    var sub = (where ? where + ' · ' : '')
      + (i.value == null ? 'no report yet' : compact(i.value) + (i.target != null ? ' / ' + compact(i.target) : ''))
      + ' · ' + n + ' activit' + (n === 1 ? 'y' : 'ies')
      + (i.updatedMs ? ' · ' + ago(i.updatedMs) : '');
    return '<div class="krow" data-kpi="' + i.id + '">'
      + '<div class="krow-main"><div class="krow-t">' + kpiLabelHTML(i) + '</div>'
      + '<div class="krow-s">' + sub + '</div>'
      + progressLine(i.progress, i.status) + '</div>'
      + '<div class="row-end">' + pill(i.status) + '<span class="row-chev">›</span></div></div>';
  }
  function filteredKpis() {
    var q = fold(S.kpi.q.trim());
    return INDS.filter(function (i) {
      if (S.kpi.region && i.region !== S.kpi.region) return false;
      if (S.kpi.status && i.status !== S.kpi.status) return false;
      if (q) {
        var hay = [trHay(i.name), fold(i.code || ''), trHay(i.result ? i.result.statement : '')].join(' ');
        if (hay.indexOf(q) < 0) return false;
      }
      return true;
    }).sort(function (a, b) { return (b.updatedMs || 0) - (a.updatedMs || 0); });
  }
  function kpiFilters() {
    var regions = {}; INDS.forEach(function (i) { if (i.region) regions[i.region] = 1; });
    var ropts = ['<option value="">All regions</option>'].concat(Object.keys(regions).sort().map(function (r) {
      return '<option value="' + esc(r) + '"' + (S.kpi.region === r ? ' selected' : '') + '>' + esc(r) + '</option>';
    })).join('');
    var sopts = ['<option value="">All statuses</option>'].concat(STATUS_ORDER.map(function (k) {
      return '<option value="' + k + '"' + (S.kpi.status === k ? ' selected' : '') + '>' + statusLabel(k) + '</option>';
    })).join('');
    return '<div class="filters">'
      + '<div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3-3"/></svg>'
      + '<input id="aq" placeholder="Search KPI, code, result statement…" value="' + esc(S.kpi.q) + '"></div>'
      + '<div class="selrow"><select id="aRegion">' + ropts + '</select><select id="aStatus">' + sopts + '</select></div>'
      + '</div>';
  }
  // The KPI list, newest-reported first. It replaced the old activity feed: a feed
  // of narratives says what someone wrote, a KPI list says what is being achieved.
  // The narratives are still one tap away, inside each KPI.
  function viewKpis() {
    var list = filteredKpis();
    var slice = list.slice(0, S.kpi.n);
    var more = list.length > S.kpi.n;
    var filtered = !!(S.kpi.q || S.kpi.region || S.kpi.status);
    var out = slice.map(kpiRow).join('');
    return kpiFilters()
      + '<div class="count">' + fmtInt(distinctKpis(list)) + ' KPI' + (distinctKpis(list) === 1 ? '' : 's') + ' · ' + fmtInt(list.length) + ' in projects & countries' + (filtered ? ' · filtered' : '') + '</div>'
      + '<div class="card list flush">' + (out || '<div class="empty">No KPIs match your filters.</div>') + '</div>'
      + (more ? '<button class="btn ghost block" id="moreKpi">Load more</button>' : (slice.length ? '<div class="foot">— end of list —</div>' : ''));
  }
  function wireKpis() {
    var q = $('#aq');
    if (q) q.addEventListener('input', debounce(function () { S.kpi.q = q.value; S.kpi.n = 60; softRerender(viewKpis, wireKpis, true); }, 180));
    var rg = $('#aRegion'); if (rg) rg.addEventListener('change', function () { S.kpi.region = rg.value; S.kpi.n = 60; softRerender(viewKpis, wireKpis); });
    var st = $('#aStatus'); if (st) st.addEventListener('change', function () { S.kpi.status = st.value; S.kpi.n = 60; softRerender(viewKpis, wireKpis); });
    var b = $('#moreKpi'); if (b) b.addEventListener('click', function () { S.kpi.n += 60; softRerender(viewKpis, wireKpis); });
  }
  ...
  function viewProfile() {
    ...
    return '<div class="prof-card">'
      ...
      +   (langAvailable()
            ? '<button class="setrow" id="setLang"><span class="setk">Language</span><span class="setv">' + esc(langCurrentName()) + ' ▸</span></button>'
            : '')
  ...
  function wireProfile() {
    ...
    var lang = $('#setLang'); if (lang) lang.addEventListener('click', openLangSheet);
  ...
  /* ============================================================ SHEETS ===== */
  // Data-entry sheets pass locked=true so a scrim tap or swipe-down can't
  // dismiss them (would lose typed input) — they close only via ✕ / Cancel.
  var sheetLocked = false;
  function openSheet(title, html, locked) {
    sheetLocked = !!locked;
  ...
  /* ---- language ---------------------------------------------------------
     The list is the platform's own `language` table, so a language an
     administrator adds on the desktop shows up here with nothing to ship. */
  function langAvailable() {
    return !!(window.I18N && I18N.ready && I18N.enabledLanguages().length > 1);
  }
  function langCurrentName() {
    if (!window.I18N || !I18N.ready) return 'English';
    var l = I18N.current();
    return l ? (l.native_name || l.name) : 'English';
  }
  function openLangSheet() {
    var rows = I18N.enabledLanguages().map(function (l) {
      var on = l.key === I18N.lang;
      return '<button class="planrow' + (on ? ' on' : '') + '" data-lang="' + esc(l.key) + '">'
        + '<div><div class="planrow-n">' + esc(l.native_name || l.name) + '</div>'
        + '<div class="planrow-s">' + esc(l.name) + '</div></div>'
        + (on ? '<span class="planrow-ck">✓</span>' : '') + '</button>';
    }).join('');
    openSheet('Language', rows);
    $$('[data-lang]', $('#sheetBody')).forEach(function (b) {
      b.addEventListener('click', function () {
        var key = b.getAttribute('data-lang');
        closeSheet();
        if (key !== I18N.lang) I18N.setLang(key);
        softRerender(viewProfile, wireProfile);
      });
    });
  }
  ...
  function openProject(id) {
    ...
    function krow(i) {
      return '<div class="krow" data-kpi="' + i.id + '">'
        + '<div class="krow-main"><div class="krow-t">' + kpiLabelHTML(i) + '</div>'
        + '<div class="krow-s">' + (i.value == null ? 'no report' : compact(i.value) + ' / ' + compact(i.target)) + '</div>'
        + progressLine(i.progress, i.status) + '</div>'
    ...
    var canEditP = mOwns('project', p.raw), canLogP = mCanReport(p.raw) && p.inds.length > 0;
    if (canEditP || canLogP) {
      head += '<div class="ef-actions">'
        + (canEditP ? '<button class="btn" id="prEdit">Edit project</button>' : '')
        + (canLogP ? '<button class="btn primary" id="prLog">Log activity</button>' : '') + '</div>';
    }
    ...
    bindTaps($('#sheetBody'));
    if ($('#prEdit')) $('#prEdit').onclick = function () { openProjectForm(p.id); };
    if ($('#prLog')) $('#prLog').onclick = function () { openActivityForm(p.id); };
  }
  ...
  function openKpi(id) {
    ...
    // THE activity log: the reports behind this KPI. Activity narratives appear
    // here — under the KPI they belong to — and nowhere else. Each row taps
    // through to the full activity (note, beneficiaries, edit).
    var meas = series.slice(0, 24).map(function (m) {
      ...
      var val = i.unit === '%' ? (Math.round(m.value * 10) / 10 + '%') : compact(m.value);
      // One KPI is in question, and the hero above has named it - so the row's
      // detail is what was done, read straight off the line rather than tapped.
      var narr = String(m.narrative == null ? '' : m.narrative).trim();
      return '<div class="mrow tap" data-act="' + m.id + '"><div class="mrow-l"><b>' + esc(val) + '</b><span>' + fmtDate(m.date) + '</span></div>'
        + '<div class="mrow-r">'
        +   (narr ? '<span class="mrow-narr">' + esc(narr) + '</span>' : '')
        +   '<span class="mrow-by">' + esc(rep) + (m.place_name ? ' · ' + esc(m.place_name) : '') + '</span>'
        + '</div></div>';
    }).join('') || '<div class="empty">No activities logged against this KPI yet.</div>';

    var body = '<div class="sheet-hero">'
      + '<div class="sheet-hero-top">' + pill(i.status) + '</div>'
      + '<div class="sheet-hero-t">' + kpiLabelHTML(i) + '</div>'
      ...
      + spark
      + '<div class="sec-h">Activities <span>' + (i.series || []).length + '</span></div>'
  ...
  function sparkline(i) {
    ...
    var target = i.target == null ? NaN : +i.target;   // no target (no project): no target line
  ...
  function updateFab() {
    ...
    fab.hidden = !(mCanWrite() && ['home', 'projects', 'kpis'].indexOf(S.view) >= 0);
  }
  ...
  function openActivityForm(prefillPid, editMid) {
    ...
    if (edit && !mOwns('measurement', edit.m)) { toast('Only the activity’s author can edit it'); return; }
    if (!edit && !mCanWrite()) return;
    var projects = PROJECTS.filter(function (p) { return mCanReport(p.raw); })
      .sort(function (a, b) { return a.name < b.name ? -1 : 1; });
    function projLabel(p) { return stripCountry(p.name, p.country) + ' · ' + (p.country ? p.country.name : p.iso); }
    var m = edit ? edit.m : null, projField, kpiField, pid = null;
    if (edit) {
      // Editing: the activity keeps its project (or its lack of one) and its KPI
      // - both shown, neither changeable, so a save can never re-point it.
      pid = edit.proj ? edit.proj.id : null;
      projField = '<select id="afProj" disabled><option value="' + (pid == null ? '' : pid) + '" selected>'
        + esc(edit.proj ? projLabel(edit.proj) : '— No project —') + '</option></select>';
      kpiField = '<label class="ef"><span>KPI</span><select id="afKpi" disabled><option value="' + edit.ind.id + '" selected>'
        + esc((edit.ind.code ? edit.ind.code + ' · ' : '') + edit.ind.name) + '</option></select></label>';
    } else {
      if (!projects.length) { openSheet('Log activity', '<div class="empty">No projects you can report on yet.</div>'); return; }
      var fixed = prefillPid != null && PROJBYID[prefillPid] ? PROJBYID[prefillPid] : null;
      pid = fixed ? fixed.id : projects[0].id;
      // opened from a project's sheet: that project is fixed
      projField = fixed
        ? '<select id="afProj" disabled><option value="' + fixed.id + '" selected>' + esc(projLabel(fixed)) + '</option></select>'
        : '<select id="afProj">' + optsFrom(projects, function (p) { return p.id; }, projLabel, pid) + '</select>';
      kpiField = kpiFieldHtml(pid, null);
    }
    var body = '<form id="eform" class="eform">'
      + '<label class="ef"><span>Project' + (edit ? '' : ' *') + '</span>' + projField + '</label>'
      + '<div id="afKpiWrap">' + kpiField + '</div>'
      ...
      + '</form>';
    openSheet(edit ? 'Edit activity' : 'Log activity', body, true);
    var host = $('#afBens');
    ...
    var projSel = $('#afProj');
    if (projSel && !projSel.disabled) projSel.onchange = function () { $('#afKpiWrap').innerHTML = kpiFieldHtml(+projSel.value, null); syncVal(); };
    syncVal();
  ...
  function saveActivity(edit) {
    ...
    if (edit && !mOwns('measurement', edit.m)) { msg.textContent = 'Only the activity’s author can edit it.'; return; }
    var pid = edit ? (edit.proj ? edit.proj.id : null) : +$('#afProj').value;
    if (!edit && !mCanReport(PROJBYID[pid] && PROJBYID[pid].raw)) { msg.textContent = 'You cannot log activities on this project.'; return; }
    var title = (fv($('#afTitle')) || '').trim(); if (!title) { msg.textContent = 'Describe what was done.'; $('#afTitle').focus(); return; }
    ...
    var place = (fv($('#afPlace')) || '').trim();   // the stored spelling, not a translation of it
    msg.textContent = 'Saving…';
    var meas = edit ? edit.m : {};
    meas.indicator_id = ind.kpiId; meas.project_id = pid; meas.date = date; meas.value = +valRaw;
    var pj = pid != null ? IDX.projectById[pid] : null;
    if (pj && pj.country_iso3) meas.country_iso3 = pj.country_iso3;   // happened in its project's country
    ...
    Promise.resolve(save).then(function () {
      ...
      if (S.view === 'kpis') render(); else navTo('kpis');
  ...
  function openProjectForm(editPid) {
    ...
    var body = '<form id="pform" class="eform">'
      ...
      + '</form>';
    openSheet(p ? 'Edit project' : 'New project', body, true);
  ...
  function saveProject(existing) {
    var msg = $('#pfMsg');
    if (existing ? !mOwns('project', existing) : !mCanWrite()) { msg.textContent = 'You do not have permission to save this project.'; return; }
    var name = (fv($('#pfName')) || '').trim(); if (!name) { msg.textContent = 'Project name is required.'; $('#pfName').focus(); return; }
  ...
  function openActivity(mid) {
    ...
    var m = a.m, ind = a.ind;
    var val = ind.unit === '%' ? (Math.round(m.value * 10) / 10 + '%') : compact(m.value);
    ...
    var body = '<div class="sheet-hero">'
      + '<div class="sheet-hero-top">' + pill(ind.status) + '</div>'
      // The KPI names this activity - code first, then name. This sheet IS one
      // activity of that KPI, so what was done is read here in full.
      + '<div class="sheet-hero-t">' + kpiLabelHTML(ind) + '</div>'
      + (String(m.narrative == null ? '' : m.narrative).trim()
          ? '<div class="hero-narr">' + esc(String(m.narrative).trim()) + '</div>' : '')
      + '<div class="chainrow"><button class="chip linkchip" data-kpi="' + ind.id + '">Open KPI ›</button></div>'
      + '</div>'
      + '<div class="minitiles">'
    ...
    bindTaps($('#sheetBody'));
    if ($('#acEdit')) $('#acEdit').onclick = function () { openActivityForm(null, mid); };
    if ($('#acEditBen')) $('#acEditBen').onclick = function () { openBenForm(mid); };
  }
  function openBenForm(mid) {
    var a = ACTBYID[mid]; if (!a) return;
    if (!mOwns('measurement', a.m)) { toast('Only the activity’s author can edit it'); return; }
    ...
    $('#bform').onsubmit = function (e) {
      ...
      Promise.all(jobs).then(function () {
        ...
        if (S.view === 'kpis') { var scr = $('#screen'); scr.innerHTML = viewKpis(); wireKpis(); bindTaps(scr); }
        openActivity(mid);
      }).catch(function () { $('#bfMsg').textContent = 'Could not save — please try again.'; });
    };
  }
  ...
  function signIn(username, password) {
    ...
    S.user = u; DB.actingUserId = u.id;   // stamped as created_by on every row inserted
  ...
  function restoreSession() {
    ...
    if (u && u.enabled !== 0) { S.user = u; DB.actingUserId = u.id; return true; }
    return false;
  }
  function signOut() {
    try { localStorage.removeItem(SESSION_KEY); } catch (e) {}
    S.user = null; DB.actingUserId = null; showLogin();
  }
  ...
  function wireChrome() {
    ...
    $('#scrim').addEventListener('click', function () { if (!sheetLocked) closeSheet(); });
    ...
    sh.addEventListener('touchstart', function (e) { if (!sheetLocked && $('#sheetBody').scrollTop <= 0) sy0 = e.touches[0].clientY; else sy0 = null; }, { passive: true });
  ...
  function boot() {
    ...
    DB.init().then(function () {
      return window.I18N ? I18N.reconcile() : null;
    }).then(function () {
      IDX = DB._idx; TODAY = deriveToday();
      S.plan = currentPlanId();
      if (restoreSession()) { enrich(); showApp(); }
      else showLogin();
      // The phone speaks the same languages the platform does, off the same
      // catalogue - so a language chosen on the desktop reads the same here.
      if (window.I18N) {
