...
(function () {
  ...
  function kpiRow(i) {
    ...
    var sub = (i.value == null ? 'no report yet' : compact(i.value) + ' / ' + compact(i.target))
      + ' · ' + n + ' activit' + (n === 1 ? 'y' : 'ies')
  ...
  function viewKpis() {
    ...
    return kpiFilters()
      + '<div class="count">' + fmtInt(list.length) + ' KPI' + (list.length === 1 ? '' : 's') + (filtered ? ' · filtered' : ' tracked') + '</div>'
  ...
  function updateFab() {
    ...
    fab.hidden = !(S.user && ['home', 'projects', 'kpis'].indexOf(S.view) >= 0);
  }
