...
(function () {
  ...
  function googleViaHost(text, from, to) {
    // resolved by db.js from its own script URL, so /mobile/ reaches /api/ too
    return withTimeout(fetch(((window.DB && DB.apiBase) || 'api/') + 'translate', {
      method: 'POST',
    ...
    try {
      ...
      var walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, {
        acceptNode: function (n) {
          // a textarea's content is the user's, but its placeholder is interface
          if (n.nodeName === 'TEXTAREA' && n.getAttribute('data-notr') == null) doAttrs(n);
  ...
  function wrapDB() {
    ...
    // a settlement logged NOW is a name like every other one: register it before
    // harvesting, or it would be queued for machine translation
    function notePlaces(table, list) {
      if (table !== 'measurement' || !list) return;
      (Array.isArray(list) ? list : [list]).forEach(function (m) {
        if (m && m.place_name) { NO_AUTO[String(m.place_name)] = 1; PLACE_NAMES[String(m.place_name)] = 1; }
      });
    }
    DB.insert = function (table, list) {
      var out = insert.apply(DB, arguments);
      try { notePlaces(table, list); harvest(table, list); } catch (e) {}
