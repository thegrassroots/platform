...
-- `evidence_file` is composition of `evidence` and is owned through it.
...
-- ONE framework per plan: each Impact, Outcome and Output exists once. Results
-- and KPIs are not "of a country" - a country enters only through a transaction
-- (a project that adopts a KPI, an activity logged against it).
...
-- One row per KPI - a free-standing definition, never a per-country copy. For a
-- framework KPI the baseline/target are PLAN-LEVEL values that cascade to every
-- project adopting it (see project_kpi). A secondary KPI belongs to one project,
-- so its baseline/target here are that project's.
...
-- Baseline and target are set ONCE on the KPI in the results framework (plan
-- level) and CASCADE to every project that adopts it. Performance is measured per
-- project, from that project's activities against these values. A NULL below
-- means "inherit the plan-level value"; a value is that project's own adjustment.
CREATE TABLE IF NOT EXISTS project_kpi (
    ...
    baseline_value REAL,                 -- project override of the KPI baseline (NULL = inherit)
    baseline_date  TEXT,                 -- ISO 8601 (NULL = inherit)
    target_value   REAL,                 -- project override of the KPI target (NULL = inherit)
    target_date    TEXT,                 -- ISO 8601 (NULL = inherit)
...
CREATE TABLE IF NOT EXISTS measurement (
    ...
    country_iso3   TEXT REFERENCES country(iso3) ON DELETE SET NULL,  -- where it happened: the project's country, or chosen when logged without one
...
-- EVALUATION ------------------------------------------------------------------
-- Evidence backs a project's achieved results: documents, photos, recordings
-- and links uploaded against ONE KPI of ONE project (Project > Evaluation tab),
-- then compiled into the project's Evaluation Report PDF.

-- The kinds of evidence (reference list, selected and saved by id). `key` is
-- what the app switches on: 'document' | 'image' | 'video' | 'audio' | 'link'.
CREATE TABLE IF NOT EXISTS evidence_type (id INTEGER PRIMARY KEY, key TEXT NOT NULL UNIQUE, name TEXT NOT NULL, seq INTEGER);

-- One piece of evidence. Either an uploaded file (file_name/mime_type/size_bytes
-- describe it; the bytes live in evidence_file) or a link (url) - a video or a
-- voice recording hosted elsewhere, printed in the report as a QR code.
CREATE TABLE IF NOT EXISTS evidence (
    id           INTEGER PRIMARY KEY,
    project_id   INTEGER NOT NULL REFERENCES project(id) ON DELETE CASCADE,    -- COMPOSITION: evidence of a project
    indicator_id INTEGER NOT NULL REFERENCES indicator(id) ON DELETE CASCADE,  -- COMPOSITION: the result (KPI) it evidences
    type_id      INTEGER REFERENCES evidence_type(id) ON DELETE SET NULL,      -- document / image / video / audio / link, by id
    name         TEXT NOT NULL,          -- title shown in the list and the report
    description  TEXT,                   -- what this evidence shows
    evidence_date TEXT,                  -- ISO 8601 date the evidence was produced
    file_name    TEXT,                   -- original file name (NULL for a link)
    mime_type    TEXT,
    size_bytes   INTEGER,
    url          TEXT,                   -- external link (NULL for an uploaded file)
    uploaded     TEXT,                   -- ISO timestamp of the upload
    created_by   INTEGER REFERENCES user(id) ON DELETE SET NULL  -- OWNER: the user who inserted this row (edit/delete gate)
);
CREATE INDEX IF NOT EXISTS idx_evidence_project   ON evidence(project_id);
CREATE INDEX IF NOT EXISTS idx_evidence_indicator ON evidence(indicator_id);

-- The uploaded bytes, one row per file evidence, base64-encoded. A BLOB-declared
-- column marks the table as COLD STORAGE: both backends keep it out of the
-- in-memory mirror and the boot dump, and read a row only when a file is opened
-- or a report is compiled - so evidence never slows the app down.
CREATE TABLE IF NOT EXISTS evidence_file (
    evidence_id  INTEGER PRIMARY KEY REFERENCES evidence(id) ON DELETE CASCADE,  -- COMPOSITION: 1:1 with its evidence row
    data         BLOB NOT NULL           -- the file, base64
);

-- GLOBALIZATION ---------------------------------------------------------------
-- The platform speaks every language declared in `language`. English is the
-- SOURCE language: every string in the interface and every text field in the
-- database is authored in it, and a translation is a row pointing back at that
-- source text. Nothing is duplicated per language in the business tables.

-- The languages the platform offers (admin-editable list) -----------------------
-- `key` is the BCP-47 code the app switches on ('en', 'ar'); `direction` drives
-- the RTL layout flip; exactly one row carries is_source = 1 (the language the
-- app is authored in - English) and is never itself translated.
CREATE TABLE IF NOT EXISTS language (
    id          INTEGER PRIMARY KEY,
    key         TEXT NOT NULL UNIQUE,        -- 'en' | 'ar' | ...
    name        TEXT NOT NULL,               -- English name, e.g. 'Arabic'
    native_name TEXT,                        -- the language's own name, e.g. 'العربية'
    direction   TEXT NOT NULL DEFAULT 'ltr', -- 'ltr' | 'rtl'
    is_source   INTEGER NOT NULL DEFAULT 0,  -- 1 = the language the app is authored in
    enabled     INTEGER NOT NULL DEFAULT 1,  -- 0 = hidden from the language switcher
    seq         INTEGER,                     -- display order in the switcher
    created_by  INTEGER REFERENCES user(id) ON DELETE SET NULL
);

-- The catalogue of translatable source strings ---------------------------------
-- One row per DISTINCT English string the platform can show. `source` is the
-- string itself and doubles as the stable key, so the same wording is translated
-- once and reused everywhere it appears. `scope` records where it was first seen:
--   'ui'      - interface chrome (labels, buttons, headings, messages)
--   'content' - a text value out of a business table (table_name/column_name say
--               which field it came from, so the admin panel can group by field)
CREATE TABLE IF NOT EXISTS phrase (
    id          INTEGER PRIMARY KEY,
    source      TEXT NOT NULL UNIQUE,        -- the English text, verbatim
    scope       TEXT NOT NULL DEFAULT 'ui',  -- 'ui' | 'content'
    table_name  TEXT,                        -- content: the table the text came from
    column_name TEXT,                        -- content: the column it came from
    seq         INTEGER
);
CREATE INDEX IF NOT EXISTS idx_phrase_scope ON phrase(scope);

-- One translation of one phrase into one language ------------------------------
-- `auto` distinguishes a machine translation (1) from wording an administrator
-- has reviewed or rewritten in the Globalization panel (0); a reviewed row is
-- never overwritten by the background translator.
CREATE TABLE IF NOT EXISTS translation (
    id          INTEGER PRIMARY KEY,
    phrase_id   INTEGER NOT NULL REFERENCES phrase(id) ON DELETE CASCADE,     -- COMPOSITION
    language_id INTEGER NOT NULL REFERENCES language(id) ON DELETE CASCADE,   -- COMPOSITION
    text        TEXT,                        -- the translated string
    auto        INTEGER NOT NULL DEFAULT 1,  -- 1 = machine, 0 = reviewed by an administrator
    updated     TEXT,                        -- ISO timestamp of the last change
    created_by  INTEGER REFERENCES user(id) ON DELETE SET NULL,
    UNIQUE (phrase_id, language_id)
);
CREATE INDEX IF NOT EXISTS idx_translation_lang ON translation(language_id);
...
CREATE INDEX IF NOT EXISTS idx_measure_country   ON measurement(country_iso3);
