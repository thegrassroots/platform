...
(function () {
  ...
  // PRE-HYDRATE FALLBACK ONLY. The authority for both the order and the colour
  // of a region is the `region` table (region.seq, region.color) - see
  // regionNames() and regionColor() below. These literals are what the first
  // paint uses in the moment before the database has loaded.
   ...
   *  `region` table (regions are admin-editable in Global Items), in the display
   *  order the TABLE declares (region.seq) - the order is data, not a coded list.
   *  Rows without a seq sort alphabetically after the ordered ones. The static
   *  list below is used only before the DB has hydrated. */
  function regionNames(){
    var rows = (window.DB && DB.refList) ? DB.refList('region')
             : ((window.DB && DB.tables && DB.tables.region) || []);
    var names = rows.map(function (r){ return r.name; }).filter(Boolean);
    return names.length ? names : REGION_ORDER.slice();
  }
  /** A region's identity colour, read off the `region` row (region.color) - the
   *  same way a donor's or a partner's colour is stored. The static map below is
   *  only a pre-hydrate fallback; an admin-added region with no colour set
   *  derives a stable palette colour from its name rather than going grey. */
  function regionColor(region){
    if (!region) return '#94a3b8';
    var rows = (window.DB && DB.tables && DB.tables.region) || [];
    for (var i = 0; i < rows.length; i++){
      if (rows[i].name === region && rows[i].color) return rows[i].color;
    }
    var m = REGION_META[region];
    return (m && m.color) || catColor(region);
  }
  ...
  /** Display label for a project's delivery modality, read off the
   *  `implementation_mode` table rather than spelled out at each call site. */
  function implLabel(key){ var r = lkByKey('implementation_mode', key || 'direct'); return r ? r.name : ''; }
  /** <option> list for the delivery modality. Unlike lookupOptions() the VALUE
   *  is the row's `key`, because project.implementation is a foreign key onto
   *  implementation_mode(key) - so adding a modality to the table adds it here. */
  function implOptions(cur){
    return (DB.refList ? DB.refList('implementation_mode') : (DB.tables.implementation_mode || []))
      .map(function (r){
        return '<option value="' + esc(r.key) + '"' + (r.key === cur ? ' selected' : '') + '>' + esc(r.name) + '</option>';
      }).join('');
  }
  ...
  var MEAS_NOPROJ = [];  // located activities carrying no project (attribution is optional)
  ...
  var S = {
    ...
    mapMode: 'globe',       // map projection: 'globe' (3D sphere, the default) | 'flat' (Web-Mercator)
    globeSpin: true,        // globe auto-rotation armed (it still suspends on interaction)
  ...
  //        6 done → 50%/50% = 100%.
  function computeStatus(use) {
    var ind = use.raw, ms = use.series;
    ...
    if (b == null || t == null || +t === +b) return {
      ...
      value: latest ? indicatorValue(ind, ms) : null };
  ...
  // =========================================================================
  //  KPI USES - KPIs are free-standing definitions
  // =========================================================================
  // A KPI (indicator row) belongs to the results framework, never to a country.
  // It meets a country only through a TRANSACTION, and performance is measured
  // per transaction context - a KPI USE:
  //   'project'   a project adopted the KPI (project_kpi). Its baseline/target
  //               CASCADE from the KPI's plan-level values, unless the project set
  //               its own (a non-null value on the link).
  //   'secondary' a project's own KPI (indicator.project_id) - its values are the
  //               project's.
  //   'free'      activities logged against the KPI with NO project, one use per
  //               country they happened in. They count and are plotted, but carry
  //               no target of their own (No Data status).
  // Every enriched record in IND is a use. `r.id` is the use's own key (runtime
  // only); `r.kpiId` is the KPI's database id - every write and every DB lookup
  // goes through r.kpiId, never r.id. `r.raw` is the KPI definition with the
  // use's effective baseline/target folded in; `r.def` is the stored KPI row.
  var USE_PROJECT = 10000000, USE_SECONDARY = 20000000, USE_FREE = 30000000;
  var USEBYPROJKPI = {};   // 'projectId|kpiId' -> project/secondary use
  var USEFREE = {};        // 'kpiId|iso'       -> free use
  /** The baseline/target a project works to for a KPI: its own where it set one,
   *  else the plan-level value it inherits. */
  function effectiveTargets(def, link){
    var out = {};
    ['baseline_value', 'baseline_date', 'target_value', 'target_date'].forEach(function (c){
      out[c] = (link && link[c] != null) ? link[c] : def[c];
    });
    return out;
  }
  function kpiRawFor(def, link, noTarget){
    var raw = {}; for (var k in def) raw[k] = def[k];
    if (noTarget){ raw.baseline_value = null; raw.target_value = null; }
    else if (link){ var eff = effectiveTargets(def, link); for (var c in eff) raw[c] = eff[c]; }
    return raw;
  }
  /** The same use after enrich() rebuilt the list (free-use ids are sequential). */
  function refreshUse(r){
    if (!r) return null;
    if (r.project) return USEBYPROJKPI[r.project.id + '|' + r.kpiId] || null;
    return USEFREE[r.kpiId + '|' + (r.iso || '')] || null;   // gone once its last activity is deleted
  }
  /** The use a measurement belongs to (null when its KPI is outside the plan in view). */
  function useOfMeas(m){
    if (!m) return null;
    if (m.project_id != null) return USEBYPROJKPI[m.project_id + '|' + m.indicator_id] || null;
    return USEFREE[m.indicator_id + '|' + (m.country_iso3 || '')] || null;
  }
  /** How many distinct KPIs a list of uses covers (a KPI recurs once per project). */
  function distinctKpiCount(uses){ var seen = {}, n = 0; (uses || []).forEach(function (u){ if (!seen[u.kpiId]){ seen[u.kpiId] = 1; n++; } }); return n; }
  /** Every use of a KPI (by database id) in the plan in view. */
  function usesOfKpi(kpiId){ return IND.filter(function (r){ return r.kpiId === kpiId; }); }
  /** The KPI definitions (framework KPIs) of a plan - one row each. */
  function planKpis(planId){
    return DB.tables.indicator.filter(function (i){ return !i.secondary && indicatorPlanId(i) === planId; });
  }

  /** Build the uses of a plan. Shared by the dashboard (active plan) and the
   *  monthly reports. `upToISO` (optional) keeps only activities dated on/before it. */
  function buildUses(planId){
    var uses = [], rById = DB._idx.resultById;
    var byProjKpi = {}, free = {};
    function base(def, kind, project, iso, link, id){
      var res = def.result_id != null ? rById[def.result_id] : null;
      var co = iso ? DB._idx.countryByIso[iso] : null;
      return {
        id: id, kpiId: def.id, kind: kind, def: def, link: link || null,
        raw: kpiRawFor(def, link, kind === 'free'),
        name: def.name, unit: kpiUnit(def), type: kpiType(def),
        result: res, project: project || null, secondary: kind === 'secondary',
        level: res ? res.level : (kind === 'secondary' ? 'secondary' : 'output'),
        sdg: res ? res.sdg : null,
        iso: iso || null, region: project ? project.region : (co ? co.region : null),
        programme: iso ? (DB._idx.programmeByIso[iso] || null) : null,
        series: []
      };
    }
    DB.tables.project.forEach(function (p){
      ...
      (DB._idx.projectKpiByProject[p.id] || []).forEach(function (pk){
        var def = DB._idx.indicatorById[pk.indicator_id]; if (!def || def.secondary) return;
        var u = base(def, 'project', p, p.country_iso3, pk, USE_PROJECT + pk.id);
        byProjKpi[p.id + '|' + def.id] = u; uses.push(u);
      });
      (DB._idx.secondaryByProject[p.id] || []).forEach(function (def){
        var u = base(def, 'secondary', p, p.country_iso3, null, USE_SECONDARY + def.id);
        byProjKpi[p.id + '|' + def.id] = u; uses.push(u);
      });
    });
    // activities: into their project's use, or a per-country free use
    var freeSeq = 0;
    DB.measurementsByDate().forEach(function (m){
      var def = DB._idx.indicatorById[m.indicator_id]; if (!def || indicatorPlanId(def) !== planId) return;
      if (m.project_id != null){
        var u = byProjKpi[m.project_id + '|' + def.id];
        if (!u){
          // an activity of a project that does not (or no longer) carries the KPI:
          // still that project's work, measured without a target
          var p = DB._idx.projectById[m.project_id]; if (!p || p.plan_id !== planId) return;
          u = base(def, 'free', p, p.country_iso3, null, USE_FREE + (++freeSeq));
          byProjKpi[m.project_id + '|' + def.id] = u; uses.push(u);
        }
        u.series.push(m);
        return;
      }
      if (def.secondary) return;
      var key = def.id + '|' + (m.country_iso3 || '');
      var f = free[key];
      if (!f){ f = free[key] = base(def, 'free', null, m.country_iso3, null, USE_FREE + (++freeSeq)); uses.push(f); }
      f.series.push(m);
    });
    return { uses: uses, byProjKpi: byProjKpi, free: free };
  }
  ...
  function enrich() {
    ...
    // Everything downstream derives from IND / FRAMEWORK / PROJECTS, so building
    // the uses of the active plan scopes the whole app to that plan.
    var built = buildUses(S.plan);
    IND = built.uses; USEBYPROJKPI = built.byProjKpi; USEFREE = built.free;
    IND.forEach(function (r) {
      var st = computeStatus(r);
      r.progress = st.progress; r.performance = st.performance;
      r.progressCode = st.progressCode; r.perfCode = st.perfCode;
      r.status = st.code; r.frac = st.frac; r.perf = st.perf;
      r.value = st.value; r.latest = st.latest;
      r.updated = st.latest ? st.latest.date : null;
      r.updatedMs = st.latest ? Date.parse(st.latest.date) : null;
    });
    INDBYID = {};
    ...
    IND.forEach(function (r) {
      ...
      // distinct reporters across this use's activities (drives the Users filter)
      ...
      // the project this use belongs to, and its donor / partner - drives the
      // Projects, Donors and Partners filters and insights dimensions
      var p = r.project;
      // only adopted / own KPIs tie a project to the facets (p.kpis holds exactly
      // those) - activities on a KPI the project never adopted are unmeasured
      r.projectIds = p && r.kind !== 'free' ? [p.id] : [];
      r.donorIds = p && p.donor_id != null ? [p.donor_id] : [];
      r.partnerIds = p && p.partner_id != null ? [p.partner_id] : [];
      r.projPrimary = p;
      r.donorPrimary = p && p.donor_id != null ? DB._idx.donorById[p.donor_id] : null;
      r.partnerPrimary = p && p.partner_id != null ? DB._idx.partnerById[p.partner_id] : null;
      var bset = {};
   ...
   *  distinct projects (see S.countBasis). Geography comes from the activity's own
   *  country (its project's, or the one chosen when it was logged without one);
   *  donor/partner from the PROJECT; KPI identity + performance from its USE;
   *  reporter and beneficiary types from the measurement itself. */
  function buildActivities() {
    ACTS = []; ACTBYMEAS = {}; MEAS_NOPROJ = [];
    (DB.tables.measurement || []).forEach(function (m) {
      var ind = useOfMeas(m); if (!ind) return;
      var proj = m.project_id != null ? DB._idx.projectById[m.project_id] : null;
      // an activity with no project attribution still happened, and still belongs
      // on the map - kept aside here so the map does not have to rescan 36k rows
      if (!proj && m.place_lat != null && m.place_lng != null) MEAS_NOPROJ.push(m);
      var iso = proj ? proj.country_iso3 : (m.country_iso3 || ind.iso);
      var co = iso ? DB._idx.countryByIso[iso] : null;
      var prog = iso ? DB._idx.programmeByIso[iso] : null;
      var bset = {};
      (DB._idx.benByMeasurement[m.id] || []).forEach(function (b) { if (b.type_id != null) bset[b.type_id] = 1; });
      ACTS.push({
        id: m.id, m: m,
        ...
        iso: iso,
        region: proj ? proj.region : (co ? co.region : null),
  ...
  function buildProjects() {
    PROJECTS = DB.tables.project.filter(function (p) { return p.plan_id === S.plan; }).map(function (p) {
      var prim = (DB._idx.projectKpiByProject[p.id] || [])
        .map(function (pk) { return USEBYPROJKPI[p.id + '|' + pk.indicator_id]; }).filter(function (u) { return u && u.kind === 'project'; });
      var secs = (DB._idx.secondaryByProject[p.id] || [])
        .map(function (i) { return USEBYPROJKPI[p.id + '|' + i.id]; }).filter(Boolean);
  ...
  function matchSearch(r, q) {
    if (!q) return true;
    q = fold(q);
    return (r.name && hay(r.name).indexOf(q) >= 0)
      || (r.result && hay(r.result.statement).indexOf(q) >= 0)
      || (r.programme && hay(r.programme.name).indexOf(q) >= 0);
  }
  ...
  /** Inverse of proj(): world (viewBox) units back to [lng, lat]. Needed in globe
   *  mode, where markers are placed by handing lng/lat to MapLibre. */
  function unproj(x, y) {
    var lng = x / VB.W * 360 - 180;
    var n = Math.PI * (1 - 2 * (y / VB.W));
    return [ lng, (2 * Math.atan(Math.exp(n)) - Math.PI / 2) * 180 / Math.PI ];
  }
  /** True while the basemap is drawn as a 3D sphere rather than a flat plane. */
  function isGlobe() { return S.mapMode === 'globe'; }
  ...
  /** Every located activity across the whole database, unfiltered - used to
   *  compute the default framing. Project attribution is optional, so this is
   *  keyed on having a POINT, not on having a project. */
  function allProjectLocations() {
    var pts = [];
    DB.tables.measurement.forEach(function (m) {
      if (m.place_lat != null && m.place_lng != null)
  ...
  function animateView(target) {
    // Globe mode: hand the framing to MapLibre's own camera ease. `target` is NOT
    // clamped here - the sphere wraps, and clamping a wider-than-world rect would
    // slide the centre to the wrong longitude.
    if (isGlobe()) { view = target; globeCameraTo(target, true); return; }
    clampObj(target);
  ...
  function fitToLocs(locs, animate) {
    ...
    if (isGlobe()) { view = target; globeCameraTo(target, animate !== false); redrawClusters(); }
    else if (animate === false) { view = clampObj(target); setView(); redrawClusters(); renderTiles(); }
  ...
  // In globe mode the overlay is no longer a window onto the flat world - MapLibre
  // owns the camera and every marker is placed in screen pixels - so the viewBox
  // becomes a 1:1 pixel space instead.
  function setView() {
    if (!mapSvg) return;
    if (isGlobe()) { setGlobeViewBox(); return; }
  ...
  /** Globe mode: the SVG overlay is a plain screen-pixel canvas (1 unit = 1 px), so
   *  its viewBox must track the container - and the container changes size without
   *  the camera moving (window resize, or the Map tab being revealed after being
   *  measured at 0x0 while hidden). `view` is re-derived at the same time: the world
   *  width a zoom level covers depends on the container width, and the cluster cell
   *  size is read off view.w. Idempotent and cheap, so it can run every frame. */
  function setGlobeViewBox() {
    if (!mapSvg) return;
    var r = mapSvg.getBoundingClientRect();
    var w = Math.max(1, Math.round(r.width)), h = Math.max(1, Math.round(r.height));
    var want = '0 0 ' + w + ' ' + h;
    if (mapSvg.getAttribute('viewBox') !== want) mapSvg.setAttribute('viewBox', want);
    // mlMap.project() answers in the GL TRANSFORM's pixel space, and the overlay only
    // lines up while that space is the same size as the SVG. On a cold load the canvas
    // is still at its construction size when the first markers are drawn, so they land
    // in the wrong space until some later event happens to resize it. Keep them equal.
    if (mlMap && mlMap.transform && (mlMap.transform.width !== w || mlMap.transform.height !== h)) {
      try { mlMap.resize(); } catch (e) {}
    }
    syncViewFromCamera();
  }
  ...
  function ensureBasemap() {
    ...
    mlMap = new maplibregl.Map({
      container: 'basemap',
      // Built interactive so every gesture handler exists, then switched off right
      // away: flat mode is driven by the SVG overlay, and globe mode re-enables the
      // handlers through setGlobeInteractive() so MapLibre can spin the sphere.
      interactive: true,
    ...
    setGlobeInteractive(isGlobe());
    mlMap.on('style.load', function () { recolorBoundaries(); collapseAttribution(); applyProjection(); syncCamera(); });
    ...
    // The projection rides along for the same reason: setStyle() resets it to mercator.
    mlMap.on('styledata', function () { recolorBoundaries(); collapseAttribution(); applyProjection(); });
    // Globe mode: MapLibre drives the camera, so every camera change has to be read
    // back into `view` and the marker overlay redrawn on top of the new frame.
    mlMap.on('move', function () { if (!isGlobe()) return; syncViewFromCamera(); redrawGlobeOverlay(); });
    // …but the camera is not the only thing that moves the markers. setProjection()
    // only bites once the style has loaded, and the mercator<->globe swap then
    // ANIMATES over several frames - all without firing 'move'. On a cold load
    // straight into globe mode that left the overlay frozen at the positions it was
    // drawn at mid-transition: points near the centre looked fine while distant ones
    // (Asia, from an Africa-centred view) sat out past the limb, until the first pan
    // redrew them. A container resize does the same. So watch what the transform
    // actually produces, and redraw whenever it changes.
    mlMap.on('render', function () {
      if (!isGlobe() || globeProbe() === _globeProbe) return;
      redrawGlobeOverlay();
    });
    // The overlay is pointer-transparent in globe mode, so a click on bare map (ocean,
    // land, anywhere off a marker) reaches the canvas instead of the SVG - dismiss the
    // location popup from here, and let the same click stop or restart the rotation.
    // MapLibre only emits 'click' when the pointer stayed put, so panning the globe
    // does not count as a click.
    mlMap.on('click', function () { if (!isGlobe()) return; closeLocPop(); toggleSpinOnMapClick(); });
    // A finger or cursor on the sphere owns it outright: the auto-rotation steps
    // aside the moment the pointer goes down and eases back in on release. Tracked
    // as a flag (not a counter) so a lost pointerup can't wedge it off for good.
    var cvc = mlMap.getCanvasContainer ? mlMap.getCanvasContainer() : null;
    if (cvc) cvc.addEventListener('pointerdown', function () { _spinHold = true; });
    window.addEventListener('pointerup', function () { _spinHold = false; });
    window.addEventListener('pointercancel', function () { _spinHold = false; });
    window._mlMap = mlMap;   // debug handle
  }
  // Convert a world-mercator view rect → MapLibre {center, zoom}.
  function viewToCamera(v) {
    if (!mapSvg || !v || !v.w) return null;
    var elemW = mapElemW();
    var z = Math.log(1000 * elemW / (512 * v.w)) / Math.LN2;
    z = Math.max(0, Math.min(24, z));
    var cxW = v.x + v.w / 2, cyW = Math.max(0.001, Math.min(999.999, v.y + v.h / 2));
    var lng = cxW / 1000 * 360 - 180;
    lng = ((lng + 180) % 360 + 360) % 360 - 180;              // wrap into [-180, 180]
    var lat = (2 * Math.atan(Math.exp(Math.PI * (1 - 2 * (cyW / 1000)))) - Math.PI / 2) * 180 / Math.PI;
    if (!isFinite(lng) || !isFinite(lat) || !isFinite(z)) return null;
    return { center: [lng, lat], zoom: z };
  }
  // Flat mode only: mirror the app's `view` rect onto the MapLibre camera. In globe
  // mode the arrow points the other way (the user drives MapLibre directly), so
  // pushing the camera here would fight the interaction - see syncViewFromCamera.
  function syncCamera() {
    if (!mlMap || isGlobe()) return;
    var cam = viewToCamera(view);
    if (cam) mlMap.jumpTo(cam);
  }
  // Globe mode: read the MapLibre camera back into `view`. Clustering, the default-
  // view framing and the flat/globe handover all keep working off `view` as before,
  // even though the overlay no longer draws in world units.
  function syncViewFromCamera() {
    if (!mlMap || !mapSvg) return;
    var w = 1000 * mapElemW() / (512 * Math.pow(2, mlMap.getZoom()));
    var c = mlMap.getCenter();
    view.w = w; view.h = w * mapAspect();
    view.x = (c.lng + 180) / 360 * 1000 - w / 2;
    view.y = mercY(c.lat) * 1000 - view.h / 2;
  }
  /** Eased globe camera move. When the page is hidden rAF is paused, so an ease
   *  would freeze mid-flight and leave the view stranded - jump instead, exactly as
   *  animateView() does for the flat map. */
  function globeEase(opts) {
    if (!mlMap) return;
    if (!document.hidden) { mlMap.easeTo(opts); return; }
    var o = {}; for (var k in opts) if (k !== 'duration' && k !== 'essential') o[k] = opts[k];
    mlMap.jumpTo(o);
  }
  // How much of the container's shorter side the globe should span when it is
  // framed from a view rect. Short of 1 so the markers - which float above their
  // ground point - clear the top limb.
  var GLOBE_FILL = 0.88;
  /** Zoom at which the globe's silhouette spans `frac` of the container's shorter
   *  side. The sphere has radius R = worldSize/2π px and the camera sits d px away
   *  (cameraToCenterDistance), so the silhouette's screen radius is
   *  d·tan(asin(R/(R+d))). Inverting that for R gives the zoom. */
  function globeFitZoom(frac) {
    if (!mlMap || !mapSvg) return null;
    var r = mapSvg.getBoundingClientRect();
    var want = frac * Math.min(r.width, r.height) / 2;      // target silhouette radius, px
    var d = mlMap.transform && mlMap.transform.cameraToCenterDistance;
    if (!isFinite(d) || d <= 0) d = 0.5 * (r.height || 600) / Math.tan(0.6435 / 2);
    var u = want / d, s = u / Math.sqrt(1 + u * u);         // s = R/(R+d)
    if (!(s > 0 && s < 1)) return null;
    var R = d * s / (1 - s);
    var z = Math.log(R * 2 * Math.PI / 512) / Math.LN2;
    return isFinite(z) ? z : null;
  }
  // Fingerprint of what the GL transform currently projects to. Two widely-separated
  // reference points are enough to catch a projection swap, a transition frame, a
  // zoom or a resize - anything that would move the markers.
  var _globeProbe = '';
  function globeProbe() {
    if (!mlMap) return '';
    try {
      var a = mlMap.project([0, 0]), b = mlMap.project([90, 45]);
      return a.x.toFixed(1) + ',' + a.y.toFixed(1) + ';' + b.x.toFixed(1) + ',' + b.y.toFixed(1);
    } catch (e) { return ''; }
  }
  /** Redraw the marker overlay and record the transform state it was drawn for, so
   *  the 'render' watcher doesn't redraw the same frame twice. */
  function redrawGlobeOverlay() { _globeProbe = globeProbe(); redrawClusters(); }

  /** Move the globe camera to the framing described by a world-mercator view rect.
   *  The rect's zoom frames the extent as a FLAT rectangle, which on a sphere leaves
   *  wide empty margins around the disc - so it is never allowed to sit below the
   *  zoom that fills the container. A tighter extent (one country, say) still wins,
   *  because its own zoom is the higher of the two. */
  function globeCameraTo(v, animate) {
    if (!mlMap) return;
    var cam = viewToCamera(v); if (!cam) return;
    var fill = globeFitZoom(GLOBE_FILL);
    if (fill != null && fill > cam.zoom) cam.zoom = Math.max(0, Math.min(24, fill));
    if (animate) globeEase({ center: cam.center, zoom: cam.zoom, duration: 620, essential: true });
    else mlMap.jumpTo(cam);
  }
  /** Push S.mapMode onto the MapLibre style. Re-applied on every style load, since
   *  a theme swap (setStyle) drops the projection back to mercator. */
  function applyProjection() {
    if (!mlMap || typeof mlMap.setProjection !== 'function') return;
    try { mlMap.setProjection({ type: isGlobe() ? 'globe' : 'mercator' }); } catch (e) {}
  }
  // The map is built with every gesture handler present but switched off: flat mode
  // is driven by the SVG overlay, globe mode hands the gestures back to MapLibre.
  var GLOBE_HANDLERS = ['dragPan', 'scrollZoom', 'dragRotate', 'touchZoomRotate', 'doubleClickZoom', 'keyboard', 'boxZoom'];
  function setGlobeInteractive(on) {
    if (!mlMap) return;
    GLOBE_HANDLERS.forEach(function (h) {
      try { if (mlMap[h]) mlMap[h][on ? 'enable' : 'disable'](); } catch (e) {}
    });
    // Pitch stays locked: the marker occlusion test assumes the camera sits directly
    // over the map centre, and a tilted globe would hide/show the wrong markers.
    try { mlMap.setMaxPitch(0); } catch (e) {}
    try { mlMap.getCanvas().style.cursor = on ? 'grab' : ''; } catch (e) {}
  }

  function drawBase() {
    mapSvg = $('#map');
    mapSvg.innerHTML = '';
    _sphGrads = {};          // gradient defs are gone with the innerHTML wipe
    ...
    mapSvg.appendChild(svgEl('defs', { id: 'bubdefs' }));   // sphere shading gradients (globe mode)
    mapSvg.appendChild(svgEl('g', { id: 'bubbles' }));
    // Re-assert the persisted projection: a reload can come back straight into globe.
    mapSvg.classList.toggle('globe', isGlobe());
    setGlobeInteractive(isGlobe());
    applyProjection();
    updateGlobeBtn();
    setView();
    if (isGlobe()) globeCameraTo(view, false);   // setView() can't drive the globe camera
    syncSpin();
    renderTiles();
  ...
  // ---- flat <-> globe toggle ------------------------------------------------
  /** Swap the projection in place: same container, same controls, same legend -
   *  only how the world (and its markers) is drawn changes. The camera is carried
   *  across in both directions, so the view never jumps. */
  function setMapMode(mode) {
    mode = (mode === 'globe') ? 'globe' : 'flat';
    if (mode === S.mapMode) return;
    ensureBasemap();
    // No MapLibre (or a build without globe support) = no globe; stay flat.
    if (mode === 'globe' && (!mlMap || typeof mlMap.setProjection !== 'function')) return;
    closeLocPop();
    S.mapMode = mode;
    if (mapSvg) mapSvg.classList.toggle('globe', isGlobe());
    if (isGlobe()) {
      setGlobeInteractive(true);
      applyProjection();
      setGlobeViewBox();            // overlay becomes a 1:1 pixel canvas first…
      globeCameraTo(view, false);   // …then adopt the flat camera, so nothing jumps
    } else {
      setGlobeInteractive(false);
      syncViewFromCamera();         // adopt wherever the user left the globe
      try { mlMap.jumpTo({ bearing: 0, pitch: 0 }); } catch (e) {}
      applyProjection();
      view.w = Math.min(view.w, VB.W); view.h = view.w * mapAspect();
      clampView();
      setView();                    // writes the world viewBox back + re-syncs the camera
    }
    updateGlobeBtn();
    syncSpin();          // starts the rotation with the globe, stands it down with the flat map
    redrawClusters();
    renderTiles();
    persist();
  }
  function updateGlobeBtn() {
    var b = $('#zGlobe'); if (!b) return;
    var on = isGlobe();
    b.classList.toggle('on', on);
    b.setAttribute('aria-pressed', on ? 'true' : 'false');
    b.title = on ? 'Switch to flat map' : 'Switch to globe view';
  }

  // ---- globe auto-rotation --------------------------------------------------
  // The sphere turns the way the Earth does - eastward, which for a camera parked
  // above it means the centre longitude walking west. ~70 seconds per revolution at
  // world zoom: slow enough to read a label, alive enough to read as a globe.
  var SPIN_DPS = 5;      // degrees of longitude per second while the whole sphere is in frame
  var SPIN_RAMP = 0.55;  // seconds to ease up to (and back down from) full speed
  var _spinRAF = null, _spinLast = 0, _spinVel = 0, _spinHold = false;

  function mapPaneHidden() { var mv = $('#mapView'); return !!(mv && mv.classList.contains('hide')); }
  /** Angular speed for the current zoom. Constant in degrees/sec while the globe is
   *  framed whole, then halved per zoom level beyond that - at a country-level zoom
   *  the same angular rate would race across the screen. */
  function spinSpeed() {
    var z = mlMap ? mlMap.getZoom() : 0, base = globeFitZoom(GLOBE_FILL);
    if (base == null || !isFinite(z)) return SPIN_DPS;
    return SPIN_DPS * Math.pow(2, Math.min(0, base - z));
  }
  /** Conditions that SUSPEND the turn without touching the play/pause state - the
   *  pointer is down on the sphere, a camera ease (zoom button, reset, re-fit) owns
   *  it, a location popup is pinned to screen coordinates the spin would slide out
   *  from under, the map pane is hidden, or the page is in the background. Every one
   *  of them clears itself, and the rotation picks up where it left off. */
  function spinBlocked() {
    if (!mlMap || !isGlobe() || document.hidden || _spinHold) return true;
    try { if (mlMap.isEasing() || mlMap.isZooming() || mlMap.isRotating()) return true; } catch (e) {}
    if (mapPaneHidden()) return true;
    var pop = $('#locPop');
    return !!(pop && pop.classList.contains('on'));
  }
  function spinTick(ts) {
    _spinRAF = null;
    if (!mlMap || !isGlobe()) { spinStop(); return; }
    // A stalled frame (a heavy re-render, a backgrounded tab) slows that frame's step
    // instead of teleporting the camera across the gap.
    var dt = _spinLast ? Math.min(0.05, (ts - _spinLast) / 1000) : 0;
    _spinLast = ts;
    // Grabbing the globe hands it over at once; starting and pausing ease in and out.
    if (spinBlocked()) _spinVel = 0;
    else _spinVel += ((S.globeSpin ? spinSpeed() : 0) - _spinVel) * (1 - Math.exp(-dt / SPIN_RAMP));
    if (!S.globeSpin && _spinVel < 0.02) { spinStop(); return; }   // glided to rest: stand down
    if (_spinVel > 0 && dt > 0) {
      var c = mlMap.getCenter(), lng = c.lng - _spinVel * dt;
      // Keep the centre inside [-180, 180): syncViewFromCamera() turns it straight
      // into a world-x, so an unwrapped longitude would strand the flat map at the
      // world edge the moment the projection is switched back.
      if (lng < -180) lng += 360;
      mlMap.jumpTo({ center: [lng, c.lat] });   // fires 'move' → overlay redraw
    }
    _spinRAF = requestAnimationFrame(spinTick);
  }
  function spinStart() {
    if (_spinRAF != null || !mlMap || !isGlobe()) return;
    _spinLast = 0;
    _spinRAF = requestAnimationFrame(spinTick);
  }
  function spinStop() {
    if (_spinRAF != null) cancelAnimationFrame(_spinRAF);
    _spinRAF = null; _spinVel = 0; _spinLast = 0;
  }
  /** Single entry point, safe to call from anywhere: the loop runs exactly while the
   *  globe is up, rotation is armed and the map pane is on screen. A *pause* is not
   *  handled here - the running loop eases down to zero and ends itself. */
  function syncSpin() {
    if (S.globeSpin && isGlobe() && !mapPaneHidden()) spinStart();
    else if (!isGlobe() || mapPaneHidden()) spinStop();
    updateSpinBtn();
  }
  ...
  /** A click on bare map is the globe's stop/go switch - it drives the SAME state as
   *  the pause/play button, so the control always mirrors what the sphere is doing.
   *  A click on a bubble only ever stops it (the popup that opens is pinned to screen
   *  coordinates), and the next click on bare map dismisses the popup and lets it go
   *  again. Nothing to toggle on the flat map. */
  function toggleSpinOnMapClick() { if (isGlobe()) setGlobeSpin(!S.globeSpin); }
  function holdSpinForPopup() { if (isGlobe()) setGlobeSpin(false); }
  function updateSpinBtn() {
    var b = $('#zSpin'); if (!b) return;
    var on = !!S.globeSpin;
    b.classList.toggle('hide', !isGlobe());   // flat map: nothing to rotate
    b.classList.toggle('on', on);
    b.classList.toggle('paused', !on);
    b.setAttribute('aria-pressed', on ? 'true' : 'false');
    b.title = on ? 'Pause globe rotation' : 'Resume globe rotation';
  }
  ...
  function buildLocations(projs) {
    ...
    // Activities with no project attribution join their location the same way -
    // they are counted, they colour the bubble through their KPI's status, and
    // they simply carry no project in the location's project list.
    MEAS_NOPROJ.forEach(function (m) {
      if (!measPassesFilters(m)) return;
      var key = (+m.place_lat).toFixed(3) + ',' + (+m.place_lng).toFixed(3);
      var loc = locs[key] || (locs[key] = { lat: +m.place_lat, lng: +m.place_lng, name: m.place_name, acts: 0, projIds: {}, actsByProj: {} });
      loc.acts++;
      (loc.freeInds = loc.freeInds || []).push(useOfMeas(m));
      if (!loc.name && m.place_name) loc.name = m.place_name;
    });
    return Object.keys(locs).map(function (k) {
      var l = locs[k];
      l.projList = Object.keys(l.projIds).map(function (id) { return projById[+id]; }).filter(Boolean);
      var free = (l.freeInds || []).filter(Boolean);
      l.status = l.projList.length
        ? domStatus(l.projList, function (p) { return projStat(p).code; })
        : domStatus(free, function (r) { return r.status; });
      l.region = l.projList[0] ? l.projList[0].region : (free[0] ? free[0].region : null);
      l.iso = l.projList[0] ? l.projList[0].iso : (free[0] ? free[0].iso : null);
      ...
      if (l.pillar == null) l.pillar = domFreq(free, function (r) { return r.sdg; });
  ...
  // ---- cluster scale: quantised to whole zoom steps -------------------------
  // Deriving the grid cell straight from view.w re-grouped the markers on every
  // wheel notch: members kept swapping cells, so each cluster's centroid, radius
  // and count twitched frame by frame - which is what read as the map shaking.
  // The cell is now snapped to whole zoom levels, so the markers diffuse at
  // intervals (~4 wheel notches, ~3 zoom-button clicks) and hold perfectly still
  // in between; only the camera moves under them.
  var CLUSTER_STEP = 1;   // zoom levels between re-groupings
  /** Quantised zoom level driving the cluster grid. 0 = whole world (the coarsest
   *  grid), +1 per doubling of scale. */
  function clusterLevel() {
    var w = Math.max(1e-6, view.w);
    var z = Math.log(VB.W / w) / Math.LN2;   // continuous zoom implied by the view width
    return Math.max(0, Math.floor(z / CLUSTER_STEP) * CLUSTER_STEP);
  }
  /** Grid cell in world units for the current quantised level. Matches the old
   *  view.w/17 exactly at whole zoom levels, and holds that value until the next. */
  function clusterCell() { return (VB.W / Math.pow(2, clusterLevel())) / 17; }

  // The grouping only changes when the level does, so it is computed once per
  // interval instead of once per wheel/move frame. Keyed on the locations array
  // itself, which renderBubbles() replaces whenever the filters change.
  var _clusterCache = null, _clusterCacheLvl = null, _clusterCacheSrc = null;
  function clustersForView() {
    var lvl = clusterLevel();
    if (_clusterCache && _clusterCacheSrc === _mapLocs && _clusterCacheLvl === lvl) return _clusterCache;
    _clusterCacheSrc = _mapLocs; _clusterCacheLvl = lvl;
    _clusterCache = clusterLocs(_mapLocs);
    return _clusterCache;
  }

  /** Grid-cluster locations. The cell shrinks a step at a time as you zoom, so
   *  nearby markers merge when zoomed out and diffuse (split apart) at intervals. */
  ...
  // Blend toward black by `amt` (0..1) - the terminator side of a globe marker.
  // NOT shade(): that one is the HSL tint/shade helper the pills and legends run
  // on, and a second shade() here would hoist over it and darken all of them.
  ...
  // Globe markers are shaded spheres, which needs one radial gradient per colour.
  // The palette is small and stable, so the defs are built once and reused (they
  // live in <defs id="bubdefs">, which drawBase() wipes along with the overlay).
  var _sphGrads = {};
  function sphereGrad(col) {
    var key = 'sph' + String(col).replace(/[^0-9a-z]/gi, '');
    if (_sphGrads[key]) return key;
    var defs = $('#bubdefs'); if (!defs) return key;
    var g = svgEl('radialGradient', { id: key, cx: '34%', cy: '27%', r: '78%' });
    [[0, lighten(col, 0.62)], [0.45, col], [1, darken(col, 0.42)]].forEach(function (s) {
      g.appendChild(svgEl('stop', { offset: s[0], 'stop-color': s[1] }));
    });
    defs.appendChild(g);
    _sphGrads[key] = 1;
    return key;
  ...
  function redrawClusters() {
    // Globe: re-anchor the pixel viewBox (and `view`) to the container before
    // clustering, so a resize or a tab reveal can't leave markers drawn at the
    // wrong scale. Deliberately BEFORE the layer is cleared - it can resize the GL
    // canvas, which re-enters here; letting that nested pass finish against an
    // already-emptied layer would leave two sets of markers stacked up.
    if (isGlobe()) setGlobeViewBox();
    var layer = $('#bubbles'); if (!layer) return; layer.innerHTML = '';
    hideMapTip();   // the hovered bubble is about to be replaced; no mouseout will fire
    var clusters = clustersForView();
    ...
    var acts = isActs();
    if (isGlobe()) { drawGlobeClusters(layer, clusters, acts); return; }
  ...
  // ---- globe markers --------------------------------------------------------
  /** Unit vector on the sphere for a lng/lat, in a fixed world frame. */
  function unitVec(lng, lat) {
    var a = lng * Math.PI / 180, b = lat * Math.PI / 180, cb = Math.cos(b);
    return [cb * Math.cos(a), Math.sin(b), cb * Math.sin(a)];
  }
  /** Build the back-of-the-globe test for the CURRENT camera. Pitch is locked to 0,
   *  so the visible cap is centred on the map centre; its half-angle is the horizon
   *  of a camera sitting `cameraToCenterDistance` px above a globe of radius
   *  worldSize/2π px. Returns 0 (hidden) … 1 (fully visible), fading across the last
   *  slice so markers dissolve at the limb instead of popping. */
  function globeCuller() {
    var z = mlMap.getZoom();
    // past the globe→mercator crossover the whole viewport is front-facing
    if (z >= 10) return function () { return 1; };
    var tr = mlMap.transform || {};
    var R = (tr.worldSize || (512 * Math.pow(2, z))) / (2 * Math.PI);
    var d = tr.cameraToCenterDistance;
    if (!isFinite(d) || d <= 0) {
      var h = (mapSvg && mapSvg.getBoundingClientRect().height) || 600;
      d = 0.5 * h / Math.tan(0.6435 / 2);   // MapLibre's default 36.87° vertical FOV
    }
    var cut = R / (R + d);                  // cos(half-angle of the visible cap)
    var band = Math.max(0.015, (1 - cut) * 0.06);
    var c = mlMap.getCenter(), v0 = unitVec(c.lng, c.lat);
    return function (lng, lat) {
      var v = unitVec(lng, lat);
      var dot = v[0] * v0[0] + v[1] * v0[1] + v[2] * v0[2];
      if (dot <= cut) return 0;
      return dot >= cut + band ? 1 : (dot - cut) / band;
    };
  }
  /** Globe mode markers: a shaded sphere floating above its ground point, tied down
   *  by a stem and casting an ellipse shadow. Screen positions come from
   *  mlMap.project(), so markers track the sphere exactly through spin, zoom and
   *  rotation; anything on the far side is culled. Sizes, colours, counts and click
   *  behaviour are the flat map's - only the drawing changes. */
  function drawGlobeClusters(layer, clusters, acts) {
    if (!mlMap) return;
    var visible = globeCuller(), drawn = [];
    clusters.forEach(function (c) {
      var ll = unproj(c.x, c.y);
      var vis = visible(ll[0], ll[1]);
      if (vis <= 0) return;
      var p; try { p = mlMap.project(ll); } catch (e) { return; }
      if (!p || !isFinite(p.x) || !isFinite(p.y)) return;
      c.__p = p; c.__vis = vis;
      drawn.push(c);
    });
    // painter's order: the limb first, then markers facing the camera; ties broken
    // by screen depth so a nearer sphere overlaps the one behind it
    drawn.sort(function (a, b) { return (a.__vis - b.__vis) || (a.__p.y - b.__p.y); });
    drawn.forEach(function (c) {
      var metric = acts ? c.acts : c.nProj, single = c.locs.length === 1;
      var base = clusterColor(c), col = lighten(base, 0.22);
      var ck = {}; c.locs.forEach(function (l){ ck[l.lng.toFixed(4) + ',' + l.lat.toFixed(4)] = 1; });
      var splittable = Object.keys(ck).length > 1;
      var rPx = single ? 4.6 : Math.min(13, 7 + Math.log(Math.max(1, metric)) / Math.LN10 * 4.5);
      var x = c.__p.x, gy = c.__p.y;          // ground anchor (the actual coordinate)
      var lift = rPx * 1.7 + 5;               // how far the sphere hovers above it
      var sy = gy - lift;                     // sphere centre
      var g = svgEl('g', { class: 'bubg', opacity: c.__vis.toFixed(3) });
      g.appendChild(svgEl('ellipse', { class: 'bubshadow', cx: x.toFixed(1), cy: gy.toFixed(1),
        rx: (rPx * 0.82).toFixed(2), ry: (rPx * 0.30).toFixed(2), fill: '#000', 'fill-opacity': 0.26 }));
      g.appendChild(svgEl('line', { class: 'bubstem', x1: x.toFixed(1), y1: gy.toFixed(1),
        x2: x.toFixed(1), y2: (sy + rPx * 0.6).toFixed(1), stroke: darken(base, 0.35),
        'stroke-width': 1.1, 'stroke-opacity': 0.55 }));
      var circle = svgEl('circle', { cx: x.toFixed(1), cy: sy.toFixed(1), r: rPx.toFixed(2),
        class: 'bub' + (single ? ' single' : ' cluster'), fill: 'url(#' + sphereGrad(col) + ')',
        stroke: darken(col, 0.55), 'stroke-width': (single ? 0.5 : 0.7), 'stroke-opacity': 0.7 });
      circle.__cluster = c; circle.__single = single;
      g.appendChild(circle);
      var hx = x - rPx * 0.30, hy = sy - rPx * 0.40;
      g.appendChild(svgEl('ellipse', { class: 'bubgloss', cx: hx.toFixed(1), cy: hy.toFixed(1),
        rx: (rPx * 0.30).toFixed(2), ry: (rPx * 0.19).toFixed(2), fill: '#fff', 'fill-opacity': 0.38,
        transform: 'rotate(-28 ' + hx.toFixed(1) + ' ' + hy.toFixed(1) + ')' }));
      // Show the count only on clusters that can still be split apart by zooming.
      if (!single && splittable) {
        var fpx = Math.min(rPx * 0.95, 5 + Math.log(Math.max(1, metric)) / Math.LN10 * 2);
        var txt = svgEl('text', { x: x.toFixed(1), y: sy.toFixed(1), dy: '0.34em', class: 'bubcount', 'font-size': fpx.toFixed(2) });
        txt.textContent = fmt(metric);
        g.appendChild(txt);
      }
      layer.appendChild(g);
  ...
  function renderBubbles() {
    ...
    redrawClusters();
    syncSpin();   // the map pane is on screen: pick the rotation back up
  }
  ...
  /** Hide the map hover tip. Module-scoped because a redraw can remove the bubble
   *  under the cursor without ever firing mouseout. */
  function hideMapTip() { var t = $('#tip'); if (t) t.classList.remove('on'); }
  function openClusterPopup(c, e) {
    ...
    // The results box opens on the KPIs measured here, so the button says so;
    // the activity count stays in the sub-line above as the underlying volume.
    var lpKpis = kpiCountOf(activitiesAtCluster(c));
    html += '<button class="lp-open" type="button">▤ Results - ' + fmt(lpKpis) + ' KPI' + (lpKpis === 1 ? '' : 's') + '</button>';
    if (multi && splittable) html += '<button class="lp-zoom" type="button">⌕ Zoom In</button>';
  ...
  function initMapInteractions() {
    ...
    // In globe mode MapLibre owns pan/zoom. The overlay itself is pointer-transparent
    // there, but a press or wheel that lands on a marker still bubbles up to this
    // element - so both handlers bail out rather than dragging the flat view.
    svg.addEventListener('mousedown', function (e) {
      if (isGlobe()) return;
      dragging = true; hideMapTip(); svg.classList.add('drag'); sx = e.clientX; sy = e.clientY; vx0 = view.x; vy0 = view.y;
    });
    ...
    svg.addEventListener('wheel', function (e) {
      if (isGlobe()) return;   // MapLibre's own scrollZoom handles the globe
      e.preventDefault(); zoomAt(e.deltaY < 0 ? 0.85 : 1.18, e);
    }, { passive: false });
    // Clicking a marker lists the projects there (and, for a single location, its
    // place name) - see openClusterPopup.
    svg.addEventListener('click', function (e) {
      var t = e.target;
      // Bubble: bring the globe to a halt first, then show its popup - the list is
      // pinned to the click point, so the sphere has to hold still underneath it.
      if (t.classList && t.classList.contains('bub') && t.__cluster) { holdSpinForPopup(); clusterClick(t.__cluster, e); }
      // Bare map (flat mode - in globe mode this arrives on the canvas instead).
      else if (!(e.target.closest && e.target.closest('#locPop'))) { closeLocPop(); toggleSpinOnMapClick(); }
    });
    // Hovering a bubble that covers exactly ONE location names that place. Clusters
    // stay silent - they have no single name to give, and their contents are a click
    // away in the popup.
    function moveTip(e) {
      var r = wrap.getBoundingClientRect();
      var x = e.clientX - r.left + 13, y = e.clientY - r.top + 15;
      tip.style.left = Math.max(4, Math.min(x, r.width - tip.offsetWidth - 6)) + 'px';
      tip.style.top = Math.max(4, Math.min(y, r.height - tip.offsetHeight - 6)) + 'px';
    }
    svg.addEventListener('mouseover', function (e) {
      var t = e.target;
      if (!tip || !(t.classList && t.classList.contains('bub') && t.__single && t.__cluster)) return;
      var nm = (t.__cluster.locs[0] || {}).name;
      if (!nm) return;
      tip.innerHTML = '<div class="tt"></div>';
      tip.firstChild.textContent = nm;
      tip.classList.add('on');
      moveTip(e);
    });
    svg.addEventListener('mousemove', function (e) { if (tip && tip.classList.contains('on')) moveTip(e); });
    svg.addEventListener('mouseout', function (e) {
      if (e.target.classList && e.target.classList.contains('bub')) hideMapTip();
    });
    wrap.addEventListener('mouseleave', hideMapTip);
    // The same three controls drive both projections - zoomAt() routes to the
    // MapLibre camera when the globe is up.
    ...
    var gb = $('#zGlobe');
    if (gb) gb.onclick = function () { setMapMode(isGlobe() ? 'flat' : 'globe'); };
    var sb = $('#zSpin');
    if (sb) sb.onclick = function () { setGlobeSpin(!S.globeSpin); };
    updateGlobeBtn();
    updateSpinBtn();
  }
  ...
  function zoomAt(factor, e) {
    // Globe: the same +/- buttons, expressed as a zoom-level step on the MapLibre
    // camera (halving the view width = +1 zoom level, so Δz = -log2(factor)).
    if (isGlobe()) {
      if (!mlMap) return;
      globeEase({ zoom: mlMap.getZoom() - Math.log(factor) / Math.LN2, duration: 260, essential: true });
      return;
    }
  ...
  function renderDonorFacet() {
    ...
    var q = fold(S.qDonor || ''), progIso = progIsoSet();
    var counts;
    if (isActs()) {
    ...
    var all = DB.tables.donor.slice()
      .filter(function (d) { return !q || hay(d.name).indexOf(q) >= 0 || hay(d.short_name || '').indexOf(q) >= 0; })
  ...
  function renderPartnerFacet() {
    ...
    var q = fold(S.qPartner || ''), progIso = progIsoSet();
    var counts;
    if (isActs()) {
    ...
    var all = DB.tables.partner.slice()
      .filter(function (d) { return !q || hay(d.name).indexOf(q) >= 0 || hay(d.acronym || '').indexOf(q) >= 0; })
      .sort(function (a, b) { return (rank[b.id] || 0) - (rank[a.id] || 0) || (a.name < b.name ? -1 : 1); });
  ...
  function renderProjectFacet() {
    ...
    var q = fold(S.qProject || ''), progIso = progIsoSet();
    var acts = isActs();
    ...
    var all = PROJECTS.filter(function (p) {
      ...
      return (fold((p.code || '') + ' ') + hay(p.name) + ' ' + hay(p.donor ? p.donor.name : '') + ' ' + hay(co)).indexOf(q) >= 0;
    });
  ...
  function renderFrameworkFacet() {
    ...
    var q = fold(S.qSdg || '');
    var hit = function (txt) { return !q || hay(txt).indexOf(q) >= 0; };
  ...
  function renderProgFacet() {
    ...
    regionOrder.forEach(function (rg) {
      ...
      var progs = (regions[rg] || []).filter(function (p) {
        return !q || hay(p.name).indexOf(q) >= 0 || hay(rg).indexOf(q) >= 0;
      });
      // when searching, drop regions that neither match nor contain a match
      if (q && !progs.length && hay(rg).indexOf(q) < 0 && !S.selRegion.has(rg)) return;
  ...
  function renderKpiFacet() {
    ...
    // The code travels with the name here too. This facet keys on the NAME (that
    // is what the filter selects), so the code is resolved across every copy of
    // that name - see pooledKpiCode.
    var instsOf = {};
    IND.forEach(function (r) { (instsOf[r.name] = instsOf[r.name] || []).push(r); });
    var codeOf = {};
    var kpiFacetLabel = function (n) {
      if (codeOf[n] === undefined) codeOf[n] = pooledKpiCode(instsOf[n] || []);
      return (codeOf[n] ? codeOf[n] + ' · ' : '') + n;
    };
    var q = fold(S.qKpi || '');
    ...
    // search matches the code as readily as the name - they are one label
    var names = Object.keys(pillarOf).filter(function (n) {
      return !q || hay(kpiFacetLabel(n)).indexOf(q) >= 0;
    });
    ...
    shown.forEach(function (n) {
      ...
      host.appendChild(facetRow({
        checkbox: true, checked: S.selKpi.has(n), color: col, name: kpiFacetLabel(n), title: kpiFacetLabel(n),
  ...
  function renderUserFacet() {
    ...
    groups.forEach(function (aff) {
      ...
      var us = DB.tables.user.filter(function (u) {
        ...
        return !q || fold(u.name).indexOf(q) >= 0 || hay(userCountryIsos(u).join(' ')).indexOf(q) >= 0;
      }).sort(function (a, b) { return (rank[b.id] || 0) - (rank[a.id] || 0) || (a.name < b.name ? -1 : 1); });
      if (!us.length) return;
  ...
  function projectSearchHay(p) {
    var co = p.country ? p.country.name : '';
    // Each part is folded with its own translation, so the same search finds a
    // project by its English name, its Arabic name, its donor or its country.
    return [fold(p.code), hay(p.name), hay(p.donor ? p.donor.name : ''),
            hay(p.partner ? p.partner.name : ''), fold(p.partner ? (p.partner.acronym || '') : ''),
            hay(co), fold(p.raw.lead_id != null ? userName(+p.raw.lead_id) : '')].join(' ');
  }
  /** Filtered + searched list of enriched projects for the right pane. */
  function projectsFor() {
    var q = fold(S.qList || ''), progIso = progIsoSet();
  ...
  function renderKpiList(host) {
    ...
    $('#pgInfo').textContent = (S.page + 1) + ' / ' + pages;
    var nk = distinctKpiCount(kpis);
    var rc = $('#reportCount'); if (rc) rc.textContent = fmt(nk) + ' primary KPI' + (nk === 1 ? '' : 's') + ' · ' + fmt(total) + ' in projects';
  }
  ...
  function kpiCard(r, tagOverride) {
    ...
    // a KPI is measured per project, so the card names the project it belongs to
    meta.appendChild(el('span', 'rval', r.project ? (r.project.code || r.project.name) : 'No project'));
    c.appendChild(meta);
  ...
  // A KPI opened via a specific project shows that project's use of it - its own
  // activities against its own (cascaded) baseline and target. Uses are already
  // per project, so this only swaps in the project's use when another was passed.
  function indicatorScopedToProject(r, projectId){
    if (projectId == null || (r.project && r.project.id === projectId)) return r;
    return USEBYPROJKPI[projectId + '|' + r.kpiId] || r;
  }
  ...
  function navBack(){
    ...
    else if (prev.t === 'locationKpi') openLocationKpiActivities(prev.cluster, prev.indId, false);
  ...
  function entityScope(kind, key){
    ...
    if (kind === 'plan'){
      ...
      projsFromInds = true;
      // The code is part of the KPI's name and leads it in every header, even
      // where the entity pools a KPI across its per-project copies.
      var oneCode = pooledKpiCode(inds);
      title = (oneCode ? oneCode + ' · ' : '') + key; sub = 'KPI'; badge = 'kpi';
  ...
  // node identified by a chainKey ('level~|~statement') - the node's one row in
  // the active plan.
      ...
      if (!inds.length) html += '<div class="empty">' + (sc.indsAll.length ? 'No KPIs on this project match the active filters.' : 'No KPIs attached to this project.') + '</div>';
        ...
        inds.slice().sort(function (a, b){ return (b.updated || '') < (a.updated || '') ? -1 : 1; }).forEach(function (r){
          ...
          html += '<tr class="esum-trow" data-ind="' + r.id + '">'
            + '<td class="esum-nm" title="' + esc(kpiLabelText(r)) + '"><span class="esum-dot" style="background:' + col + '"></span>' + kpiLabelHTML(r) + '</td>'
  ...
  function activitiesAtCluster(c, raw){
    ...
    MEAS_NOPROJ.forEach(function (m){          // unattributed activities logged here
      if (!raw && !measPassesFilters(m)) return;
      if (keys[(+m.place_lat).toFixed(3) + ',' + (+m.place_lng).toFixed(3)]) out.push(m);
    });
    return out;
  ...
  function openLocationSummary(c, push){
    ...
    // ---- KPI table + beneficiary heat-map -------------------------------------
    // A location box reports the KPIs measured here, NOT the activity narratives:
    // the narratives are one report each, the KPIs are what the location achieved.
    // Clicking a KPI row opens its own activity log at this location.
    var kpiHead = 'KPIs reported here - ' + fmt(kpiCountOf(acts));
    var block = kpiRollupBlock(acts, {
      head: kpiHead,
      emptyMsg: actsAllN ? 'No KPIs reported at this location match the active filters.'
                         : 'No KPIs reported at this location yet.'
    });
    html += block.html;

    lastResultsExport = { badge: badge, badgeColor: color, title: title, sub: sub,
      summary: kidsTxt, filters: activeFilterSummary(), stats: [], section: kpiHead,
      columns: block.columns, rows: block.rows, note: block.note, table2: block.benTable, grid: true };
    ...
    captureColWidths($('#mBody .esum-static'), lastResultsExport.columns);
    if (lastResultsExport.table2) captureColWidths($('#mBody .ben-heat'), lastResultsExport.table2.columns);
    // KPI row → the activities behind it, at this location. push=true so ‹ Back
    // returns to the location's KPI list rather than dismissing the popup.
    $('#mBody').querySelectorAll('.esum-trow[data-kroll]').forEach(function (row){
      row.onclick = function (){ openLocationKpiActivities(c, +row.dataset.kroll, true); };
    });

    $('#mBody').scrollTop = 0;
    $('#modal').classList.add('on');
  }
  /** How many KPI uses a pool of activities reports against - the rows the
   *  KPI roll-up renders (a KPI counts once per project / free country). */
  function kpiCountOf(acts){
    var seen = {}, n = 0;
    (acts || []).forEach(function (m){
      var u = useOfMeas(m), k = u ? 'u' + u.id : 'k' + m.indicator_id;
      if (!seen[k]){ seen[k] = 1; n++; }
    });
    return n;
  }

  // Drill-down from the location box: the ACTIVITIES logged against ONE KPI at
  // this location. This is the only shape in which activity narratives appear -
  // reached by clicking the KPI they belong to, never as a default listing.
  function openLocationKpiActivities(c, indId, push){
    if (!c || !c.locs || !c.locs.length) return;
    navEnter({ t: 'locationKpi', cluster: c, indId: indId }, push);
    curDetail = null;
    var tabs = $('#mTabs'); if (tabs) tabs.style.display = 'none';
    $('#mProjects').classList.add('hide'); $('#mResults').classList.add('hide');
    $('#mBody').classList.remove('hide');

    var r = INDBYID[indId];   // indId is a KPI USE id (see kpiRollupBlock)
    var mine = function (m){ var u = useOfMeas(m); return !!u && u.id === indId; };
    var acts = activitiesAtCluster(c).filter(mine).sort(function (a, b){ return (a.date < b.date) ? 1 : -1; });
    var actsAllN = activitiesAtCluster(c, true).filter(mine).length;

    var multi = c.locs.length > 1, loc0 = c.locs[0];
    var locName = multi ? (fmt(c.locs.length) + ' activity locations') : ((loc0 && loc0.name) ? loc0.name : 'Location');
    var color = (r && r.sdg) ? (PILLAR_COLORS[r.sdg] || '#7c8aa5') : '#7c8aa5';
    var title = kpiLabelText(r);
    var badgeTxt = r ? (r.secondary ? 'Secondary' : 'Primary') : 'KPI';
    setModalKicker('RESULTS REPORT', 'KPI', color);
    $('#mTitle').textContent = title;
    $('#mSub').textContent = locName;
    $('#mImpact').textContent = '';

    var kidsTxt = ofTotal(acts.length, actsAllN, 'activity', 'activities') + '  ·  ' + locName;
    var html = '<div class="chain"><div class="lnk here">'
      + '<span class="badge" style="background:' + color + '">' + esc(badgeTxt) + '</span>'
      + '<span class="txt">' + esc(title) + '</span></div>'
      + '<div class="kids">↳ ' + esc(kidsTxt) + '</div>'
      + chainExportBtn() + '</div>'
      + filterStripHTML();
    ...
    var block = activitiesResultBlock(acts, {
      head: actHead,
      emptyMsg: 'No activities for this KPI at this location under the current filters.'
    });
    html += block.html;

    lastResultsExport = { badge: badgeTxt, badgeColor: color, title: title, sub: locName,
      summary: kidsTxt, filters: activeFilterSummary(), stats: [], section: actHead,
      columns: block.columns, rows: block.rows, note: block.note, table2: block.benTable, grid: true };
  ...
  function captureColWidths(table, cols){
    ...
    for (var k = 0; k < ths.length && k < cols.length; k++){
      // A column marked noCapture is not the same shape in print as on screen -
      // the activity note is a hover icon here and a text column there - so it
      // keeps its declared width instead of inheriting the icon's few pixels.
      if (cols[k].noCapture) continue;
  ...
  // `opts.groups` collapses the grid to one row per GROUP instead of one per
  // activity - the KPI-first tables pass their KPI groups so the heat-map lines
  // up 1:1 with the table above it, exactly as it does in the activity view.
  function beneficiaryHeatmap(acts, opts){
    if (!acts || !acts.length) return { html: '', table: null };
    opts = opts || {};
    var types = benTypes();
    // One "unit" = one row of the grid: an activity by default, a KPI when the
    // caller groups. Each unit carries its own label + dot so the row reads the
    // same as the row it mirrors in the table above.
    // In a one-KPI box the row is labelled by what was done, exactly as the
    // activities table above it is: naming the same KPI on every line says
    // nothing the header has not already said.
    var singleKpi = opts.singleKpi != null ? opts.singleKpi : (!opts.groups && oneKpiPool(acts));
    var labelHead = opts.groups ? 'KPI' : (singleKpi ? 'Activity' : 'KPI');
    var units = opts.groups || acts.map(function (m){
      var pr = PROJECTSBYID[m.project_id], bInd = useOfMeas(m);
      return { acts: [m],
        labelHTML: singleKpi ? '<span class="act-narr">' + esc(actNarrText(m)) + '</span>'
                             : (kpiLabelHTML(bInd) + actNoteHTML(m.narrative)),
        labelText: singleKpi ? actNarrText(m) : kpiLabelText(bInd),
        dot: pr ? ((pr.donor && pr.donor.color) || countryColor(pr.iso)) : '#94a3b8' };
    });
    // keep only types with at least one non-zero count across these activities
    var totalByType = {}, grid = [], maxCell = 0, grand = 0;
    units.forEach(function (unit){
      var row = { u: unit, vals: {}, total: 0 };
      unit.acts.forEach(function (m){
    ...
    var h = '<div class="msec"><h4>Beneficiaries - ' + fmt(grand) + '</h4>'
      + '<div class="cp-note">Reach by beneficiary type per ' + esc(opts.rowNoun || 'activity') + '. Darker pills = more beneficiaries; totals are hollow.</div>'
      + '<table class="esum-tbl ben-heat loc-tbl" style="--pill-w:' + maxLen + 'ch"><thead><tr><th class="rn-col">#</th><th class="kpi-col">' + esc(labelHead) + '</th>';
    ...
    var eqW = (878 - 42 - 230) / (cols.length + 1);
    // Rows carry whatever labels the table above them carries - the KPI (code +
    // name) when the box spans several, the narrative when it is about one - so
    // the two grids read as the same rows twice, numbered alike.
    var expCols = [{ t: '#', align: 'center', w: 42 }, { t: labelHead, w: 230 }];
    cols.forEach(function (t){ expCols.push({ t: benCode(t.id), align: 'center', w: eqW }); });
    expCols.push({ t: 'Total', align: 'center', w: eqW });
    var expRows = [];

    grid.forEach(function (row, i){
      var unit = row.u, dot = unit.dot || '#94a3b8', bKpi = unit.labelText;
      h += '<tr><td class="rn-col">' + (i + 1) + '</td>'
        + '<td class="kpi-col esum-nm" title="' + esc(bKpi) + '"><span class="esum-dot" style="background:' + dot + '"></span>'
        + unit.labelHTML + '</td>';
      var er = [{ t: String(i + 1) }, { t: bKpi, dot: dot }];
  ...
  /** How an activity row is LABELLED everywhere: its KPI, code and name together.
   *  `kpiLabelText` is the same thing as plain text, for PDF cells and tooltips. */
  function kpiLabelHTML(r){
    if (!r) return '<span class="kpi-code">–</span>';
    var code = (r.raw && r.raw.code) ? r.raw.code : '';
    return (code ? '<span class="kpi-code">' + esc(code) + '</span> ' : '')
         + '<span class="kpi-name">' + esc(r.name) + '</span>';
  }
  function kpiLabelText(r){
    if (!r) return '–';
    var code = (r.raw && r.raw.code) ? r.raw.code : '';
    return (code ? code + ' · ' : '') + r.name;
  }
  /** The code to put in front of a KPI that is POOLED across its per-project
   *  copies (the KPI entity box, the KPI facet). Framework KPIs carry the same
   *  code in every copy, so that code is used. A secondary KPI is project-local
   *  by definition - its code is SEC-<project>.<n>, different in all 43 copies -
   *  so the pooled row is tagged with the family, `SEC`, which is the part of
   *  the code they genuinely share. Inventing one project's code would be a lie. */
  function pooledKpiCode(inds){
    var codes = {}, allSec = true, n = 0;
    (inds || []).forEach(function (r){
      var c = (r.raw && r.raw.code) ? r.raw.code : '';
      if (!codes[c]){ codes[c] = 1; n++; }
      if (!r.secondary) allSec = false;
    });
    var keys = Object.keys(codes);
    if (n === 1) return keys[0] || '';
    return (allSec && n > 1) ? 'SEC' : '';
  }
  /** Is this pool of activities about ONE KPI statement? Same code + name is the
   *  test, not the same id: a KPI is instantiated per project, so "this KPI"
   *  across 29 projects is 29 ids but one statement - and a box about one
   *  statement must not repeat it on every row. */
  function oneKpiPool(acts){
    var label = null;
    for (var i = 0; i < (acts || []).length; i++){
      var l = kpiLabelText(useOfMeas(acts[i]));
      if (label === null) label = l; else if (l !== label) return false;
    }
    return label !== null;
  }
  /** The activity's own free text as plain text - the row's detail once the KPI
   *  has moved up into the header. */
  function actNarrText(m){
    var n = String((m && m.narrative) == null ? '' : m.narrative).trim();
    return n || '–';
  }
  /** The activity's own free text, behind a hover icon rather than in a column:
   *  it describes one report, it does not identify the row. Returns a dash when
   *  the activity carries no note at all. */
  function actNoteHTML(narrative){
    var n = String(narrative == null ? '' : narrative).trim();
    if (!n) return '<span class="act-note empty">–</span>';
    return '<button type="button" class="act-note" title="' + esc(n) + '" aria-label="' + esc('Activity note: ' + n) + '">🗨</button>';
  }

  function activitiesResultBlock(acts, opts){
    opts = opts || {};
    // WHAT NAMES A ROW depends on what the box is about.
    //
    // A box about ONE KPI has already said which KPI in its header, so repeating
    // it down 265 rows names nothing - there, the activity IS the detail, and its
    // narrative takes the first column in full. This is the only shape a results
    // box ever shows narratives in, and it is reached by asking for one KPI.
    //
    // A box spanning SEVERAL KPIs is named by the KPI instead - code AND name,
    // always together - and the narrative steps back behind a hover icon, since
    // there it is the KPI that tells the rows apart.
    var single = opts.singleKpi != null ? opts.singleKpi : oneKpiPool(acts);
    var expCols = single
      ? [{ t: '#', align: 'center', w: 42 }, { t: 'Activity', w: 300 }, { t: 'Location', w: 120 },
         { t: 'Value', align: 'center', w: 100 }, { t: 'Date', align: 'center', w: 105 },
         { t: 'Project', align: 'right', w: 130 }]
      : [{ t: '#', align: 'center', w: 42 }, { t: 'KPI', w: 230 }, { t: 'Location', w: 110 },
         { t: 'Value', align: 'center', w: 100 }, { t: 'Date', align: 'center', w: 105 },
         { t: 'Project', align: 'right', w: 120 }, { t: 'Activity', w: 200, noCapture: true }];
    var expRows = [], expNote = '';
    ...
    if (!acts.length) html += '<div class="empty">' + esc(opts.emptyMsg || 'No activities logged under the current filters.') + '</div>';
      ...
      html += '<table class="esum-tbl esum-static loc-tbl"><thead><tr><th class="rn-col">#</th>'
        + (single
            ? '<th class="kpi-col" title="What was done - one report per row">Activity</th>'
            : '<th class="kpi-col">KPI</th>')
        + '<th class="loc-col">Location</th><th class="col-eq col-c">Value</th><th class="col-eq col-c">Date</th><th class="col-eq col-r">Project</th>'
        + (single ? '' : '<th class="note-col" title="Activity note - hover a row\'s icon to read what was done">Activity</th>')
        + '</tr></thead><tbody>';
      acts.forEach(function (m, i){
        var p = PROJECTSBYID[m.project_id], r = useOfMeas(m);
        var u = (r && r.unit === '%') ? '%' : '';
        var dot = p ? ((p.donor && p.donor.color) || countryColor(p.iso)) : '#94a3b8';
        var projCode = p ? (p.code || p.name) : '–', projFull = p ? ((p.code ? p.code + ' · ' : '') + p.name) : '–';
        var leadTxt = single ? actNarrText(m) : kpiLabelText(r);
        var leadHTML = single ? '<span class="act-narr">' + esc(leadTxt) + '</span>' : kpiLabelHTML(r);
        // Project is plain read-only text (short CODE, full name on hover); the
        // lead column is the narrative (one KPI) or the KPI itself (several).
        html += '<tr>'
          + '<td class="rn-col">' + (i + 1) + '</td>'
          + '<td class="kpi-col esum-nm" title="' + esc(leadTxt) + '"><span class="esum-dot" style="background:' + dot + '"></span>' + leadHTML + '</td>'
          ...
          + (single ? '' : '<td class="note-col">' + actNoteHTML(m.narrative) + '</td>')
          + '</tr>';
        var er = [
          { t: String(i + 1) },
          { t: leadTxt, dot: dot },
        ...
        if (!single) er.push({ t: m.narrative || '–' });
        expRows.push(er);
      });
      html += '</tbody></table>';
    }
    ...
    var benBox = beneficiaryHeatmap(acts, { singleKpi: single });
    html += benBox.html;

    return { html: html, columns: expCols, rows: expRows, note: expNote, benTable: benBox.table };
  }

  // ---- shared KPI-FIRST result block -----------------------------------------
  // THE DEFAULT BODY OF EVERY RESULTS BOX. A results box never opens on a wall of
  // activity narratives: an activity narrative describes one report ("Confirmed
  // during regional review."), a KPI is the thing being measured. So a pool of
  // activities is rolled up to the KPIs it reports against - one row each - and
  // the narratives stay exactly one click away: clicking a row opens that KPI's
  // own activity log, scoped to the same pool. Mirrors activitiesResultBlock's
  // shape (table + beneficiary heat-map + PDF export spec) so the two are
  // interchangeable at every call site.
  function kpiRollupBlock(acts, opts){
    opts = opts || {};
    var byInd = {}, order = [];
    (acts || []).forEach(function (m){
      // one row per KPI USE - a KPI in a project, or free activities in a country
      var use = useOfMeas(m); if (!use) return;
      var g = byInd[use.id];
      if (!g){
        g = byInd[use.id] = { id: use.id, r: use,
          acts: [], sum: 0, latest: null, projIds: {} };
        order.push(g);
      }
      g.acts.push(m);
      g.sum += (+m.value || 0);
      if (!g.latest || String(m.date || '') > String(g.latest.date || '')) g.latest = m;
      if (m.project_id != null) g.projIds[m.project_id] = 1;
    });
    // busiest KPI first - the one this scope has the most to say about
    order.sort(function (a, b){
      return (b.acts.length - a.acts.length)
        || (kpiLabelText(a.r) < kpiLabelText(b.r) ? -1 : 1);
    });

    var expCols = [{ t: '#', align: 'center', w: 42 }, { t: 'KPI', w: 250 },
                   { t: 'Reported', align: 'center', w: 92 }, { t: 'Activities', align: 'center', w: 82 },
                   { t: 'Latest', align: 'center', w: 96 }, { t: 'Project', align: 'right', w: 110 },
                   { t: 'Progress', align: 'right', w: 84 }, { t: 'Performance', align: 'right', w: 96 }];
    var expRows = [];
    var head = opts.head || ('KPIs - ' + fmt(order.length));
    var html = '<div class="msec"><h4>' + esc(head) + '</h4>';
    if (!order.length){
      html += '<div class="empty">' + esc(opts.emptyMsg || 'No KPIs reported under the current filters.') + '</div>';
    } else {
      html += '<div class="cp-note">One row per KPI. <b>Click a KPI</b> to read the activities logged against it.</div>'
        + '<table class="esum-tbl esum-static loc-tbl"><thead><tr><th class="rn-col">#</th><th class="kpi-col">KPI</th>'
        + '<th class="col-eq col-c" title="What this scope reported: a count KPI accumulates, any other unit shows its latest level">Reported</th>'
        + '<th class="col-eq col-c">Activities</th><th class="col-eq col-c">Latest</th>'
        + '<th class="col-eq col-r" title="The project that reported this KPI - a KPI is measured per project, so the same KPI recurs once per project">Project</th>'
        + '<th class="col-eq col-r">Progress</th><th class="col-eq col-r">Performance</th></tr></thead><tbody>';
      order.forEach(function (g, i){
        var r = g.r, u = (r && r.unit === '%') ? '%' : '';
        var dot = r ? (r.sdg ? (PILLAR_COLORS[r.sdg] || '#94a3b8') : (r.secondary ? '#27500a' : '#94a3b8')) : '#94a3b8';
        var kpiFull = kpiLabelText(r);
        // "Reported" is what THIS scope logged: a count KPI accumulates its
        // increments, any other unit reads its most recent report (summing
        // levels or percentages would be meaningless).
        var isCount = !!(r && r.raw) && kpiUnit(r.raw) === 'count';
        var repVal = isCount ? g.sum : (g.latest && g.latest.value != null ? +g.latest.value : null);
        var rep = repVal == null ? '–' : fmtNum(repVal) + u;
        // Each project carries its OWN copy of the results chain, so a KPI code +
        // name repeats once per project inside a multi-project scope. Naming the
        // project is what tells those rows apart - a bare count never could, it is
        // 1 on every row. Short CODE with the full name on hover, exactly as the
        // activities table renders it.
        var gProjs = Object.keys(g.projIds).map(function (id){ return PROJECTSBYID[+id]; }).filter(Boolean);
        var projCode, projFull;
        if (gProjs.length === 1){
          projCode = gProjs[0].code || gProjs[0].name;
          projFull = (gProjs[0].code ? gProjs[0].code + ' · ' : '') + gProjs[0].name;
        } else if (gProjs.length > 1){
          projCode = fmt(gProjs.length) + ' projects';
          projFull = gProjs.map(function (pr){ return (pr.code ? pr.code + ' · ' : '') + pr.name; }).join('\n');
        } else { projCode = '–'; projFull = 'No project'; }
        var late = g.latest && g.latest.date ? shortDate(g.latest.date) : '–';
        var code = (r && r.perfCode) || 'nodata', sc = STATUS[code] || STATUS.nodata;
        var pro = (r && r.progress != null) ? Math.round(r.progress * 100) + '%' : '–';
        var per = (r && r.performance != null) ? Math.round(r.performance * 100) + '%' : '–';
        html += '<tr class="esum-trow" data-kroll="' + g.id + '" title="' + esc('Open the ' + fmt(g.acts.length) + ' activit' + (g.acts.length === 1 ? 'y' : 'ies') + ' logged against this KPI') + '">'
          + '<td class="rn-col">' + (i + 1) + '</td>'
          + '<td class="kpi-col esum-nm" title="' + esc(kpiFull) + '"><span class="esum-dot" style="background:' + dot + '"></span>' + kpiLabelHTML(r) + '</td>'
          + '<td class="col-eq col-c">' + rep + '</td>'
          + '<td class="col-eq col-c">' + fmt(g.acts.length) + '</td>'
          + '<td class="col-eq col-c">' + late + '</td>'
          + '<td class="col-eq col-r" title="' + esc(projFull) + '">' + esc(projCode) + '</td>'
          + '<td class="col-eq col-r"><b style="color:' + sc.c + '">' + pro + '</b></td>'
          + '<td class="col-eq col-r"><b style="color:' + sc.c + '">' + per + '</b></td></tr>';
        expRows.push([
          { t: String(i + 1) },
          { t: kpiFull, dot: dot },
          { t: rep },
    ...
    // the heat-map mirrors the table above it row for row - one row per KPI
    var benBox = beneficiaryHeatmap(acts, {
      rowNoun: 'KPI',
      groups: order.map(function (g){
        return { acts: g.acts, labelHTML: kpiLabelHTML(g.r), labelText: kpiLabelText(g.r),
          dot: g.r ? (g.r.sdg ? (PILLAR_COLORS[g.r.sdg] || '#94a3b8') : (g.r.secondary ? '#27500a' : '#94a3b8')) : '#94a3b8' };
      })
    });
    html += benBox.html;

    return { html: html, columns: expCols, rows: expRows, note: '', benTable: benBox.table, groups: order };
  }
  ...
  function fillReportPreview(hostId, r, valId, exclMeasId){
    ...
    if (ind.target_value == null || ind.baseline_value == null){
      // no project = no target: the activity counts toward the KPI, unmeasured
      host.innerHTML = '<div class="rp-tgt">No target - an activity logged without a project counts toward the KPI in this country, but is not measured against a target. Targets come from a project that adopts the KPI.</div>';
      return;
    }
    var pool = (r.series || []).slice();
  ...
  function openActivityEdit(r, m){
    ...
    var co = DB._idx.countryByIso[m.country_iso3 || r.iso] || null;
    // opened from inside the project it belongs to: the project is fixed here too
    var inProject = $('#projectModal').classList.contains('on') && curProject && m.project_id != null && +m.project_id === curProject.id;
    var details = '<div class="aform">'
      + '<div class="na-ctx"><span>KPI</span><b>' + kpiLabelHTML(r) + '</b>'
      + '<div class="na-ctx-sub">' + esc((co ? co.name : r.iso || '') + ' · ' + pillarLabel(r.sdg)) + '</div></div>'
      + '<div class="afgrid">'
      + '  <label class="af full"><span>Project</span>'
      + (inProject
          ? '<select id="naProject" disabled title="Opened from this project - the project cannot be changed here"><option value="' + curProject.id + '" selected>'
            + esc((curProject.code ? curProject.code + ' · ' : '') + curProject.name) + '</option></select>'
          : '<select id="naProject">' + activityProjectOptionsFor(r, m.project_id != null ? +m.project_id : null) + '</select>')
      + '</label>'
      + activityFieldsHtml({
      ...
      + (inProject
          ...
          : '<div class="cp-note">Editing updates this activity. Clearing <b>Project</b> keeps the activity and its KPI '
            + 'but drops its donor and project lineage. The KPI value, status and the Output → Outcome → Impact roll-up '
            + 're-derive immediately.</div></div>');
  ...
  function submitActivityEdit(r, m){
    if (!ownsRow('measurement', m)) { $('#naMsg').textContent = 'Only the activity’s author can edit it.'; return; }
    ...
    var projSel = $('#naProject');
    if (projSel) m.project_id = projSel.value ? +projSel.value : null;   // attribution is optional
    // an attributed activity happens in its project's country; unattributed, it
    // keeps the country it was logged in
    var mp = m.project_id != null ? DB._idx.projectById[m.project_id] : null;
    if (mp && mp.country_iso3) m.country_iso3 = mp.country_iso3;
    m.date = date;
    if (valRaw !== '' && !isNaN(+valRaw)) m.value = +valRaw;
    m.narrative = (fv($('#naNote')) || '').trim();
    ...
    Promise.resolve(DB.persist('measurement', [m]))
      ...
      .then(function (){
        ...
        else { var nr = useOfMeas(m) || refreshUse(r); if (nr){ openDetail(nr); setModalTab('results'); } else closeDetail(); }
      });
  }
  ...
  function renderResultsEditor(r){
    ...
    var ms = (r.series || []).slice().reverse();   // newest first - this use's own activities

    var sec = el('div', 'msec');
    var head = '<h4>Activity history - ' + fmt(ms.length) + ' activit' + (ms.length === 1 ? 'y' : 'ies') + '</h4>';
    head += '<div class="cp-note">' + (editable
      ? 'Use <b>＋ Log activity</b> to add one against this KPI - naming a project is optional. <b>Edit</b> changes an activity, the bin deletes it. The KPI value, status and the Output → Outcome → Impact roll-up update immediately.'
    ...
    var rows = ms.map(function (m){
      return '<tr data-mid="' + m.id + '">'
        ...
        + '<td class="re-narr"><span class="act-narr">' + esc(actNarrText(m)) + '</span></td>'
        + (editable ? '<td class="re-actcell">' + (ownsRow('measurement', m)
            ? '<button class="re-edit" title="Edit this activity">Edit</button><button class="re-del" title="Delete this activity">🗑</button>'
            : '') + '</td>' : '')
        + '</tr>';
    }).join('');
    if (!ms.length) rows = '<tr><td colspan="' + (editable ? 5 : 4) + '" class="re-empty">No activities yet.</td></tr>';

    sec.innerHTML = head
      + '<div class="retbl-wrap"><table class="retbl retbl-ro"><thead><tr>'
      + '<th>Date</th><th>Value (' + esc(unit) + ')</th><th>Logged by</th><th title="What was done - one report per row">Activity</th>'
      + (editable ? '<th></th>' : '') + '</tr></thead><tbody>' + rows + '</tbody></table></div>';
    host.appendChild(sec);

    // Activities are logged AGAINST A KPI - a project is optional attribution -
    // so the KPI's own history is where one is added from.
    if (editable) {
      var add = el('button', 'hbtn primary', '＋ Log activity');
      // logged from a project's use: that project; from a free use: its country
      add.onclick = function (){ openActivityAdd(r.project || null, r.kpiId, r.project ? null : r.iso); };
      var bar = el('div', 'msec-actbar'); bar.appendChild(add);
      sec.insertBefore(bar, sec.querySelector('.retbl-wrap'));
    }
    ...
    if (editable) {
      host.querySelectorAll('tr[data-mid]').forEach(function (tr){
        ...
        if (del) del.onclick = function (){
          if (!ownsRow('measurement', m)) return;   // only the author (or an Admin) may delete
          if (!confirm('Delete this activity? This cannot be undone.')) return;
          removeActivities([m.id]).then(function (){
            enrich(); renderTicker(); renderAll();
            var nr = refreshUse(r); if (nr){ openDetail(nr); setModalTab('results'); } else closeDetail();
          });
        };
      });
    }
  ...
  // CAPABILITY (role + country scope): who may LOG activities / create on a project.
  // Unchanged by ownership - governs the "＋ Add activity" button and column, and
  // which country a new project may be placed in. (Was canEditThisProject.)
  function canReportProject(p){
    if (!canEditProjects()) return false;
    var lock = projectCountryLock();
    return !lock || !p || p.country_iso3 === lock;
  }
  // OWNERSHIP: who may edit the project's own fields / KPI structure. A brand-new
  // (unsaved) project is editable by anyone with the base capability; an existing
  // one only by its creator (or an Admin).
  function canEditProjectContainer(p){
    if (!CURRENT_USER || curStatus() === 'viewer') return false;
    if (!p || p.id == null) return canEditProjects();   // new project (country lock applied in the form)
    return ownsRow('project', p);
  }

  function openProject(row){
    curProject = row || null;
    _prActKpi = null;                 // every project opens on its KPI list, not a drill-down
  ...
  function setProjectTab(tab){
    ...
    else if (tab === 'evaluation') node = projectEvaluationTab();
  ...
  function configProjectFoot(node){
    ...
    var evp = $('#prEvalPdf'); if (evp) evp.style.display = curProject ? '' : 'none';
  ...
  function projectDetailsForm(){
    var p = curProject, lock = projectCountryLock(), ro = !canEditProjectContainer(p);
    ...
    f.innerHTML =
      ...
      '  <label><span>Delivery</span><select class="pf-impl">' + implOptions(projImpl(p)) + '</select></label>' +
  ...
  /** A record's searchable text: what it says, plus what the reader sees.
   *  `fold` does the same to the query, so the two meet on equal terms. */
  function hay(txt){ return window.I18N && I18N.hay ? I18N.hay(txt) : String(txt == null ? '' : txt).toLowerCase(); }
  function fold(txt){ return window.I18N && I18N.fold ? I18N.fold(txt) : String(txt == null ? '' : txt).toLowerCase(); }

  /** Read one form field.
   *
   *  A field showing a translation must save the wording the record was written
   *  in, not the translation the reader was shown. I18N.fieldValue works that
   *  out; where nothing was translated it is el.value unchanged. */
  function fv(e){ return e ? (window.I18N && I18N.fieldValue ? I18N.fieldValue(e) : e.value) : ''; }

  /** Setting .value fires no mutation, so the observer cannot see a field the
   *  code filled in. Ask for a pass over it directly. */
  function trField(root){ if (window.I18N && I18N.translate) I18N.translate(root); }

  function saveProjectDetails(f){
    var v = function (c){ return fv(f.querySelector(c)); };
    ...
    // OWNERSHIP: a new project needs the base capability; an existing one, its creator (or Admin).
    if (!canEditProjectContainer(curProject)) { if (msg) msg.textContent = 'Only the creator can edit this project.'; return; }
  ...
  /** Why a project's KPI tabs are read-only for this user. Without it the search
   *  box and Add buttons simply vanish, which reads as a broken screen. */
  function projectKpiLockNote(p){
    var msg = curStatus() === 'viewer'
      ? 'Read-only - viewers cannot change a project’s KPIs.'
      : 'Read-only - only the user who created this project (' + userName(p && p.created_by) + ') or an Admin can add or remove its KPIs.'
        + (canReportProject(p) ? ' You can still log activities against the KPIs it already has.' : '');
    return el('div', 'cp-note pr-lock', msg);
  }

  // ---- TAB 2 · primary KPIs (from the inventory) ---------------------------
  /** How many activities this project has logged against a KPI. A linked KPI
   *  carrying activities cannot be detached: the activities belong to the
   *  project THROUGH that KPI, so they have to go first. */
  function projKpiActivityCount(indicatorId, projectId){
    var n = 0;
    (DB._idx.measByIndicator[indicatorId] || []).forEach(function (m){ if (m.project_id === projectId) n++; });
    return n;
  }

  /** Primary KPIs tab: the KPIs this project reports against, one row each, with
   *  ＋ Add primary KPI opening a picker over the whole inventory - the same
   *  shape as the Secondary KPIs tab. Adding and removing save immediately. */
  function projectPrimaryEditor(){
    var p = curProject, ro = !canEditProjectContainer(p);
    var box = el('div', 'prtab');
    box.appendChild(elHTML('div', 'cp-note', 'Primary KPIs are drawn from the <b>KPI inventory</b>. Use <b>＋ Add primary KPI</b> to choose the KPIs this project reports against; their activities aggregate into the project. Secondary (project-local) KPIs are managed on the next tab.'));
    if (ro) box.appendChild(projectKpiLockNote(p));
    else { var add = el('button', 'hbtn primary', '＋ Add primary KPI'); add.type = 'button'; add.onclick = function (){ openPrimaryPicker(); }; box.appendChild(add); }

    var links = (DB._idx.projectKpiByProject[p.id] || []).map(function (pk){ return { pk: pk, r: USEBYPROJKPI[p.id + '|' + pk.indicator_id] }; })
      .filter(function (x){ return x.r; })
      .sort(function (a, b){
        return (a.r.sdg || 99) - (b.r.sdg || 99)
          || String(a.r.raw.code || '').localeCompare(String(b.r.raw.code || ''), 'en', { numeric: true });
      });
    var tbl = el('table', 'utbl');
    // Code and name are one label, so they share one column (as on the Secondary tab).
    tbl.innerHTML = '<thead><tr><th>KPI</th><th>Impact</th><th title="Inherited from the results framework unless this project set its own">Baseline</th><th title="Inherited from the results framework unless this project set its own">Target</th><th>Progress</th><th></th></tr></thead>';
    var tb = el('tbody');
    if (!links.length) tb.innerHTML = '<tr><td colspan="6" class="re-empty">' + (ro ? 'This project reports against no primary KPIs.' : 'No primary KPIs yet - use ＋ Add primary KPI to choose the first one.') + '</td></tr>';
    links.forEach(function (x){
      var r = x.r, ind = r.raw, u = kpiUnit(ind) === '%' ? '%' : '';
      var tr = el('tr');
      tr.innerHTML = '<td><span class="udot" style="background:' + (r.sdg ? PILLAR_COLORS[r.sdg] : '#94a3b8') + '"></span>'
          + kpiLabelHTML(r) + '</td>'
        + '<td>' + esc(r.sdg ? pillarLabel(r.sdg) : 'Unaligned') + '</td>'
        + '<td class="umono">' + fmtNum(ind.baseline_value) + u + '</td>'
        + '<td class="umono">' + fmtNum(ind.target_value) + u + '</td>'
        + '<td class="umono" style="color:' + STATUS[r.status || 'nodata'].c + '">' + (r.frac != null ? Math.round(r.frac * 100) + '%' : '–') + '</td>';
      var act = el('td', 'uact');
      if (!ro){
        var tg = el('button', 'cp-mini', 'Targets'); tg.type = 'button';
        tg.title = 'Baseline and target for this project - inherited from the results framework unless set here';
        tg.onclick = function (){ openProjectTargets(x.pk, r); };
        act.appendChild(tg);
        // A KPI carrying this project's activities or evidence cannot be detached:
        // those records belong to the project THROUGH the KPI.
        var acts = projKpiActivityCount(r.kpiId, p.id), evN = evRows(p.id, r.kpiId).length;
        var del = el('button', 'cp-del', '🗑'); del.type = 'button';
        if (acts || evN){
          del.disabled = true;
          del.title = (acts ? acts + ' activit' + (acts === 1 ? 'y is' : 'ies are') + ' logged' : '')
            + (acts && evN ? ' and ' : '')
            + (evN ? evN + ' evidence item' + (evN === 1 ? ' is' : 's are') + ' uploaded' : '')
            + ' against this KPI in this project. Remove '
            + (acts && evN ? 'them' : (acts ? 'those activities' : 'that evidence'))
            + ' first (' + (acts ? 'Activities' : '') + (acts && evN ? ' / ' : '') + (evN ? 'Evaluation' : '') + ' tab), then this KPI can be removed.';
        } else {
          del.title = 'Remove this KPI from the project';
          del.onclick = function (){
            if (!confirm('Remove ' + kpiLabelText(r) + ' from this project?')) return;
            removePrimaryKpi(x.pk);
          };
        }
        act.appendChild(del);
      }
  ...
  /** Adjust a project's baseline/target for one adopted KPI. Empty fields inherit
   *  the plan-level values from the results framework (and follow them when the
   *  framework changes); a filled field is this project's own value. */
  function openProjectTargets(pk, r){
    if (!canEditProjectContainer(curProject)) return;
    var def = DB._idx.indicatorById[pk.indicator_id]; if (!def) return;
    var u = kpiUnit(def) === '%' ? ' %' : '';
    $('#secEditTitle').textContent = 'Targets · ' + kpiDefLabel(def);
    var body = $('#secEditBody'); body.innerHTML = '';
    var f = el('div', 'uform');
    function fld(cls, label, col, type){
      var own = pk[col], plan = def[col];
      return '  <label><span>' + label + '</span><input class="' + cls + '" type="' + type + '"' + (type === 'number' ? ' step="any"' : '')
        + ' value="' + esc(own == null ? '' : own) + '" placeholder="' + esc(plan == null ? '' : (type === 'number' ? fmtNum(plan) + u : plan)) + '">'
        + '<small class="ev-hint">Plan value: <b data-notr>' + esc(plan == null ? '–' : (type === 'number' ? fmtNum(plan) + u : shortDate(plan))) + '</b></small></label>';
    }
    f.innerHTML =
      '<div class="cp-note">Baseline and target are set once for this KPI in the <b>results framework</b> and cascade to every project that adopts it. Leave a field empty to keep inheriting the plan value; fill it to give this project its own.</div>' +
      '<div class="ufgrid kpigrid">' +
      fld('pt-base', 'Baseline value', 'baseline_value', 'number') +
      fld('pt-basedate', 'Baseline date', 'baseline_date', 'date') +
      fld('pt-tgt', 'Target value', 'target_value', 'number') +
      fld('pt-tgtdate', 'Target date', 'target_date', 'date') +
      '</div>' +
      '<div class="ufbtns"><span class="ufmsg"></span>' +
        '<button class="hbtn pt-reset" type="button">Use plan values</button>' +
        '<button class="hbtn pt-cancel" type="button">Cancel</button>' +
        '<button class="hbtn primary pt-save" type="button">Save targets</button></div>';
    body.appendChild(f);
    function save(values){
      var msg = f.querySelector('.ufmsg');
      if (!canEditProjectContainer(curProject)){ msg.textContent = 'Only the creator can change this project’s KPIs.'; return; }
      Object.keys(values).forEach(function (k){ pk[k] = values[k]; });
      msg.textContent = 'Saving…';
      Promise.resolve(DB.persist('project_kpi', [pk])).then(function (){
        closeSecEdit(); enrich(); renderTicker(); renderAll(); setProjectTab('primary');
        var m2 = $('#prMsg'); if (m2){ m2.textContent = 'Targets saved.'; m2.classList.add('ok'); }
      }, function (err){ msg.textContent = 'Could not save: ' + (err && err.message || err); });
    }
    var num = function (c){ var v = f.querySelector(c).value; return v === '' || isNaN(+v) ? null : +v; };
    var dt = function (c){ return f.querySelector(c).value || null; };
    f.querySelector('.pt-cancel').onclick = closeSecEdit;
    f.querySelector('.pt-reset').onclick = function (){ save({ baseline_value: null, baseline_date: null, target_value: null, target_date: null }); };
    f.querySelector('.pt-save').onclick = function (){
      save({ baseline_value: num('.pt-base'), baseline_date: dt('.pt-basedate'), target_value: num('.pt-tgt'), target_date: dt('.pt-tgtdate') });
    };
    $('#secEditOverlay').classList.add('on');
    trField($('#secEditOverlay'));   // the title was set before the popup showed
  }
  function removePrimaryKpi(pk){
    if (!canEditProjectContainer(curProject)) return;   // creator (or Admin) only
    var msg = $('#prMsg'); if (msg){ msg.classList.remove('ok'); msg.textContent = 'Saving…'; }
    Promise.resolve(DB.remove('project_kpi', [pk.id])).then(function (){
      enrich(); renderTicker(); renderAll(); setProjectTab('primary');
      var m2 = $('#prMsg'); if (m2){ m2.textContent = 'Removed.'; m2.classList.add('ok'); }
    });
  }

  // ---- primary KPI picker (popup over the project modal) -------------------
  /** Choose framework KPIs for the open project: search by code, name or impact
   *  (in English or the reader's language), narrow by impact, tick any number,
   *  add them in one go. Each KPI appears once - a KPI belongs to no country -
   *  and the project inherits its plan-level baseline and target on adoption. */
  function openPrimaryPicker(){
    var p = curProject;
    if (!p || !canEditProjectContainer(p)) return;
    var linked = {}; (DB._idx.projectKpiByProject[p.id] || []).forEach(function (pk){ linked[pk.indicator_id] = 1; });
    var pool = planKpis(S.plan).filter(function (def){ return !linked[def.id]; }).map(function (def){
      var res = DB._idx.resultById[def.result_id];
      return { id: def.id, raw: def, name: def.name, sdg: res ? res.sdg : null, output: res };
    });
    var chosen = {};
    var hayCache = {};
    function kpiHay(r){
      if (hayCache[r.id] == null) hayCache[r.id] = [r.raw.code || '', r.name, pillarLabel(r.sdg || 0), PILLAR_NAMES[r.sdg] || '', r.output ? r.output.statement : ''].map(hay).join(' ');
      return hayCache[r.id];
    }
    var sdgs = {};
    pool.forEach(function (r){ if (r.sdg) sdgs[r.sdg] = 1; });

    $('#pkPickTitle').textContent = '＋ Add primary KPIs · ' + (p.code || p.name);
    var body = $('#pkPickBody'); body.innerHTML = '';
    var f = el('div', 'pkp');
    f.innerHTML =
      '<div class="pkp-filters">' +
      '  <input class="pkp-q" type="search" placeholder="Search by code, name, output or impact…">' +
      '  <select class="pkp-imp"><option value="">All impacts</option>' + Object.keys(sdgs).sort(function (a, b){ return a - b; })
            .map(function (s){ return '<option value="' + s + '">' + esc(pillarLabel(+s) + ' · ' + (PILLAR_NAMES[+s] || '')) + '</option>'; }).join('') + '</select>' +
      '</div>' +
      '<div class="pkp-count"></div>' +
      '<div class="pkp-listwrap"><table class="utbl pkp-list"><thead><tr><th class="pkp-ck"><input type="checkbox" class="pkp-all" title="Select all shown"></th><th>KPI</th><th>Impact</th><th title="Plan-level values - the project inherits them">Baseline</th><th title="Plan-level values - the project inherits them">Target</th></tr></thead><tbody></tbody></table></div>' +
      '<div class="ufbtns"><span class="ufmsg"></span>' +
      '  <button class="hbtn pkp-cancel" type="button">Cancel</button>' +
      '  <button class="hbtn primary pkp-add" type="button" disabled>Add selected</button></div>';
    body.appendChild(f);

    var q = f.querySelector('.pkp-q'), impSel = f.querySelector('.pkp-imp');
    var tb = f.querySelector('tbody'), count = f.querySelector('.pkp-count'), addBtn = f.querySelector('.pkp-add'), all = f.querySelector('.pkp-all');
    var LIMIT = 200, shown = [];

    function nChosen(){ return Object.keys(chosen).length; }
    function syncButton(){
      var n = nChosen();
      addBtn.disabled = !n;
      addBtn.textContent = n ? ('Add ' + n + ' KPI' + (n === 1 ? '' : 's')) : 'Add selected';
    }
    function paint(){
      var terms = fold((q.value || '').trim()).split(/\s+/).filter(Boolean);
      var sdg = impSel.value ? +impSel.value : null;
      var hits = pool.filter(function (r){
        if (sdg != null && r.sdg !== sdg) return false;
        if (!terms.length) return true;
        var h = kpiHay(r);
        return terms.every(function (t){ return h.indexOf(t) >= 0; });
      }).sort(function (a, b){
        return String(a.raw.code || '').localeCompare(String(b.raw.code || ''), 'en', { numeric: true });
      });
      shown = hits.slice(0, LIMIT);
      count.textContent = hits.length > LIMIT
        ? ('Showing ' + fmt(LIMIT) + ' of ' + fmt(hits.length) + ' matching KPIs - search or filter to narrow the list.')
        : (fmt(hits.length) + ' matching KPI' + (hits.length === 1 ? '' : 's') + (nChosen() ? ' · ' + fmt(nChosen()) + ' selected' : ''));
      tb.innerHTML = shown.length ? '' : '<tr><td colspan="5" class="re-empty">No KPIs match - try another search or impact.</td></tr>';
      shown.forEach(function (r){
        var tr = el('tr', 'pkp-row' + (chosen[r.id] ? ' on' : ''));
        var u = kpiUnit(r.raw) === '%' ? '%' : '';
        tr.innerHTML = '<td class="pkp-ck"><input type="checkbox"' + (chosen[r.id] ? ' checked' : '') + '></td>'
          + '<td>' + kpiLabelHTML(r) + '</td>'
          + '<td><span class="udot" style="background:' + (r.sdg ? PILLAR_COLORS[r.sdg] : '#94a3b8') + '"></span>' + esc(r.sdg ? pillarLabel(r.sdg) : 'Unaligned') + '</td>'
          + '<td class="umono">' + fmtNum(r.raw.baseline_value) + u + '</td>'
          + '<td class="umono">' + fmtNum(r.raw.target_value) + u + '</td>';
        var cb = tr.querySelector('input');
        function toggle(on){ if (on) chosen[r.id] = 1; else delete chosen[r.id]; cb.checked = on; tr.classList.toggle('on', on); syncButton(); syncAll(); }
        cb.onchange = function (){ toggle(cb.checked); };
        tr.onclick = function (e){ if (e.target !== cb) toggle(!chosen[r.id]); };
        tb.appendChild(tr);
      });
      syncAll(); syncButton();
    }
    function syncAll(){
      var on = shown.filter(function (r){ return chosen[r.id]; }).length;
      all.checked = !!shown.length && on === shown.length;
      all.indeterminate = on > 0 && on < shown.length;
    }
    all.onchange = function (){
      shown.forEach(function (r){ if (all.checked) chosen[r.id] = 1; else delete chosen[r.id]; });
      paint();
    };
    q.oninput = debounce(paint, 120);
    q.onkeydown = function (e){ if (e.key === 'Escape' && q.value){ e.stopPropagation(); q.value = ''; paint(); } };
    impSel.onchange = paint;
    f.querySelector('.pkp-cancel').onclick = closePrimaryPicker;
    addBtn.onclick = function (){ addPrimaryKpis(Object.keys(chosen).map(Number), f.querySelector('.ufmsg')); };

    paint();
    $('#pkPickOverlay').classList.add('on');
    trField($('#pkPickOverlay'));
    setTimeout(function (){ q.focus(); }, 0);
  }
  function closePrimaryPicker(){ var o = $('#pkPickOverlay'); if (o) o.classList.remove('on'); }
  function addPrimaryKpis(ids, msg){
    var p = curProject;
    if (!p || !canEditProjectContainer(p)) { if (msg) msg.textContent = 'Only the creator can change this project’s KPIs.'; return; }
    var linked = {}; (DB._idx.projectKpiByProject[p.id] || []).forEach(function (pk){ linked[pk.indicator_id] = 1; });
    var rows = ids.filter(function (iid){ return !linked[iid] && DB._idx.indicatorById[iid]; })
      .map(function (iid){ return { project_id: p.id, indicator_id: iid }; });
    if (!rows.length){ closePrimaryPicker(); return; }
    if (msg) msg.textContent = 'Saving…';
    // Only the link is written. Activities logged against these KPIs without a
    // project stay unattributed - naming a project on an activity is its
    // author's choice, never a side effect of someone else's KPI list.
    Promise.resolve(DB.insert('project_kpi', rows)).then(function (){
      closePrimaryPicker();
      enrich(); renderTicker(); renderAll(); setProjectTab('primary');
      var m2 = $('#prMsg'); if (m2){ m2.textContent = rows.length + ' primary KPI' + (rows.length === 1 ? '' : 's') + ' added.'; m2.classList.add('ok'); }
    }, function (err){ if (msg) msg.textContent = 'Could not save: ' + (err && err.message || err); });
  }
  ...
  function projectSecondaryEditor(){
    ...
    if (ro) box.appendChild(projectKpiLockNote(p));
    else { var add = el('button', 'hbtn primary', '＋ Add secondary KPI'); add.onclick = function (){ openSecEdit(null); }; box.appendChild(add); }
    var tbl = el('table', 'utbl');
    // Code and name are one label, so they share one column here too rather than
    // sitting in a Code column of their own.
    tbl.innerHTML = '<thead><tr><th>KPI</th><th>Unit</th><th>Baseline</th><th>Target</th><th>Progress</th><th></th></tr></thead>';
    var tb = el('tbody');
    if (!secs.length) tb.innerHTML = '<tr><td colspan="6" class="re-empty">No secondary KPIs yet.</td></tr>';
    secs.forEach(function (ind){
      var r = USEBYPROJKPI[p.id + '|' + ind.id]; var u = kpiUnit(ind) === '%' ? '%' : '';
      var tr = el('tr');
      tr.innerHTML = '<td><span class="udot" style="background:#33C2B4"></span>'
          + (ind.code ? '<span class="kpi-code">' + esc(ind.code) + '</span> ' : '')
          + '<span class="kpi-name">' + esc(ind.name) + '</span></td>'
      ...
      var act = el('td', 'uact');
      if (!ro && ownsRow('indicator', ind)) {   // creator (or Admin) only
  ...
  function saveSecondary(ind, f){
    ...
    if (ind && !ownsRow('indicator', ind)) { msg.textContent = 'Only the creator can edit this KPI.'; return; }
    var num = function (x){ return x === '' || isNaN(+x) ? null : +x; };
  ...
  /** Delete activities together with their beneficiary rows. Only SQLite
   *  cascades; in the browser store a left-behind beneficiary row would attach
   *  itself to the next activity that reuses the freed id. */
  function removeActivities(measIds){
    var set = {}; (measIds || []).forEach(function (id){ set[id] = 1; });
    var benIds = DB.tables.beneficiary.filter(function (b){ return set[b.measurement_id]; }).map(function (b){ return b.id; });
    return Promise.all([ DB.remove('beneficiary', benIds), DB.remove('measurement', measIds || []) ]);
  }
  function deleteSecondary(ind){
    if (!ownsRow('indicator', ind)) return;   // only the creator (or an Admin) may delete
    var measIds = (DB._idx.measByIndicator[ind.id] || []).map(function (m){ return m.id; });
    Promise.all([ evRemoveWhere(function (e){ return e.indicator_id === ind.id; }),
                  removeActivities(measIds), DB.remove('indicator', [ind.id]) ]).then(function (){
      enrich(); renderTicker(); renderAll(); setProjectTab('secondary');
    });
  }

  // ---- TAB 4 · activities (moved from the main screen) ---------------------
  function projectKpiOptions(p, selId){
    // option value = the KPI's database id; the project's use of it is implied
    var prim = (DB._idx.projectKpiByProject[p.id] || []).map(function (pk){ return DB._idx.indicatorById[pk.indicator_id]; }).filter(Boolean);
    var secs = (DB._idx.secondaryByProject[p.id] || []).slice();
    function opt(def, tag){
      return '<option value="' + def.id + '"' + (selId != null && def.id === selId ? ' selected' : '') + '>'
        + esc(kpiDefLabel(def)) + ' [' + tag + ']</option>';
    }
    var html = '';
  ...
  /** `iso3` may be a country code or a function returning one, because the
   *  activity popup can change country while it is open (the project or the KPI
   *  changed) and re-mounting would stack a second set of listeners. */
  function mountPlaceSearch(input, results, chosen, iso3){
    var isoOf = (typeof iso3 === 'function') ? iso3 : function (){ return iso3; };
    var timer = null;
    ...
    input.addEventListener('input', function (){
      ...
      timer = setTimeout(function (){
        searchPlaces(q, isoOf(), function (err, list){
  ...
  // Activities tab = this project's reporting, KPI FIRST. The tab opens on the
  // KPIs the project has reported against (one row each, never a flat wall of
  // narratives); clicking a KPI drills into the activities logged against THAT
  // KPI, where the narratives, Edit and Delete live. New activities are added
  // through the activity popup (＋ Add activity).
  var _prActKpi = null;   // indicator id being drilled into on the Activities tab
  function projectActivitiesEditor(){
    // editable = may LOG activities here (role + country scope). Logging stays open
    // to in-scope users regardless of who owns the project; per-row Edit/Delete is
    // then further restricted to the activity's own author (ownsRow).
    var p = curProject, editable = canReportProject(p);
    var kpiOpts = projectKpiOptions(p);
    var all = (DB._idx.measByProject[p.id] || []).slice().sort(function (a, b){ return (a.date < b.date) ? 1 : -1; });
    // a KPI removed from the project (or with its last activity deleted) must not
    // leave the tab stuck on an empty drill-down
    if (_prActKpi != null && !all.some(function (m){ return m.indicator_id === _prActKpi; })) _prActKpi = null;
    var box = el('div', 'prtab');

    var head = el('div', 'pr-acthead');
    var drillR = _prActKpi != null ? USEBYPROJKPI[p.id + '|' + _prActKpi] : null;   // _prActKpi = KPI id
    var ms = drillR ? all.filter(function (m){ return m.indicator_id === _prActKpi; }) : all;
    head.appendChild(el('div', 'sec-h2', drillR
      ? (fmt(ms.length) + ' activit' + (ms.length === 1 ? 'y' : 'ies') + ' · ' + kpiLabelText(drillR))
      : (fmt(kpiCountOf(all)) + ' KPI' + (kpiCountOf(all) === 1 ? '' : 's') + ' reported · ' + fmt(all.length) + ' activit' + (all.length === 1 ? 'y' : 'ies'))));
    if (drillR){
      var back = el('button', 'hbtn', '‹ All KPIs');
      back.onclick = function (){ _prActKpi = null; setProjectTab('activities'); };
    ...
    if (!drillR) box.appendChild(projActKpiTable(p, all, editable));
    else box.appendChild(projActRowTable(ms, editable));
    return box;
  }
  /** Default view of the Activities tab: one row per KPI this project reported
   *  against, click-through to that KPI's own activities. */
  function projActKpiTable(p, all, editable){
    var byInd = {}, order = [];
    all.forEach(function (m){
      var g = byInd[m.indicator_id];
      if (!g){ g = byInd[m.indicator_id] = { id: m.indicator_id, r: useOfMeas(m), acts: [], sum: 0, latest: null, ben: 0 }; order.push(g); }
      g.acts.push(m); g.sum += (+m.value || 0); g.ben += (benTotalFor(m.id) || 0);
      if (!g.latest || String(m.date || '') > String(g.latest.date || '')) g.latest = m;
    });
    order.sort(function (a, b){ return b.acts.length - a.acts.length; });

    var wrap = el('div');
    var note = el('div', 'cp-note', 'One row per KPI. Click a KPI to open the activities logged against it' + (editable ? ' - and to edit or delete them.' : '.'));
    wrap.appendChild(note);
    var tbl = el('table', 'utbl retbl');
    tbl.innerHTML = '<thead><tr><th>KPI</th><th>Activities</th><th>Reported</th><th>Latest</th><th>Beneficiaries</th><th></th></tr></thead>';
    var tb = el('tbody');
    if (!order.length) tb.innerHTML = '<tr><td colspan="6" class="re-empty">No activities logged for this project yet.</td></tr>';
    order.forEach(function (g){
      var r = g.r;
      var isCount = !!(r && r.raw) && kpiUnit(r.raw) === 'count';
      var repVal = isCount ? g.sum : (g.latest && g.latest.value != null ? +g.latest.value : null);
      var tr = el('tr', 'prk-row'); tr.dataset.kind = g.id;
      tr.innerHTML = '<td>' + esc(r ? ((r.raw && r.raw.code ? r.raw.code + ' · ' : '') + r.name) : ('#' + g.id))
          + (r && r.secondary ? ' <span class="sec-badge">sec</span>' : '') + '</td>'
        + '<td class="umono">' + fmt(g.acts.length) + '</td>'
        + '<td class="umono">' + (repVal == null ? '–' : fmtNum(repVal) + ((r && r.unit === '%') ? '%' : '')) + '</td>'
        + '<td class="re-datecell">' + (g.latest && g.latest.date ? shortDate(g.latest.date) : '–') + '</td>'
        + '<td class="umono">' + (g.ben ? fmt(g.ben) : '–') + '</td>'
        + '<td class="re-actcell"><span class="prk-chev">›</span></td>';
      tb.appendChild(tr);
    });
    tbl.appendChild(tb); wrap.appendChild(tbl);
    wrap.querySelectorAll('tr[data-kind]').forEach(function (tr){
      tr.onclick = function (){ _prActKpi = +tr.dataset.kind; setProjectTab('activities'); };
    });
    return wrap;
  }
  /** Drill-down of the Activities tab: the activities of ONE KPI on this project,
   *  with the narrative, Edit and Delete. */
  function projActRowTable(ms, editable){
    var wrap = el('div');
    var tbl = el('table', 'utbl retbl');
    // One KPI is in question here, and the header names it - so the rows carry
    // what was done, in full, rather than repeating that KPI down the table.
    tbl.innerHTML = '<thead><tr><th>Date</th><th>Value</th><th>Location</th><th>Beneficiaries</th><th>Logged by</th>'
      + '<th title="What was done - one report per row">Activity</th>' + (editable ? '<th></th>' : '') + '</tr></thead>';
    var tb = el('tbody');
    if (!ms.length) tb.innerHTML = '<tr><td colspan="' + (editable ? 7 : 6) + '" class="re-empty">No activities logged against this KPI yet.</td></tr>';
    ms.forEach(function (m){
      ...
      tr.innerHTML = '<td class="re-datecell">' + (m.date ? shortDate(m.date) : '–') + '</td>'
        ...
        + '<td class="re-narr"><span class="act-narr">' + esc(actNarrText(m)) + '</span></td>'
        + (editable ? '<td class="re-actcell">' + (ownsRow('measurement', m) ? '<button class="re-edit">Edit</button><button class="re-del">🗑</button>' : '') + '</td>' : '');
      tb.appendChild(tr);
    });
    tbl.appendChild(tb); wrap.appendChild(tbl);
    if (editable) {
      wrap.querySelectorAll('tr[data-mid]').forEach(function (tr){
        ...
        var del = tr.querySelector('.re-del'); if (del) del.onclick = function (){
          ...
          removeActivities([m.id]).then(function (){ enrich(); renderTicker(); renderAll(); setProjectTab('activities'); });
        };
      });
    }
    return wrap;
  ...
  // ---- Log an activity -----------------------------------------------------
  //  PROJECT ATTRIBUTION IS OPTIONAL. An activity is one logged report of
  //  progress against a KPI; naming a project is what puts it on the delivery
  //  chain (Donor -> Project -> Activity), and plenty of field work belongs to
  //  no project at all. So the picker opens on whatever project the popup was
  //  launched from and can always be set back to none - the activity is still
  //  logged against its KPI, still counted, still plotted, still rolled up.
  //
  //  The country scopes the settlement search, so a location can never
  //  contradict the record it hangs off. Logged from inside a project it is the
  //  project's country; logged standalone the user picks it. A KPI belongs to no
  //  country - the country is a fact about the ACTIVITY, stored on it.

  /** Projects an activity may be attributed to, with an explicit empty option.
   *  `iso` narrows the list to one country when the user has picked it. */
  function activityProjectOptions(selId, iso){
    var list = (DB.tables.project || []).filter(function (p){
      return p.plan_id === S.plan && canReportProject(p) && (!iso || p.country_iso3 === iso);
    }).sort(function (a, b){
      return String(a.code || a.name).localeCompare(String(b.code || b.name));
    });
    return '<option value="">– No project –</option>' + list.map(function (p){
      return '<option value="' + p.id + '"' + (selId != null && p.id === selId ? ' selected' : '') + '>'
        + esc((p.code ? p.code + ' · ' : '') + p.name) + '</option>';
    }).join('');
  }
  function kpiDefLabel(def){ return (def.code ? def.code + ' · ' : '') + def.name; }
  /** With no project in the picture, the KPIs to report against are the plan's
   *  framework KPIs - each exactly once, grouped by the Impact they serve. */
  function planKpiOptions(selId){
    var byImpact = {};
    planKpis(S.plan).forEach(function (def){
      var res = DB._idx.resultById[def.result_id], sdg = res ? (res.sdg || 0) : 0;
      (byImpact[sdg] = byImpact[sdg] || []).push(def);
    });
    // nothing is pre-selected: the user chooses what the activity reports against
    return '<option value="">– Select KPI –</option>' + Object.keys(byImpact).sort(function (a, b){ return (+a || 99) - (+b || 99); }).map(function (sdg){
      var opts = byImpact[sdg].sort(function (a, b){ return String(a.code || '').localeCompare(String(b.code || ''), 'en', { numeric: true }); })
        .map(function (def){
          return '<option value="' + def.id + '"' + (selId != null && def.id === selId ? ' selected' : '') + '>' + esc(kpiDefLabel(def)) + '</option>';
        }).join('');
      return '<optgroup label="' + esc((+sdg) ? (pillarLabel(+sdg) + ' · ' + (PILLAR_NAMES[+sdg] || '')) : 'Unaligned') + '">' + opts + '</optgroup>';
    }).join('');
  }
  function activityKpiOptions(p, selId){ return p ? projectKpiOptions(p, selId) : planKpiOptions(selId); }
  /** Countries an activity can be logged in: any country - a KPI has none of its
   *  own - except for a Countries-affiliated user, who reports in the countries
   *  they Lead. */
  function activityCountryOptions(selIso){
    var mine = (CURRENT_USER && curSection() === 'co' && !isAdmin()) ? userCountryIsos(CURRENT_USER) : null;
    var list = DB.tables.country.filter(function (c){ return !mine || mine.indexOf(c.iso3) >= 0; })
      .sort(function (a, b){ return String(a.name).localeCompare(String(b.name)); });
    return '<option value="">– Select country –</option>' + list
      .map(function (c){ return '<option value="' + c.iso3 + '"' + (c.iso3 === selIso ? ' selected' : '') + '>' + esc(c.name) + '</option>'; })
      .join('');
  }
  /** Projects an EXISTING activity may be moved to: none, or a project that
   *  actually carries the KPI it reports (plus wherever it sits today, so an
   *  activity is never silently detached by opening its form). */
  function activityProjectOptionsFor(r, selId){
    var ids = {};
    (DB._idx.projectKpiByIndicator[r.kpiId] || []).forEach(function (pk){ ids[pk.project_id] = 1; });
    if (r.secondary && r.def && r.def.project_id != null) ids[r.def.project_id] = 1;
    if (selId != null) ids[selId] = 1;
    var list = Object.keys(ids).map(function (id){ return DB._idx.projectById[+id]; })
      .filter(function (p){ return p && (p.id === selId || canReportProject(p)); })
      .sort(function (a, b){ return String(a.code || a.name).localeCompare(String(b.code || b.name)); });
    return '<option value="">– No project –</option>' + list.map(function (p){
      return '<option value="' + p.id + '"' + (p.id === selId ? ' selected' : '') + '>'
        + esc((p.code ? p.code + ' · ' : '') + p.name) + '</option>';
    }).join('');
  }
  function countryLabel(iso){
    var c = iso ? DB._idx.countryByIso[iso] : null;
    return c ? c.name : (iso || '–');
  }
  /** The KPI use a new activity would join: the project's use of the KPI, or the
   *  free use in that country - or, when neither exists yet, a stand-in carrying
   *  the definition (and no target when there is no project), for the preview. */
  function prospectiveUse(kpiId, project, iso){
    var def = kpiId != null ? DB._idx.indicatorById[kpiId] : null; if (!def) return null;
    var u = project ? USEBYPROJKPI[project.id + '|' + kpiId] : USEFREE[kpiId + '|' + (iso || '')];
    if (u) return u;
    return { id: null, kpiId: kpiId, def: def, name: def.name, raw: kpiRawFor(def, null, !project), series: [], project: project || null, iso: iso };
  }

  // Add-activity popup - over the project modal, standalone (side pane in
  // Activities mode), or from a KPI use's Activities tab. Reuses the New
  // Activity overlay. `presetIso` pre-selects the country when standalone.
  function openProjectActivityAdd(p){ openActivityAdd(p, null); }
  function openActivityAdd(p, presetKpiId, presetIso){
    var host = $('#naBody'); host.innerHTML = '';
    $('#naHead').textContent = p ? ('Log activity - ' + (p.code || p.name)) : 'Log a new activity';
    _actBens = [];
    var today = new Date(TODAY).toISOString().slice(0, 10);
    // Logged from INSIDE a project, the activity belongs to that project: the
    // field shows it but cannot be changed. From anywhere else the project is
    // an optional choice.
    var projectField = p
      ? '<select id="paProject" disabled title="Logged from this project - the project cannot be changed here">'
        + '<option value="' + p.id + '" selected>' + esc((p.code ? p.code + ' · ' : '') + p.name) + '</option></select>'
      : '<select id="paProject">' + activityProjectOptions(null, presetIso || null) + '</select>';
    // Standalone, the user picks the COUNTRY the activity happened in: it narrows
    // the projects on offer and scopes the settlement search. Inside a project
    // the country is the project's and is shown, not chosen.
    var startIso = p ? p.country_iso3 : (presetIso || projectCountryLock());
    var countryField = p
      ? '<input class="af-ro" id="paCountry" type="text" readonly value="" title="The project’s country">'
      : '<select id="paCountry">' + activityCountryOptions(startIso) + '</select>';
    var details = '<div class="aform"><div class="afgrid">' +
      (p ? '' : '  <label class="af"><span>Country <em>*</em></span>' + countryField + '</label>') +
      '  <label class="af"><span>Project</span>' + projectField + '</label>' +
      (p ? '  <label class="af"><span>Country</span>' + countryField + '</label>' : '') +
      '  <label class="af full"><span>KPI <em>*</em></span><select id="paKpi">' + activityKpiOptions(p, presetKpiId) + '</select></label>' +
      ...
      '  <label class="af full"><span>Location - search a real city / village in <b id="paPlaceCo"></b> <em>*</em></span>' +
      '     <div class="place-search"><input id="paPlace" data-notr type="text" autocomplete="off" placeholder="Start typing a settlement name…">' +
      ...
      '</div>' + (p
        ? '<div class="cp-note">This activity is logged for <b>this project</b>, against one of its KPIs. Locations are searched live from the <b>OpenStreetMap</b> database and pinned to their real coordinates.</div>'
        : '<div class="cp-note"><b>Project is optional.</b> Leave it empty and the activity is still logged against its KPI in the chosen country, ' +
          'counted and plotted - it simply carries no donor or project attribution, and no target. Locations are searched live from the ' +
          '<b>OpenStreetMap</b> database and pinned to their real coordinates.</div>') + '</div>';
    ...
    var _iso = null;                       // the country the settlement search is scoped to
    var coSel = p ? null : $('#paCountry');   // the country picker (standalone only)
    function selProject(){ var v = $('#paProject').value; return v ? DB._idx.projectById[+v] : null; }
    function selIso(){ var pp = selProject(); return pp ? pp.country_iso3 : (coSel ? (coSel.value || null) : null); }
    function selKpi(){ var v = $('#paKpi').value; return v ? prospectiveUse(+v, selProject(), selIso()) : null; }
    function clearPlace(){
      var pe = $('#paPlace'); if (!pe) return;
      pe.value = ''; pe.__place = null;
      $('#paPlaceChosen').textContent = ''; $('#paPlaceChosen').classList.remove('on');
      $('#paPlaceResults').innerHTML = '';
    }
    function syncContext(){
      var r = selKpi(), iso = selIso();
      if (coSel){ if (iso && coSel.value !== iso) coSel.value = iso; }
      else $('#paCountry').value = countryLabel(iso);
      $('#paPlaceCo').textContent = countryLabel(iso);
      if (iso !== _iso){ _iso = iso; clearPlace(); }
      $('#paValLbl').textContent = activityValLabel(r);
      fillReportPreview('paPreview', r, 'paValue');
    }
    if (coSel) coSel.onchange = function (){
      // a new country: its projects, keeping the chosen one when it still fits
      var iso = coSel.value || null, pp = selProject(), keepKpi = $('#paKpi').value ? +$('#paKpi').value : null;
      $('#paProject').innerHTML = activityProjectOptions(pp && (!iso || pp.country_iso3 === iso) ? pp.id : null, iso);
      if (!selProject()) $('#paKpi').innerHTML = activityKpiOptions(null, keepKpi);
      syncContext();
    };
    $('#paProject').onchange = function (){
      var keepKpi = $('#paKpi').value ? +$('#paKpi').value : null;
      $('#paKpi').innerHTML = activityKpiOptions(selProject(), keepKpi);
      syncContext();
    };
    $('#paKpi').onchange = syncContext;
    $('#paValue').addEventListener('input', function (){ fillReportPreview('paPreview', selKpi(), 'paValue'); });
    $('#paCancel').onclick = closeNewActivity;
    $('#paSave').onclick = function (){ submitActivity(); };
    // the search reads the country at query time, so switching project or country
    // re-scopes it without re-binding a second set of listeners
    mountPlaceSearch($('#paPlace'), $('#paPlaceResults'), $('#paPlaceChosen'), function (){ return _iso; });
    syncContext();
    $('#newActivityOverlay').classList.add('on');
    setTimeout(function (){ var f = (coSel && !coSel.value) ? coSel : $('#paKpi'); if (f) f.focus(); }, 0);
  }
  function submitActivity(){
    var msg = $('#paMsg');
    var kpiId = $('#paKpi').value ? +$('#paKpi').value : null;
    var def = kpiId != null ? DB._idx.indicatorById[kpiId] : null;
    if (!def){ msg.textContent = 'Select a KPI.'; return; }
    var projId = $('#paProject').value ? +$('#paProject').value : null;
    var proj = projId != null ? DB._idx.projectById[projId] : null;
    if (proj && !canReportProject(proj)){ msg.textContent = 'You cannot log activities on this project.'; return; }
    var iso = proj ? proj.country_iso3 : ($('#paCountry').value || null);
    if (!iso || !DB._idx.countryByIso[iso]){ msg.textContent = 'Select the country the activity happened in.'; return; }
    if (!canReport({ iso: iso })){ msg.textContent = 'You cannot log activities in this country.'; return; }
    var title = (fv($('#paTitle')) || '').trim();
    ...
    var fromProject = $('#projectModal').classList.contains('on');
    msg.textContent = 'Saving…';
    insertProjectActivity(kpiId, projId, iso, title, valRaw, date, (fv($('#paNote')) || '').trim(), place)
      .then(function (measRow){ return saveBeneficiaries(measRow.id).then(function (){ return measRow; }); })
      // land on the KPI use the activity joined, so the new row is in view
      .then(function (measRow){
        enrich(); renderTicker(); renderAll(); closeNewActivity();
        if (fromProject && projId != null && curProject && projId === curProject.id){ _prActKpi = kpiId; setProjectTab('activities'); }
        else { var nr = useOfMeas(measRow); if (nr){ openDetail(nr); setModalTab('results'); } }
      });
  }
  // Log an activity at a geographic point. An Activity lives in the DELIVERY chain
  // (Donor → Project → Activity), NOT in the results chain: it IS the measurement -
  // one logged report of progress against a KPI. The KPI (indicator_id) is what an
  // activity always has; the project attribution (project_id) is OPTIONAL and may be
  // null, in which case the activity carries no donor or project lineage but is
  // otherwise a first-class activity. The country it happened in is always stored.
  // Beneficiaries hang off it. Returns the measurement row (id assigned).
  function insertProjectActivity(kpiId, projectId, iso, title, valRaw, date, note, place){
    var byId = CURRENT_USER ? CURRENT_USER.id : null;
    var measRow = {
      indicator_id: kpiId, date: date, value: +valRaw,
      narrative: note || title, reported_by_id: byId, project_id: projectId, country_iso3: iso,
  ...
  //  GUIDED TOUR - a self-contained walkthrough of the platform, one file per
  //  language, sitting beside the app. The Arabic tour was captured on the
  //  Arabic interface and the English one on the English interface, so the
  //  active language picks the file rather than translating it.
  function tourUrl(){
    var ar = window.I18N && I18N.lang === 'ar';
    var file = ar ? 'grassroots-tour-arabic.html' : 'grassroots-tour-english.html';
    try { return new URL('tours/' + file, location.href).href; }
    catch (e) { return 'tours/' + file; }
  }
  //  The tour narrates itself with the browser's own speech voices, and the
  //  natural Arabic and English ones ship with Microsoft Edge. On Windows the
  //  microsoft-edge: handler hands it straight to Edge; everywhere else - and
  //  from the fallback link - it opens in whatever browser is already running.
  function openTour(forceHere){
    var url = tourUrl();
    if (!forceHere && /Windows/i.test(navigator.userAgent || '')) {
      try { location.href = 'microsoft-edge:' + url; return; } catch (e) {}
    }
  ...
  function renderOrgPane(){
    ...
    function collect(){
      ...
      return {
        ...
        name: fv(host.querySelector('.org-name')).trim(),
  ...
  function openDonorEdit(d){
    ...
    f.querySelector('.dn-save').onclick = function (){
      var name = fv(f.querySelector('.dn-name')).trim(), msg = f.querySelector('.ufmsg');
  ...
  function partnersEditor(){
    ...
    list.forEach(function (d){
      ...
      if (d.website) contact.push('<a href="' + esc(/^https?:\/\//i.test(d.website) ? d.website : 'https://' + String(d.website).replace(/^[a-z][a-z0-9+.-]*:/i, '')) + '" target="_blank" rel="noopener noreferrer">' + esc(String(d.website).replace(/^https?:\/\//, '')) + '</a>');
      var tr = el('tr');
      tr.innerHTML =
  ...
  function openPartnerEdit(d){
    ...
    f.querySelector('.pn-save').onclick = function (){
      var name = fv(f.querySelector('.pn-name')).trim(), msg = f.querySelector('.ufmsg');
  ...
  // The KPI USES of a plan for the reports, built once per data version (batch
  // generation asks for them hundreds of times). Uses are the unit a report
  // measures: a KPI in a project (with its cascaded baseline/target), a project's
  // own KPI, or unattributed activities in a country.
  var _commUsesCache = null;
  function commUses(planId){
    var idx = DB._idx;
    if (!_commUsesCache || _commUsesCache.idx !== idx || _commUsesCache.plan !== planId){
      _commUsesCache = { idx: idx, plan: planId, built: buildUses(planId) };
    }
    return _commUsesCache.built;
  }
  function commIndicators(ent){
    var cat = ent.cat;
    if (cat === 'plan'){
      return commUses(+ent.ref.split(':')[1]).uses.slice();
    }
    var uses = commUses(S.plan).uses;
    if (cat === 'impact' || cat === 'outcome' || cat === 'output'){
      // every use of a KPI under the statement's subtree; sdg must match EXACTLY
      // (null only pairs with null), or a null-sdg entity would swallow every
      // same-statement subtree of every sdg
      ...
      return uses.filter(function (u){ return u.def.result_id != null && ids[u.def.result_id]; });
    }
    if (cat === 'project'){
      return commProjectKpis(ent.project);
    }
    if (cat === 'donor' || cat === 'partner'){
      ...
      var fld = cat === 'donor' ? 'donor_id' : 'partner_id', entId = cat === 'donor' ? ent.donor.id : ent.partner.id;
      return uses.filter(function (u){ return u.project && u.project[fld] === entId && u.kind !== 'free'; });
    }
    // region / country: the uses that happened there - projects in scope and
    // unattributed activities logged in those countries (a KPI has no country)
    var isoSet = {};
    ...
    return uses.filter(function (u){ return !!(u.iso && isoSet[u.iso]); });
  }
  ...
  // `use` is a KPI use (see commUses): its own activities against its own
  // (cascaded) baseline and target. `projId` is kept for callers; a use already
  // belongs to one project.
  function commSnapshot(use, startISO, endISO, projId){
    var ind = use.raw;
    var upto = (use.series || []).filter(function (m){ return m.date && m.date <= endISO; });
  ...
  /** A project's KPI uses (its adopted KPIs and its own), for the reports. */
  function commProjectKpis(p){
    var by = commUses(p.plan_id).byProjKpi;
    var prim = (DB._idx.projectKpiByProject[p.id] || []).map(function (pk){ return by[p.id + '|' + pk.indicator_id]; })
      .filter(function (u){ return u && u.kind === 'project'; });
    return prim.concat((DB._idx.secondaryByProject[p.id] || []).map(function (i){ return by[p.id + '|' + i.id]; }).filter(Boolean));
  }
  ...
  function commGroups(ent, projRows){
    ...
    DB.tables.result.forEach(function (r){
      ...
      while (stack.length){
        var n = stack.pop();
        (DB._idx.indicatorByResult[n.id] || []).forEach(function (i){ g.ids[i.id] = 1; });   // KPI ids
    ...
    projRows.forEach(function (row){
      ...
      order.forEach(function (g){
        var n = 0; row.kpis.forEach(function (u){ if (g.ids[u.kpiId]) n++; });
  ...
  function commProjectSnap(row, startISO, endISO){
    ...
    var inScope = {}; row.kpis.forEach(function (u){ inScope[u.kpiId] = 1; });
    var acts = 0, ben = 0;
  ...
  function commBuildPdf(ent, year, month){
    ...
    if (projMode){
      ...
      snaps.sort(function (a, b){ return String(a.ind.raw.code || '') < String(b.ind.raw.code || '') ? -1 : 1; });
    ...
    if (projMode){
      ...
      for (var i = 0; i < snaps.length; i++){
        ...
        var sn = snaps[i], ind = sn.ind.raw, s = sn.s;   // the use's KPI with its cascaded baseline/target
        drawCells([
  ...
  function commBuildForecastPdf(ent, year, month){
    ...
    try {
      ...
      // report scopes are KPI uses already (commUses) - they carry raw, series
      // and projectIds, which is all kpiForecast needs
      function toRows(uses){ return uses.slice(); }
  ...
  function commEmailEditor(){
    ...
    function preview(){
      ...
      head.innerHTML =
        '<div class="comm-prev-lbl">PREVIEW - as <span>' + esc(userName(ent.leadId)) + '</span> will receive it</div>' +
  ...
  function openBenTypeEdit(t){
    ...
    f.querySelector('.bt-save').onclick = function (){
      var name = fv(f.querySelector('.bt-name')).trim(), msg = f.querySelector('.ufmsg');
      var code = f.querySelector('.bt-code').value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 3);
      var description = fv(f.querySelector('.bt-desc-in')).trim();
  ...
  // Rename any Impact / Outcome / Output statement (one node per plan; older data
  // may still hold duplicates, so every matching row is renamed).
  ...
  function openFwRename(labelText, level, stmt, pillar){
    openFwPopup('Edit ' + labelText, function (){
      ...
      f.querySelector('.fw-save').onclick = function (){
        var v = fv(f.querySelector('.fw-stmt')).trim();
  ...
  function openFwAddOutcome(pillar){
    openFwPopup('Add outcome', function (){
      ...
      f.querySelector('.fw-cancel').onclick = closeFwEdit;
      f.querySelector('.fw-save').onclick = function (){
        var stmt = fv(f.querySelector('.fw-ostmt')).trim();
  ...
  function openFwAddPillar(){
    openFwPopup('Add impact', function (){
      ...
      f.querySelector('.fw-cancel').onclick = closeFwEdit;
      f.querySelector('.fw-save').onclick = function (){
        var name = fv(f.querySelector('.fw-pname')).trim(), impact = fv(f.querySelector('.fw-pimpact')).trim();
        var leadId = f.querySelector('.fw-lead').value ? +f.querySelector('.fw-lead').value : null;
  ...
  // The parent outcome statement of an existing output.
  ...
  function openFwOutput(mode, pillar, outcomeStmt, curStmt){
    ...
    openFwPopup(isEdit ? 'Edit output' : 'Add output', function (){
      ...
      if (isEdit) { f.querySelector('.fw-stmt').value = curStmt; trField(f); }
      f.querySelector('.fw-cancel').onclick = closeFwEdit;
      f.querySelector('.fw-save').onclick = function (){
        var v = fv(f.querySelector('.fw-stmt')).trim();
        var leadId = f.querySelector('.fw-lead').value ? +f.querySelector('.fw-lead').value : null;
  ...
  // The plan's framework KPIs - one row each (a KPI belongs to no country).
  // `projects` counts the projects that adopted it. Secondary (project-local)
  // KPIs are managed inside their project.
  function kpiGroups(){
    return planKpis(S.plan).map(function (def){
      var res = DB._idx.resultById[def.result_id];
      var projects = (DB._idx.projectKpiByIndicator[def.id] || []).filter(function (pk){
        var p = DB._idx.projectById[pk.project_id]; return p && p.plan_id === S.plan;
      }).length;
      return { name: def.name, rep: def, sdg: res ? res.sdg : null, level: 'output', projects: projects };
    }).sort(function (a, b){
      return String(a.rep.code || '').localeCompare(String(b.rep.code || ''), 'en', { numeric: true }) || (a.name < b.name ? -1 : 1);
    });
  }

  function kpiInventoryEditor(){
    var box = el('div', 'cp-kpis');
    box.appendChild(elHTML('div', 'cp-note', 'Fine-tune KPI definitions - name, type, unit, direction, baseline/target, means & method of measurement, frequency, responsible and <b>parent Output</b>. Every KPI belongs to an Output; use <b>＋ Add KPI</b> to create one under an Output, or Edit to change it. A KPI belongs to no country. Its <b>baseline and target are set here, at plan level, and cascade</b> to every project that adopts it (a project can set its own on its Primary KPIs tab).'));
    ...
    tbl.innerHTML = '<thead><tr><th>KPI</th><th>Impact</th><th>Unit</th><th>Baseline</th><th>Target</th><th title="Projects that adopted this KPI">Projects</th><th></th></tr></thead>';
    var tb = el('tbody');
  ...
  function kpiRow(g){
    ...
    var u = kpiUnit(g.rep) === '%' ? '%' : '';
    tr.innerHTML = '<td><span class="udot" style="background:' + col + '"></span>'
        + (g.rep.code ? '<span class="kpi-code">' + esc(g.rep.code) + '</span> ' : '')
        + '<span class="kpi-name">' + esc(g.name) + '</span></td>'
      ...
      + '<td class="umono">' + fmtNum(g.rep.baseline_value) + u + '</td>'
      + '<td class="umono">' + fmtNum(g.rep.target_value) + u + '</td><td class="umono">' + fmt(g.projects) + '</td>';
    var act = el('td', 'uact');
  ...
  function openKpiEdit(g){
    ...
    $('#kpiEditTitle').textContent = 'Edit KPI · ' + (g.rep.code ? g.rep.code + ' · ' : '') + g.name;
  ...
  function kpiForm(g){
    ...
    f.innerHTML =
      '<div class="uform-h">Edit KPI</div>' +
      '<div class="ufgrid kpigrid">' +
      ...
      '  <label title="Plan-level - cascades to every project that adopts this KPI"><span>Baseline value (plan)</span><input class="kf-base" type="number" step="any" value="' + esc(ind.baseline_value) + '"></label>' +
      '  <label><span>Baseline date</span><input class="kf-basedate" type="date" value="' + esc(baseDate) + '"></label>' +
      '  <label title="Plan-level - cascades to every project that adopts this KPI"><span>Target value (plan)</span><input class="kf-tgt" type="number" step="any" value="' + esc(ind.target_value) + '"></label>' +
  ...
  function saveKpiEdits(g, f, curParent){
    ...
    var num = function (x){ return x === '' || isNaN(+x) ? null : +x; };
    var changed = [], touchedOutputs = {};
    [g.rep].forEach(function (ind){
      ind.name = newName;
      ...
      // re-parent to ANY sibling result (same level): the chosen statement's one
      // node in this plan's framework (the KPI adopts its pillar via the chain).
      if (newParent && newParent !== curParent){
        var nr = DB.tables.result.filter(function (x){ return x.plan_id === S.plan && x.level === g.level && x.statement === newParent; })[0];
        if (nr && nr.id !== ind.result_id){ touchedOutputs[ind.result_id] = 1; touchedOutputs[nr.id] = 1; ind.result_id = nr.id; }
      }
      changed.push(ind);
    });
    // A move renumbers BOTH Outputs - the one left (closing its gap) and the one
    // joined - so no two KPIs ever share a code. Ordinals follow id order, the
    // same rule DB._codeForIndicator uses for a new KPI.
    Object.keys(touchedOutputs).forEach(function (rid){
      var out = DB._idx.resultById[+rid];
      var base = out && out.code ? String(out.code).replace(/^\s*[A-Za-z]+\s+/, '') : '';
      DB.tables.indicator.filter(function (i){ return i.result_id === +rid && !i.secondary; })
        .sort(function (a, b){ return a.id - b.id; })
        .forEach(function (i, k){
          var code = 'KPI ' + base + '.' + (k + 1);
          if (i.code !== code){ i.code = code; if (changed.indexOf(i) < 0) changed.push(i); }
        });
    });
  ...
  // Add a brand-new KPI, assigned to a parent Output (one row - a KPI belongs to
  // no country).
  function openKpiAdd(){
    var body = $('#kpiEditBody'); body.innerHTML = '';
  ...
  function kpiAddForm(){
    var f = el('div', 'uform kpiuform');
    f.innerHTML =
      '<div class="uform-h">New KPI - added under the chosen Output</div>' +
      '<div class="ufgrid kpigrid">' +
  ...
  function saveNewKpi(f){
    ...
    // ONE indicator under the Output (code auto-generated) - never a per-country copy
    base.result_id = outputs[0].id;
    var rows = [base];
    msg.textContent = 'Saving…';
  ...
  function userAssignments(u){
    ...
    if (key === 'plan') allPlans().forEach(function (p){ if (p.lead_id === u.id) add(p.name); });
      ...
      DB.tables.result.forEach(function (r){ if (r.level === key && r.owner_id === u.id) add(r.code || r.statement); });   // dedupe by code
  ...
  function userForm(u){
    ...
    f.querySelector('.uf-save').onclick = function (){
      ...
      // never let an administrator lock themselves out from their own record
      if (!isNew && CURRENT_USER && u.id === CURRENT_USER.id){
        var stRow = lkRow('user_status', statusId);
        if (!enabled) { msg.textContent = 'You cannot disable your own account.'; return; }
        if (isAdmin() && (!stRow || stRow.key !== 'admin')) { msg.textContent = 'You cannot remove your own Admin status - ask another Admin.'; return; }
      }
      if (isNew) {
  ...
  // =========================================================================
  //  GLOBALIZATION - the languages the platform speaks, and every word it says
  //  in them. Languages, interface wording and translated database content are
  //  all rows (see the `language` / `phrase` / `translation` tables), so an
  //  administrator adds a language here and the switcher, the catalogue and the
  //  background translator pick it up with no code change.
  // =========================================================================
  var GL = { tab: 'languages', lang: null, q: '', missing: false, page: 0 };
  var GL_PER_PAGE = 40;

  function i18nReady(){ return !!(window.I18N && I18N.ready); }
  function openGlobal(){
    if (!canEditFramework()) return;          // Admin only - the switcher is everyone's
    renderGlobal(GL.tab); $('#glModal').classList.add('on');
  }
  function closeGlobal(){ $('#glModal').classList.remove('on'); }

  /** The languages a translation can be written INTO (everything but the source). */
  function glTargets(){
    return i18nReady() ? I18N.languages().filter(function (l){ return !l.is_source; }) : [];
  }
  /** The language the Interface / Content tabs are currently editing. */
  function glLang(){
    var list = glTargets(); if (!list.length) return null;
    var hit = null;
    list.forEach(function (l){ if (l.id === GL.lang) hit = l; });
    if (!hit){ hit = list[0]; GL.lang = hit.id; }
    return hit;
  }
  function glPhrases(scope){
    return (DB.tables.phrase || []).filter(function (p){ return (p.scope || 'ui') === scope; });
  }
  /** phrase id -> translation row, for one language. */
  function glTransIndex(langId){
    var m = {};
    (DB.tables.translation || []).forEach(function (t){ if (t.language_id === langId) m[t.phrase_id] = t; });
    return m;
  }
  /** How much of the catalogue a language covers: {done, total}. */
  function glCoverage(langId){
    var total = (DB.tables.phrase || []).length;
    var done = 0, idx = glTransIndex(langId);
    Object.keys(idx).forEach(function (k){ if (idx[k].text && String(idx[k].text).trim()) done++; });
    return { done: done, total: total };
  }

  /** Write (or rewrite) one translation. `auto` 0 marks it as reviewed wording,
   *  which the background translator will never overwrite. */
  function glSaveTranslation(phraseId, langId, text, auto){
    var cur = null;
    (DB.tables.translation || []).forEach(function (t){ if (t.phrase_id === phraseId && t.language_id === langId) cur = t; });
    var stamp = new Date().toISOString();
    var job;
    if (cur){ cur.text = text; cur.auto = auto ? 1 : 0; cur.updated = stamp; job = DB.persist('translation', [cur]); }
    else job = DB.insert('translation', { phrase_id: phraseId, language_id: langId, text: text, auto: auto ? 1 : 0, updated: stamp });
    return Promise.resolve(job).then(function (){ I18N.reindex(); I18N.applyAll(); });
  }

  function renderGlobal(tab){
    tab = tab || 'languages';
    GL.tab = tab;
    Array.prototype.forEach.call($('#glTabs').children, function (x){ x.classList.toggle('on', x.dataset.gltab === tab); });
    var body = $('#glBody'); body.innerHTML = '';
    if (!i18nReady()){ body.appendChild(el('div', 'cp-note', 'The globalization layer is still starting up.')); return; }
    if (tab === 'interface') body.appendChild(glPhraseEditor('ui'));
    else if (tab === 'content') body.appendChild(glPhraseEditor('content'));
    else if (tab === 'settings') body.appendChild(glSettingsEditor());
    else body.appendChild(glLanguagesEditor());
  }

  // ---- Languages tab -------------------------------------------------------
  function glLanguagesEditor(){
    var box = el('div', 'cp-users');
    box.appendChild(elHTML('div', 'cp-note',
      'The platform is authored in the <b>source language</b> and speaks every other language as a ' +
      'translation of it. Add a language here and it appears in the switcher at the top of the screen ' +
      'for every user; disable one to take it out of circulation without losing a word of its wording.'));
    var add = el('button', 'hbtn primary', '＋ Add language');
    add.onclick = function (){ openLangEdit(null); };
    box.appendChild(add);
    var tbl = el('table', 'utbl');
    tbl.innerHTML = '<thead><tr><th>Language</th><th>Native name</th><th>Code</th><th>Direction</th>' +
                    '<th>Translated</th><th>Enabled</th><th></th></tr></thead>';
    var tb = el('tbody');
    I18N.languages().forEach(function (l){
      var cov = l.is_source ? null : glCoverage(l.id);
      var tr = el('tr');
      tr.innerHTML =
        '<td>' + esc(l.name) + (l.is_source ? ' <span class="gl-tag">source</span>' : '') + '</td>' +
        '<td>' + esc(l.native_name || '') + '</td>' +
        '<td class="umono" data-notr>' + esc(l.key) + '</td>' +
        '<td>' + (l.direction === 'rtl' ? 'Right to left' : 'Left to right') + '</td>' +
        '<td class="umono">' + (cov ? (fmt(cov.done) + ' / ' + fmt(cov.total)) : '–') + '</td>' +
        '<td>' + (l.enabled === 0 ? 'No' : 'Yes') + '</td>';
      var act = el('td', 'uact');
      var ed = el('button', 'cp-mini', 'Edit'); ed.onclick = function (){ openLangEdit(l); };
      act.appendChild(ed);
      if (!l.is_source){
        var del = el('button', 'cp-del', '🗑'); del.title = 'Delete this language and its translations';
        del.onclick = function (){
          if (!confirm('Delete "' + l.name + '" and every translation written in it?')) return;
          var ids = (DB.tables.translation || []).filter(function (t){ return t.language_id === l.id; }).map(function (t){ return t.id; });
          var jobs = [];
          if (ids.length) jobs.push(DB.remove('translation', ids));
          jobs.push(DB.remove('language', [l.id]));
          Promise.all(jobs).then(function (){
            if (I18N.lang === l.key) I18N.setLang(I18N.source);
            I18N.reindex(); renderLangChip(); renderGlobal('languages');
          });
        };
        act.appendChild(del);
      }
  ...
  function openLangEdit(l){
    var body = $('#langEditBody'); body.innerHTML = '';
    $('#langEditTitle').textContent = l ? ('Edit language · ' + l.name) : '＋ Add language';
    var f = el('div', 'uform');
    f.innerHTML =
      '<div class="ufgrid" style="grid-template-columns:2fr 2fr 1fr">' +
      '  <label><span>Language name *</span><input class="lg-name" type="text" value="' + esc(l ? l.name : '') + '" placeholder="e.g. French"></label>' +
      '  <label><span>Native name</span><input class="lg-native" type="text" value="' + esc(l && l.native_name ? l.native_name : '') + '" placeholder="e.g. Français"></label>' +
      '  <label><span>Code *</span><input class="lg-key" type="text" maxlength="8" value="' + esc(l ? l.key : '') + '" placeholder="fr"></label>' +
      '</div>' +
      '<div class="ufgrid" style="grid-template-columns:1fr 1fr">' +
      '  <label><span>Reading direction</span><select class="lg-dir">' +
      '    <option value="ltr"' + (l && l.direction === 'rtl' ? '' : ' selected') + '>Left to right</option>' +
      '    <option value="rtl"' + (l && l.direction === 'rtl' ? ' selected' : '') + '>Right to left</option></select></label>' +
      '  <label><span>Availability</span><select class="lg-en">' +
      '    <option value="1"' + (l && l.enabled === 0 ? '' : ' selected') + '>Offered in the switcher</option>' +
      '    <option value="0"' + (l && l.enabled === 0 ? ' selected' : '') + '>Hidden</option></select></label>' +
      '</div>' +
      '<div class="cp-note">The <b>code</b> is the standard two-letter language tag (en, ar, fr, es…). It is what the ' +
      'translation service is asked for, so it has to be the real tag, not a nickname.</div>' +
      '<div class="ufbtns"><span class="ufmsg"></span>' +
      '<button class="hbtn lg-cancel" type="button">Cancel</button>' +
      '<button class="hbtn primary lg-save" type="button">' + (l ? 'Save changes' : 'Add language') + '</button></div>';
    f.querySelector('.lg-cancel').onclick = closeLangEdit;
    f.querySelector('.lg-save').onclick = function (){
      var msg = f.querySelector('.ufmsg');
      var name = f.querySelector('.lg-name').value.trim();
      var key = f.querySelector('.lg-key').value.trim().toLowerCase();
      var native = f.querySelector('.lg-native').value.trim();
      var dir = f.querySelector('.lg-dir').value;
      var en = +f.querySelector('.lg-en').value;
      if (!name || !key){ msg.textContent = 'Name and code are required.'; return; }
      if (!/^[a-z]{2,3}(-[a-z0-9]{2,8})?$/.test(key)){ msg.textContent = 'Use a standard language tag, e.g. fr or pt-br.'; return; }
      var clash = I18N.languageByKey(key);
      if (clash && (!l || clash.id !== l.id)){ msg.textContent = 'That code is already in use.'; return; }
      if (l && l.is_source && en === 0){ msg.textContent = 'The source language cannot be hidden.'; return; }
      msg.textContent = 'Saving…';
      var job;
      if (l){
        l.name = name; l.key = key; l.native_name = native || null; l.direction = dir; l.enabled = en;
        job = DB.persist('language', [l]);
      } else {
        var seq = (DB.tables.language || []).length + 1;
        job = DB.insert('language', { key: key, name: name, native_name: native || null,
                                      direction: dir, is_source: 0, enabled: en, seq: seq });
      }
      Promise.resolve(job).then(function (){
        I18N.reindex(); closeLangEdit(); renderLangChip(); renderGlobal('languages');
      });
    };
    body.appendChild(f);
    $('#langEditOverlay').classList.add('on');
    var fn = f.querySelector('.lg-name'); if (fn) fn.focus();
  }
  function closeLangEdit(){ var o = $('#langEditOverlay'); if (o) o.classList.remove('on'); }

  // ---- Interface / Content tabs -------------------------------------------
  //  Both tabs are the same editor over the same catalogue; only the scope
  //  differs - 'ui' is the wording the platform itself uses, 'content' is the
  //  text held in the database (each row says which field it came from).
  function glPhraseEditor(scope){
    var box = el('div', 'cp-users gl-pane');
    var lang = glLang();
    if (!lang){
      box.appendChild(elHTML('div', 'cp-note', 'Add a second language on the <b>Languages</b> tab first - ' +
        'there is nothing to translate into yet.'));
      return box;
    }
    box.appendChild(elHTML('div', 'cp-note', scope === 'ui'
      ? 'Every word the interface itself says. Blank rows are wording the platform has met but not translated ' +
        'yet; anything you type here is marked <b>reviewed</b> and the background translator will never ' +
        'overwrite it.'
      : 'Every piece of text held in the database - project and donor names, result statements, activity ' +
        'narratives and the rest. New records are translated in the background as they are written; use ' +
        '<b>Scan the database</b> to sweep up everything that was already there.'));

    // toolbar: language, search, filter, bulk actions
    var bar = el('div', 'gl-bar');
    var langSel = el('select', 'gl-lang');
    glTargets().forEach(function (l){
      var o = el('option', null, l.name + (l.native_name ? ' · ' + l.native_name : ''));
      o.value = l.id; if (l.id === lang.id) o.selected = true;
      langSel.appendChild(o);
    });
    langSel.onchange = function (){ GL.lang = +langSel.value; GL.page = 0; renderGlobal(scope === 'ui' ? 'interface' : 'content'); };
    bar.appendChild(langSel);

    var q = el('input', 'gl-q'); q.type = 'search'; q.placeholder = 'Search the catalogue…'; q.value = GL.q;
    q.oninput = debounce(function (){ GL.q = q.value; GL.page = 0; glFillRows(box, scope); }, 160);
    bar.appendChild(q);

    var only = el('label', 'gl-only');
    only.innerHTML = '<input type="checkbox"' + (GL.missing ? ' checked' : '') + '> Untranslated only';
    only.querySelector('input').onchange = function (){ GL.missing = this.checked; GL.page = 0; glFillRows(box, scope); };
    bar.appendChild(only);

    bar.appendChild(el('span', 'gl-spacer'));

    if (scope === 'content'){
      var scan = el('button', 'hbtn', '⟳ Scan the database');
      scan.title = 'Register every translatable value currently held in the database';
      scan.onclick = function (){
        var r = I18N.syncContent();
        renderGlobal('content');
        glStatus($('#glBody').firstChild, fmt(r.total) + ' values in the database, ' + fmt(r.added) +
                 ' newly catalogued, ' + fmt(r.missing) + ' still untranslated.');
      };
      bar.appendChild(scan);
    }
    var all = el('button', 'hbtn primary', '✨ Translate what is missing');
    all.onclick = function (){
      var missing = glRows(scope).filter(function (r){ return !r.text; });
      if (!missing.length){ glStatus(box, 'Nothing is missing - this language is complete.'); return; }
      // an explicit admin request overrides the "never machine-translate a proper
      // noun" rule - they asked for these specific rows
      missing.forEach(function (r){ I18N.queue(r.source, true); });
      glStatus(box, 'Queued ' + fmt(missing.length) + ' phrase(s). Translating in the background…');
      I18N.flush();
    };
    bar.appendChild(all);
    box.appendChild(bar);

    box.appendChild(el('div', 'gl-status'));
    box.appendChild(el('div', 'gl-rows'));
    glFillRows(box, scope);
    return box;
  }

  function glStatus(box, text){
    var s = box && box.querySelector ? box.querySelector('.gl-status') : null;
    if (s) s.textContent = text || '';
  }

  /** The catalogue rows for the current scope / language / search filter. */
  function glRows(scope){
    var lang = glLang(); if (!lang) return [];
    var idx = glTransIndex(lang.id);
    var q = fold((GL.q || '').trim());
    return glPhrases(scope).map(function (p){
      var tr = idx[p.id];
      return { phrase: p, source: p.source, tr: tr || null,
               text: (tr && tr.text && String(tr.text).trim()) ? tr.text : '',
               reviewed: !!(tr && tr.auto === 0) };
    }).filter(function (r){
      if (GL.missing && r.text) return false;
      if (!q) return true;
      return fold(r.source).indexOf(q) >= 0 || fold(r.text).indexOf(q) >= 0;
    }).sort(function (a, b){ return String(a.source).localeCompare(String(b.source)); });
  }

  function glFillRows(box, scope){
    var host = box.querySelector('.gl-rows'); if (!host) return;
    var lang = glLang(); if (!lang) return;
    host.innerHTML = '';
    var all = glRows(scope);
    var pages = Math.max(1, Math.ceil(all.length / GL_PER_PAGE));
    if (GL.page >= pages) GL.page = pages - 1;
    if (GL.page < 0) GL.page = 0;
    var slice = all.slice(GL.page * GL_PER_PAGE, (GL.page + 1) * GL_PER_PAGE);

    var done = all.filter(function (r){ return r.text; }).length;
    // "442 / 472 translated" rather than a sentence with the numbers inside it:
    // the digits carry no words, so the label survives translation intact.
    host.appendChild(el('div', 'gl-count', fmt(done) + ' / ' + fmt(all.length) + ' translated'));

    var tbl = el('table', 'utbl gl-tbl');
    tbl.innerHTML = '<thead><tr><th>Source text (English)</th>' +
      (scope === 'content' ? '<th class="gl-field">Field</th>' : '') +
      '<th>' + esc(lang.name) + '</th><th class="gl-actcol"></th></tr></thead>';
    var tb = el('tbody');
    if (!slice.length) tb.innerHTML = '<tr><td colspan="4" class="re-empty">Nothing here yet.</td></tr>';
    slice.forEach(function (r){
      var tr = el('tr');
      var src = el('td', 'gl-src'); src.textContent = r.source; tr.appendChild(src);
      if (scope === 'content'){
        var fd = el('td', 'gl-field');
        fd.setAttribute('data-notr', '');   // a column name, not wording
        fd.textContent = r.phrase.table_name ? (r.phrase.table_name + '.' + (r.phrase.column_name || '')) : '–';
        tr.appendChild(fd);
      }
      var td = el('td', 'gl-val');
      var inp = el('input', 'gl-in' + (r.reviewed ? ' reviewed' : ''));
      inp.type = 'text'; inp.value = r.text || '';
      inp.placeholder = 'Not translated yet';
      if (lang.direction === 'rtl') inp.dir = 'rtl';
      inp.onchange = function (){
        glSaveTranslation(r.phrase.id, lang.id, inp.value.trim(), 0).then(function (){
          inp.classList.add('reviewed');
          glStatus(box, 'Saved: ' + r.source);
        });
      };
      td.appendChild(inp); tr.appendChild(td);
      var act = el('td', 'uact');
      var mt = el('button', 'cp-mini', '✨');
      mt.title = 'Translate this one now';
      mt.onclick = function (){
        mt.disabled = true; var was = inp.value; inp.value = '…';
        I18N.translateNow(r.source, lang.key).then(function (out){
          mt.disabled = false;
          if (!out){ inp.value = was; glStatus(box, 'The translation service did not answer - try again.'); return; }
          inp.value = out;
          glSaveTranslation(r.phrase.id, lang.id, out, 1);
        });
      };
      act.appendChild(mt); tr.appendChild(act);
      tb.appendChild(tr);
    });
    tbl.appendChild(tb); host.appendChild(tbl);

    if (pages > 1){
      var pg = el('div', 'gl-pager');
      var prev = el('button', 'cp-mini', '‹ Previous');
      prev.disabled = GL.page === 0;
      prev.onclick = function (){ GL.page--; glFillRows(box, scope); };
      var next = el('button', 'cp-mini', 'Next ›');
      next.disabled = GL.page >= pages - 1;
      next.onclick = function (){ GL.page++; glFillRows(box, scope); };
      pg.appendChild(prev); pg.appendChild(el('span', 'gl-pg', (GL.page + 1) + ' / ' + pages)); pg.appendChild(next);
      host.appendChild(pg);
    }
  }

  // ---- Settings tab --------------------------------------------------------
  function glSettingsEditor(){
    var box = el('div', 'cp-users');
    var st = I18N.settings();
    box.appendChild(elHTML('div', 'cp-note',
      'The platform ships its own vocabulary, so it reads correctly with no network at all. Beyond that, ' +
      '<b>records you write</b> - a new project, activity or partner - are translated in the background by a ' +
      'free service - <b>Google Translate</b> where this installation runs its own backend, <b>MyMemory</b> ' +
      'otherwise, neither needing an account - and saved, so they are never asked for twice. Wording ' +
      'the platform merely meets on screen is <b>catalogued but not sent</b>: it appears on the Interface and ' +
      'Content tabs for you to write, or to fill in bulk with <b>Translate what is missing</b>. ' +
      '<b>Never sent anywhere:</b> user names, usernames and email addresses; postal addresses, which have to ' +
      'stay legible to the postal service that delivers to them; and codes and acronyms. Turn the background ' +
      'translator off to keep everything local; wording already saved keeps working.'));

    var f = el('div', 'uform');
    f.innerHTML =
      '<label class="org-onoff"><input type="checkbox" class="gl-auto"' + (st.auto ? ' checked' : '') + '> ' +
      '<b>Translate new wording automatically</b><span class="org-status"></span></label>' +
      '<div class="ufgrid" style="grid-template-columns:2fr 1fr">' +
      '  <label><span>Self-hosted translation endpoint (optional)</span>' +
      '    <input class="gl-endpoint" type="text" value="' + esc(st.endpoint || '') + '" placeholder="https://your-libretranslate/translate"></label>' +
      '  <label><span>Endpoint key (optional)</span>' +
      '    <input class="gl-key" type="text" value="' + esc(st.apiKey || '') + '" placeholder="leave blank if none"></label>' +
      '</div>' +
      '<div class="cp-note">Give a <b>LibreTranslate-compatible</b> address and the platform asks it first, so no ' +
      'text reaches a third party. Leave it blank and it falls back to the public free services.</div>' +
      '<div class="ufbtns"><span class="ufmsg gl-msg"></span>' +
      '<button class="hbtn primary gl-save" type="button">Save settings</button></div>';
    box.appendChild(f);
    ...
    function paint(){
      var s = I18N.stats;
      var lines = ['<b>' + fmt((DB.tables.phrase || []).length) + '</b> phrases catalogued'];
      glTargets().forEach(function (l){
        var c = glCoverage(l.id);
        lines.push('<b>' + fmt(c.done) + '</b> of <b>' + fmt(c.total) + '</b> translated into <b>' +
                   esc(l.name) + '</b>');
      });
      lines.push('<b>' + fmt(I18N.queueLength()) + '</b> waiting in the background queue');
      lines.push('<b>' + fmt(s.done) + '</b> translated this session, <b>' + fmt(s.failed) + '</b> could not be reached');
      stats.innerHTML = '<div class="gl-stat-list">' + lines.map(function (x){ return '<div class="gl-stat">' + x + '</div>'; }).join('') + '</div>';
    }
    paint();
    I18N.onProgress = function (){ if ($('#glModal').classList.contains('on')) paint(); };

    var msg = f.querySelector('.gl-msg');
    function save(note){
      I18N.saveSettings({
        auto: f.querySelector('.gl-auto').checked,
        endpoint: f.querySelector('.gl-endpoint').value,
        apiKey: f.querySelector('.gl-key').value
      });
      msg.textContent = note || '✓ Saved.'; msg.classList.add('ok');
    }
    f.querySelector('.gl-save').onclick = function (){ save('✓ Settings saved.'); };
    f.querySelector('.gl-auto').onchange = function (){
      save(this.checked ? '✓ The background translator is on.' : '✓ The background translator is off.');
    };
    return box;
  }

  // ---- header language switcher (every user, not just administrators) ------
  function renderLangChip(){
    var chip = $('#langChip'); if (!chip || !i18nReady()) return;
    var cur = I18N.current();
    var nm = chip.querySelector('.langchip-nm');
    if (nm) nm.textContent = cur ? (cur.native_name || cur.name) : 'English';
    chip.title = 'Language' + (cur ? ' · ' + cur.name : '');
    var box = $('#langbox');
    if (box) box.classList.toggle('hide', I18N.enabledLanguages().length < 2);
  }
  function closeLangMenu(){
    var m = $('#langMenu'); if (m){ m.hidden = true; m.innerHTML = ''; }
    var c = $('#langChip'); if (c) c.setAttribute('aria-expanded', 'false');
  }
  function toggleLangMenu(){
    var m = $('#langMenu'); if (!m || !i18nReady()) return;
    if (!m.hidden){ closeLangMenu(); return; }
    m.innerHTML = ''; m.hidden = false;
    var c = $('#langChip'); if (c) c.setAttribute('aria-expanded', 'true');
    m.appendChild(el('div', 'lang-head', 'Language'));
    I18N.enabledLanguages().forEach(function (l){
      var b = el('button', 'lang-item' + (l.key === I18N.lang ? ' on' : ''));
      b.innerHTML = '<span class="lang-code">' + esc(String(l.key).toUpperCase()) + '</span>' +
                    '<span class="lang-nm">' + esc(l.native_name || l.name) + '</span>' +
                    '<span class="lang-en">' + esc(l.name) + '</span>';
      b.onclick = function (){ closeLangMenu(); if (l.key !== I18N.lang) I18N.setLang(l.key); };
      m.appendChild(b);
    });
  }
  ...
  function deletePlan(id){
    ...
    return Promise.all([
      evRemoveWhere(function (e){ return indSet[e.indicator_id] || projSet[e.project_id]; }),
  ...
  // update every result row matching level+statement(+pillar)
  ...
  function updateResults(level, oldStmt, pillar, newStmt, leadId){
    ...
    // an active filter on the renamed node follows the new statement, or the
    // whole view would silently empty
    if (newStmt != null && newStmt !== oldStmt && S.selNodes.has(level + SEP + oldStmt)){
      S.selNodes.delete(level + SEP + oldStmt); S.selNodes.add(level + SEP + newStmt);
    }
    return changed.length ? DB.persist('result', changed) : Promise.resolve();
  }

  // add a new output under the parent outcome (active plan) - one node, no copies.
  ...
  function addOutput(pillar, outcomeStmt, outStmt, leadId){
    var outcomes = DB.tables.result.filter(function (x){ return x.plan_id === S.plan && x.level === 'outcome' && x.statement === outcomeStmt && x.sdg === pillar; }).slice(0, 1);
    ...
    var newOuts = outcomes.map(function (oc){
      ...
      return { plan_id: oc.plan_id, parent_id: oc.id, level: 'output',
  ...
  // add a brand-new Pillar (impact) to the active plan's framework - one node
  ...
  function addPillar(name, impactStmt, leadId){
    ...
    var rows = [0].map(function (){
      // `code` is system-generated in DB.insert (Impact #) - never set here.
      return { plan_id: S.plan, parent_id: null, level: 'impact',
  ...
  // add a new Outcome under the pillar's impact (active plan) - one node
  function addOutcome(pillar, outcomeStmt, leadId){
    var impacts = DB.tables.result.filter(function (x){ return x.plan_id === S.plan && x.level === 'impact' && x.sdg === pillar; }).slice(0, 1);
    ...
    var rows = impacts.map(function (im){
      ...
      return { plan_id: im.plan_id, parent_id: im.id, level: 'outcome',
  ...
  function deleteOutput(pillar, outStmt){
    ...
    var pkIds = DB.tables.project_kpi.filter(function (pk){ return indSet[pk.indicator_id]; }).map(function (pk){ return pk.id; });
    S.selNodes.delete('output' + SEP + outStmt);   // a filter on a deleted node would empty every view
    return Promise.all([
      evRemoveWhere(function (e){ return indSet[e.indicator_id]; }),
      DB.remove('project_kpi', pkIds),   // the links die with the KPIs, or a reused id inherits them
      removeActivities(measIds),
  ...
  function frameworkEditor(){
    ...
    box.appendChild(el('div', 'cp-note', ro
      ...
      : 'This is the Results Framework of the plan shown above (switch or manage plans from 🗂 Plan Management in the header). Use the Edit buttons to rename a statement or assign its Lead, and the ＋ controls to add Impacts, Outcomes and Outputs. Each Impact, Outcome and Output exists once in the plan - KPIs and results belong to no country - and every change is saved immediately.'));
  ...
  // The system-generated code of a framework node, used to prefix statements in
  // the framework tree.
  ...
  /** Shorten a label to fit. Translate FIRST, then cut: cutting first would
   *  leave a fragment of an English word, which no catalogue can answer and
   *  which would be catalogued as if it were wording of its own. */
  function truncTxt(s, n){ s = String(window.I18N ? I18N.t(s) : s); return s.length > n ? s.slice(0, n - 1) + '…' : s; }
  ...
  function renderTicker() {
    ...
    add(lead, 'sep', '·'); add(lead, 'tv', distinctKpiCount(IND) + ' KPIs');
  ...
  //  WIRING
  // =========================================================================
  // Preset period labels. The picker itself was retired (the dashboard is always
  // all-time), but a saved preference can still name a range, and the filter strip
  // has to be able to spell it - see activeFilterSummary().
  ...
  function wire() {
    ...
    $('#tabs').addEventListener('click', function (e) {
      ...
      syncSpin();   // leaving the map pane stands the globe rotation down
      renderAll();
    });
    ...
    // Globalization modal: close + tab switching, and the language child popup
    var bGl=$('#btnGlobal'); if(bGl) bGl.onclick=openGlobal;
    var glC=$('#glClose'); if(glC) glC.onclick=closeGlobal;
    var glT=$('#glTabs'); if(glT) glT.addEventListener('click',function(e){
      var b=e.target.closest('button'); if(!b) return; GL.page=0; renderGlobal(b.dataset.gltab);
    });
    var leC=$('#langEditClose'); if(leC) leC.onclick=closeLangEdit;

    // Header language switcher - open to every user
    var lChip=$('#langChip'); if(lChip) lChip.onclick=function(e){ e.stopPropagation(); toggleLangMenu(); };
    var lMenu=$('#langMenu'); if(lMenu) lMenu.addEventListener('click',function(e){ e.stopPropagation(); });
    document.addEventListener('click',function(e){
      var m=$('#langMenu'); if(m && !m.hidden && !$('#langbox').contains(e.target)) closeLangMenu();
    });
    ...
    var tourBtn=$('#aboutTourBtn'); if(tourBtn) tourBtn.onclick=function(){ openTour(false); };
    var tourHere=$('#aboutTourHere'); if(tourHere) tourHere.onclick=function(){ openTour(true); };
    var avEl=$('#appVersion'); if(avEl) avEl.textContent=APP_VERSION;   // render the app version from its single source of truth
    var bNPSide=$('#btnNewProjectSide');
    if(bNPSide) bNPSide.onclick=function(){
      if (S.countBasis === 'activities') {
        if (!CURRENT_USER || !canEditProjects()) { alert('Your account can view activities but not log them.'); return; }
        openActivityAdd(null, null);
    ...
    var prEval = $('#prEvalPdf');
    if (prEval) prEval.onclick = function (){ exportEvaluationReport(curProject); };
    $('#prTabs').addEventListener('click', function (e){ var b = e.target.closest('button'); if (!b) return; setProjectTab(b.dataset.prtab); });
    // Evidence popups (Project → Evaluation)
    $('#evClose').onclick = closeEvidence;
    $('#pkPickClose').onclick = closePrimaryPicker;
    $('#evItemClose').onclick = closeEvidenceItem;
    ...
    document.addEventListener('keydown',function(e){
      if(e.key==='Escape'){
        ...
        if ($('#langMenu') && !$('#langMenu').hidden) { closeLangMenu(); return; }
        if ($('#langEditOverlay').classList.contains('on')) { closeLangEdit(); return; }
        ...
        if ($('#pkPickOverlay').classList.contains('on')) { closePrimaryPicker(); return; }
        if ($('#evItemOverlay').classList.contains('on')) { closeEvidenceItem(); return; }
        if ($('#evOverlay').classList.contains('on')) { closeEvidence(); return; }
        ...
        if ($('#glModal').classList.contains('on')) { closeGlobal(); return; }
  ...
  function wireResizer(handle, side){
    ...
    handle.addEventListener('mousedown', function (e) {
      ...
      // In a right-to-left layout the grid mirrors, so the "left" pane is the one
      // against the right edge of the screen. Measure from the edge the pane is
      // actually on, or dragging inverts.
      var rtl = document.documentElement.getAttribute('dir') === 'rtl';
      function move(ev){
        var fromStart = rtl ? (rect.right - ev.clientX) : (ev.clientX - rect.left);
        var fromEnd   = rtl ? (ev.clientX - rect.left)  : (rect.right - ev.clientX);
        if (side === 'left') S.leftW = clamp(Math.round(fromStart), 150, Math.min(460, rect.width - S.rightW - 220));
        else S.rightW = clamp(Math.round(fromEnd), 170, Math.min(560, rect.width - S.leftW - 220));
        setGrid();
      }
  ...
  function syncBasisToggles(){
    var acts = S.countBasis === 'activities';
    var c = $('#countBasis'); if (c) c.textContent = '⇄ ' + (acts ? 'Activities' : 'Projects');
    // the side pane's create button follows the basis: a project, or an activity
    // logged straight against a KPI with no project needed
    var b = $('#btnNewProjectSide');
    if (b){
      b.textContent = acts ? '＋ New Activity' : '＋ New Project';
      b.title = acts ? 'Log an activity against a KPI - naming a project is optional' : 'Create a new project';
    }
  }
  ...
  function activeFilterSummary(){
    ...
    add('Geographical region', names(S.selProg, function (id){
  ...
  function persist() {
    try {
      ...
      var p = {
        ...
        colorMode: S.colorMode, mapMode: S.mapMode, globeSpin: S.globeSpin, perfBasis: S.perfBasis, expandRegion: Array.from(S.expandRegion),
  ...
  function loadPrefs() {
    ...
    try {
      ...
      // Projection is sticky across reloads. A reader who has never chosen one
      // gets the globe; a reader who has chosen keeps their choice, either way.
      if (p.mapMode === 'globe' || p.mapMode === 'flat') S.mapMode = p.mapMode;
      S.globeSpin = (p.globeSpin !== false);                    // rotation is on unless it was paused
      ...
      // 'quad' is a retired mode - an old saved state falls back to Bar
      if (p.insMode && p.insMode !== 'quad') S.insMode = p.insMode;
  ...
  // ROW-LEVEL OWNERSHIP: edit/delete of an existing row is limited to the user who
  // created it (created_by), with Admins overriding universally. `measurement`
  // reuses its `reported_by_id` (the logging user) as its owner.
  function rowOwnerId(table, row){ return table === 'measurement' ? row.reported_by_id : row.created_by; }
  function ownsRow(table, row){
    if (!CURRENT_USER) return false;
    if (isAdmin()) return true;                 // Admins: universal edit/delete
    if (!row) return false;
    var o = rowOwnerId(table, row);
    return o != null && +o === +CURRENT_USER.id;
  }
  function canReport(r){
    ...
    // a KPI has no country: the country is the transaction's (its project's, or
    // where an unattributed activity happened)
    return !!(r && r.iso && userCountryIsos(CURRENT_USER).indexOf(r.iso) >= 0);
  }
  ...
  function loginAs(u){
    CURRENT_USER = u; DB.actingUserId = u ? u.id : null; saveSession(u.id);
  ...
  function doLogout(){ closeUserMenu(); CURRENT_USER = null; DB.actingUserId = null; clearSession(); showLogin(); }
  ...
  // NOTE ON window.SEED: the demo seed is loaded ONLY in browser-store mode (see
  // loadSeed in js/db.js). In LOCAL mode it is absent and every reconcile below
  // is a no-op - correctly, because there the SQLite database is the authority
  // and server.py seeded it.
  //
  ...
  function reconcileLookups() {
    var jobs = [];
    // A cached pre-lookup store has empty reference tables - fill them from the
    // seed. WHICH tables those are is derived from the schema (a reference list
    // is a table carrying only id/key/name/seq/lead_id), never listed here, so a
    // lookup added to schema.sql is reconciled without touching this function.
    var LK = Object.keys((DB.model && DB.model.tables) || {}).filter(function (t) {
      return DB.model.tables[t].lookup && DB.tables[t] && window.SEED && window.SEED[t];
    });
  ...
  // Remove dependants whose parent is already gone. Older builds deleted
  // activities without their beneficiary rows and outputs without their project
  // links; in the browser store (no cascades) those rows linger and attach
  // themselves to the next record that reuses the freed id. Idempotent.
  function reconcileOrphans() {
    var idx = DB._idx, jobs = [];
    function sweep(table, orphan) {
      var ids = (DB.tables[table] || []).filter(orphan).map(function (r) { return r.id; });
      if (ids.length) jobs.push(DB.remove(table, ids));
    }
    sweep('beneficiary', function (b) { return !idx.measById[b.measurement_id]; });
    sweep('project_kpi', function (pk) { return !idx.indicatorById[pk.indicator_id] || !idx.projectById[pk.project_id]; });
    var orphanEv = (DB.tables.evidence || []).filter(function (e) { return !idx.indicatorById[e.indicator_id] || !idx.projectById[e.project_id]; });
    if (orphanEv.length) jobs.push(evRemove(orphanEv));
    return Promise.all(jobs);
  }

  // ROW-LEVEL OWNERSHIP backfill: stamp `created_by` on any legacy/seeded row that
  // predates the ownership feature. Idempotent - only touches rows where created_by
  // is still null, so it re-runs harmlessly (and self-heals after a reseed). Owners:
  //   · project           -> the LEAD of its country (country.lead_id), so country
  //                          offices keep control of their existing data; else Admin
  //   · project_kpi, secondary KPI -> the owning project's owner
  //   · framework KPI, result       -> the result's accountable Lead (owner_id); else Admin
  //   · beneficiary       -> its parent activity's author (measurement.reported_by_id)
  //   · donor/partner/plan/beneficiary_type/report -> the Admin owner
  //   · measurement is skipped - its `reported_by_id` already carries the author.
  function backfillCreatedBy() {
    var admin = (DB.tables.user || []).filter(function (u) { return userStatus(u) === 'admin'; })[0];
    var adminId = admin ? admin.id : 1;
    function pending(table) { return (DB.tables[table] || []).filter(function (r) { return r.created_by == null; }); }
    var jobs = [];
    function stamp(table, ownerFn) {
      var rows = pending(table);
      if (!rows.length) return;
      rows.forEach(function (r) { var o = ownerFn(r); r.created_by = (o != null ? o : adminId); });
      jobs.push(DB.persist(table, rows));
    }
    // projects first so downstream rows can read the resolved project owner
    stamp('project', function (p) { var co = DB._idx.countryByIso[p.country_iso3]; return co ? co.lead_id : null; });
    var projOwner = function (pid) { var p = DB._idx.projectById[pid]; return p ? p.created_by : null; };
    stamp('project_kpi', function (pk) { return projOwner(pk.project_id); });
    stamp('indicator', function (ind) {
      if (ind.secondary) return projOwner(ind.project_id);
      var res = DB._idx.resultById[ind.result_id]; return res ? res.owner_id : null;
    });
    stamp('result', function (res) { return res.owner_id; });
    stamp('beneficiary', function (b) { var m = DB._idx.measById[b.measurement_id]; return m ? m.reported_by_id : null; });
    ['donor', 'partner', 'plan', 'beneficiary_type', 'report'].forEach(function (t) { stamp(t, function () { return adminId; }); });
    return Promise.all(jobs);
  }
  ...
  DB.init().then(function () {
    ...
    return reconcileOrphans();       // drop dependants whose parent row is gone
  }).then(function () {
    return backfillCreatedBy();      // stamp row-level ownership onto legacy/seeded rows
  }).then(function () {
    return I18N.reconcile();         // seed the language catalogue onto a store that predates it
  }).then(function () {
    enrich();
    drawBase();
    initMapInteractions();
    ...
    renderTicker();
    renderAll();
    // Globalization: the whole interface renders in English and is then spoken
    // in the active language. Booted LAST, so the first pass covers a fully
    // wired and rendered page (including the login gate).
    I18N.onChange = function () { renderLangChip(); renderTicker(); renderAll(); };
    I18N.boot();
    renderLangChip();
  ...
  function pdfWriter(opts){
    ...
    function enc(s){
      ...
      for (var i = 0; i < s.length; i++){
        ...
        else if (c === 0x203A) o = 155;                    // single right guillemet › (WinAnsi 0x9B)
        else if (c === 0x2039) o = 139;                    // single left guillemet ‹ (WinAnsi 0x8B)
    ...
    function cw(code, bold){
      ...
      if (code === 155 || code === 139) return 333;   // guillemets
      return bold ? 611 : 556;
    }
  ...
  // =========================================================================
  //  EVALUATION - the evidence behind a project's achieved results.
  //
  //  Project modal → Evaluation tab lists every result (KPI) the project reports
  //  against, grouped down the results chain (Impact → Outcome → Output), each
  //  with what it achieved, its performance and how many evidence items back it.
  //  📎 Evidence opens the manager for that one result: upload any number of
  //  files, add video / voice links, open, edit and delete them. The footer's
  //  Evaluation Report compiles the results and all of the evidence into ONE PDF.
  //
  //  File bytes live in COLD storage (`evidence_file`, see DB.putCold in
  //  js/db.js); the `evidence` row carries everything the screens read, so the
  //  files load only when one is opened or a report is compiled.
  // =========================================================================
  var EV_MAX_BYTES = 25 * 1024 * 1024;   // per file; the store holds base64, a third larger again
  var EV_ICON = { document: '📄', image: '🖼', video: '🎬', audio: '🎙', link: '🔗' };

  /** Evidence of a project, optionally for one KPI, in upload order. */
  function evRows(pid, iid){
    return (DB.tables.evidence || []).filter(function (e){
      return e.project_id === pid && (iid == null || e.indicator_id === iid);
    }).sort(function (a, b){ return a.id - b.id; });
  }
  function evCountsByKpi(pid){
    var m = {};
    (DB.tables.evidence || []).forEach(function (e){ if (e.project_id === pid) m[e.indicator_id] = (m[e.indicator_id] || 0) + 1; });
    return m;
  }
  function evTypeKey(e){ var r = lkRow('evidence_type', e && e.type_id); return r ? r.key : (e && e.file_name ? 'document' : 'link'); }
  function evTypeName(e){ var r = lkRow('evidence_type', e && e.type_id); return r ? r.name : ''; }
  function evIsFile(e){ return !!(e && e.file_name); }
  /** The evidence kind a file most likely is, from its MIME type or extension. */
  function evGuessTypeKey(mime, name){
    mime = String(mime || '').toLowerCase();
    var ext = String(name || '').toLowerCase().split('.').pop();
    if (/^image\//.test(mime) || /^(jpe?g|png|gif|webp|bmp|heic|heif|tiff?)$/.test(ext)) return 'image';
    if (/^video\//.test(mime) || /^(mp4|mov|avi|mkv|webm|m4v|3gp|wmv)$/.test(ext)) return 'video';
    if (/^audio\//.test(mime) || /^(mp3|wav|m4a|aac|ogg|oga|opus|amr|flac|wma)$/.test(ext)) return 'audio';
    return 'document';
  }
  function evGuessLinkKey(url){
    if (/youtu\.?be|vimeo\.com|tiktok\.com|\/video|\.(mp4|mov|webm|m4v)(\?|$)/i.test(url)) return 'video';
    if (/soundcloud\.com|anchor\.fm|spotify\.com|\/audio|voice|\.(mp3|m4a|wav|ogg|opus)(\?|$)/i.test(url)) return 'audio';
    return 'link';
  }
  function evSize(n){
    if (n == null) return '';
  ...
  function evTitleFromFile(name){ return String(name || '').replace(/\.[^.]+$/, '').replace(/[_]+/g, ' ').trim() || 'Evidence'; }
  function evReadBase64(file){
  ...
  function evB64ToBytes(b64){
    var bin = atob(b64 || ''), out = new Uint8Array(bin.length);
    for (var i = 0; i < bin.length; i++) out[i] = bin.charCodeAt(i);
    return out;
  }
  /** Run an async step over a list one item at a time. */
  function evSerial(items, step){
    return items.reduce(function (chain, item, i){ return chain.then(function (){ return step(item, i); }); }, Promise.resolve());
  }
  // Uploading follows the activity-logging gate (role + country scope); editing
  // and deleting an item follow row-level ownership (its uploader, or an Admin).
  function canAddEvidence(p){ return canReportProject(p); }

  /** Remove evidence rows and their files. Deleting a project's KPI, output or
   *  plan calls this too: SQLite cascades on disk, but the in-memory mirror and
   *  the browser store do not, so the dependants are removed explicitly - the
   *  same way activities are. */
  function evRemove(rows){
    var ids = (rows || []).filter(Boolean).map(function (e){ return e.id; });
    if (!ids.length) return Promise.resolve();
    return Promise.all([ DB.removeCold('evidence_file', ids), DB.remove('evidence', ids) ]);
  }
  function evRemoveWhere(pred){ return evRemove((DB.tables.evidence || []).filter(pred)); }

  /** Open an evidence item: a link in a new tab; a file from cold storage -
   *  shown in a tab when the browser can display it, downloaded otherwise. */
  function evOpen(e){
    if (!e) return;
    if (!evIsFile(e)){ if (e.url) window.open(e.url, '_blank', 'noopener'); return; }
    var mime = e.mime_type || 'application/octet-stream';
    // Only content that cannot run script opens in a tab. A blob URL shares this
    // page's origin, so an uploaded HTML or SVG file would run as the app.
    var viewable = (/^(image|video|audio)\//.test(mime) && !/svg/i.test(mime)) || mime === 'application/pdf' || mime === 'text/plain';
    var tab = viewable ? window.open('', '_blank') : null;   // opened in the click, so it is not blocked
    DB.getCold('evidence_file', e.id).then(function (f){
      if (!f){ if (tab) tab.close(); alert('The file behind this evidence is missing.'); return; }
      var url = URL.createObjectURL(new Blob([evB64ToBytes(f.data)], { type: mime }));
      if (tab) tab.location.href = url;
      else {
        var a = document.createElement('a'); a.href = url; a.download = e.file_name || 'evidence';
        document.body.appendChild(a); a.click(); a.remove();
      }
      setTimeout(function (){ URL.revokeObjectURL(url); }, 120000);
    }).catch(function (err){ if (tab) tab.close(); alert('Could not open the file: ' + (err && err.message || err)); });
  }

  /** A project's results, grouped down the results chain for evaluation:
   *  [{ code, name, color, outcomes: [{ code, name, outputs: [{ code, name, kpis }] }] }]
   *  Framework KPIs sit under their Impact → Outcome → Output; a project's own
   *  secondary KPIs form one closing group. Shared by the tab and the report. */
  function evalGroups(P){
    function nat(a, b){ return String(a || '').localeCompare(String(b || ''), 'en', { numeric: true }); }
    function vals(o){ return Object.keys(o).map(function (k){ return o[k]; }); }
    function byCode(a, b){ return nat(a.raw.code, b.raw.code) || nat(a.name, b.name); }
    var imps = {};
    P.primary.forEach(function (r){
      var im = null, oc = null, op = null;
      resultChain(r.result).forEach(function (n){
        if (n.level === 'impact') im = n; else if (n.level === 'outcome') oc = n; else if (n.level === 'output') op = n;
      });
      var sdg = r.sdg || (im && im.sdg) || 0;
      var I = imps[sdg] || (imps[sdg] = {
        sdg: sdg, code: sdg ? pillarLabel(sdg) : '', color: sdg ? PILLAR_COLORS[sdg] : '#94a3b8',
        name: sdg ? (PILLAR_NAMES[sdg] || (im && im.statement) || '') : 'Unaligned results', outcomes: {}
      });
      var ok = oc ? (oc.code || oc.statement) : '';
      var O = I.outcomes[ok] || (I.outcomes[ok] = { code: oc ? (oc.code || '') : '', name: oc ? oc.statement : '', outputs: {} });
      var pk = op ? (op.code || op.statement) : '';
      var Q = O.outputs[pk] || (O.outputs[pk] = { code: op ? (op.code || '') : '', name: op ? op.statement : 'Results', kpis: [] });
      Q.kpis.push(r);
    });
    var groups = vals(imps).sort(function (a, b){ return (a.sdg || 99) - (b.sdg || 99); }).map(function (I){
      I.outcomes = vals(I.outcomes).sort(function (a, b){ return nat(a.code, b.code); }).map(function (O){
        O.outputs = vals(O.outputs).sort(function (a, b){ return nat(a.code, b.code); });
        O.outputs.forEach(function (Q){ Q.kpis.sort(byCode); });
        return O;
      });
      return I;
    });
    if (P.secondary.length){
      groups.push({ secondary: true, code: 'Secondary KPIs', name: 'Project-specific results', color: '#27500a',
        outcomes: [{ code: '', name: '', outputs: [{ code: '', name: 'Project-specific results', kpis: P.secondary.slice().sort(byCode) }] }] });
    }
    return groups;
  }
  /** Achieved / target / progress / performance of one KPI, as display text. */
  function evMetrics(r){
    var suf = r.unit === '%' ? '%' : '', st = STATUS[r.status] || STATUS.nodata;
    return {
      achieved: r.value != null ? fmtNum(r.value) + suf : '–',
      target: r.raw.target_value != null ? fmtNum(r.raw.target_value) + suf : '–',
      progress: r.progress != null ? Math.round(r.progress * 100) + '%' : '–',
      performance: r.performance != null ? Math.round(r.performance * 100) + '%' : '–',
      status: st
    };
  }

  // ---- TAB 5 · evaluation ----------------------------------------------------
  function projectEvaluationTab(){
    var p = curProject, P = PROJECTSBYID[p.id], box = el('div', 'prtab ev-tab');
    if (!P){
      box.appendChild(el('div', 'cp-note', 'This project is not in the plan in view - switch to its plan to evaluate it.'));
      return box;
    }
    var counts = evCountsByKpi(p.id), N = P.kpis.length;
    var covered = P.kpis.filter(function (r){ return counts[r.kpiId]; }).length;
    var items = evRows(p.id), nFiles = items.filter(evIsFile).length;
    var st = P.statAll || {}, S0 = STATUS[st.code || 'nodata'] || STATUS.nodata;

    var head = el('div', 'pr-acthead');
    head.appendChild(el('div', 'sec-h2', fmt(N) + ' result' + (N === 1 ? '' : 's') + ' · ' + fmt(items.length) + ' evidence item' + (items.length === 1 ? '' : 's')));
    box.appendChild(head);
    box.appendChild(elHTML('div', 'cp-note', 'Every result this project reports against, grouped down the results chain. Use <b>📎 Evidence</b> to upload what proves a result - documents, photos, video and voice recordings, or links to them. <b>Evaluation Report</b> (below) compiles the results and all of their evidence into one PDF.'));

    var stats = el('div', 'ev-stats');
    [
      { l: 'Results', v: fmt(N), s: fmt(P.primary.length) + ' primary · ' + fmt(P.secondary.length) + ' secondary' },
      { l: 'With evidence', v: fmt(covered) + ' of ' + fmt(N), s: (N ? Math.round(covered / N * 100) : 0) + '% of results evidenced' },
      { l: 'Evidence items', v: fmt(items.length), s: fmt(nFiles) + ' file' + (nFiles === 1 ? '' : 's') + ' · ' + fmt(items.length - nFiles) + ' link' + (items.length - nFiles === 1 ? '' : 's') },
      { l: 'Performance', v: st.ratio != null ? Math.round(st.ratio * 100) + '%' : '–', s: S0.label, c: S0.c }
    ].forEach(function (s){
      var c = el('div', 'ev-stat');
      c.appendChild(el('div', 'ev-stat-l', s.l));
      c.appendChild(el('div', 'ev-stat-v', s.v));
      var sub = el('div', 'ev-stat-s', s.s);
      if (s.c){ sub.classList.add('ev-st'); sub.style.setProperty('--st', s.c); }
      c.appendChild(sub);
      stats.appendChild(c);
    });
    box.appendChild(stats);
    ...
    if (!N){
      box.appendChild(el('div', 'empty', 'This project reports against no KPIs yet - attach primary or secondary KPIs first, then evidence can be uploaded against their results.'));
      return box;
    }

    var thead = '<thead><tr><th>Result</th><th class="num">Achieved</th><th class="num">Target</th><th class="num">Progress</th><th class="tcol">Performance</th><th class="ev-evh">Evidence</th></tr></thead>';
    function rowHTML(r){
      var m = evMetrics(r), n = counts[r.kpiId] || 0;
      return '<tr data-ind="' + r.id + '">'
        + '<td class="esum-nm">' + kpiLabelHTML(r) + '</td>'
        + '<td class="num">' + esc(m.achieved) + '</td>'
        + '<td class="num">' + esc(m.target) + '</td>'
        + '<td class="num">' + esc(m.progress) + '</td>'
        + '<td class="tcol"><span class="ev-st" style="--st:' + m.status.c + '">' + esc(m.status.label) + '</span>'
        +   (r.performance != null ? ' <span class="ev-perf">' + esc(m.performance) + '</span>' : '') + '</td>'
        + '<td class="ev-evcell"><span class="ev-n' + (n ? '' : ' zero') + '" title="' + n + ' evidence item' + (n === 1 ? '' : 's') + ' uploaded">' + n + '</span>'
        +   '<button class="hbtn ev-btn" type="button">📎 Evidence</button></td>'
        + '</tr>';
    }
    var html = '';
    evalGroups(P).forEach(function (I){
      html += '<section class="ev-impact" style="--grp:' + (I.color || '#94a3b8') + '">'
        + '<div class="ev-impact-hd">'
        + (I.code ? '<span class="esum-grp-code">' + esc(I.code) + '</span>' : '')
        + '<span class="esum-grp-t">' + esc(I.name) + '</span></div>';
      I.outcomes.forEach(function (O){
        if (O.code || O.name){
          html += '<div class="ev-outcome-hd">' + (O.code ? '<span class="esum-grp-code">' + esc(O.code) + '</span>' : '')
            + '<span class="esum-grp-t">' + esc(O.name) + '</span></div>';
        }
        O.outputs.forEach(function (Q){
          html += '<div class="esum-group ev-output">'
            + '<div class="esum-group-hd"><span class="esum-grp-bar" style="background:' + (I.color || '#94a3b8') + '"></span>'
            + (Q.code ? '<span class="esum-grp-code">' + esc(Q.code) + '</span>' : '')
            + '<span class="esum-grp-t">' + esc(Q.name) + '</span>'
            + '<span class="esum-grp-n">' + Q.kpis.length + ' result' + (Q.kpis.length === 1 ? '' : 's') + '</span></div>'
            + '<div class="esum-card ev-card"><table class="esum-tbl esum-static ev-tbl">' + thead + '<tbody>'
            + Q.kpis.map(rowHTML).join('') + '</tbody></table></div></div>';
        });
      });
      html += '</section>';
    });
    var list = elHTML('div', 'ev-groups', html);
    list.querySelectorAll('tr[data-ind]').forEach(function (tr){
      var r = INDBYID[+tr.getAttribute('data-ind')];
      tr.querySelector('.ev-btn').onclick = function (){ if (r) openEvidence(r); };
    });
    box.appendChild(list);
    return box;
  }
  /** Re-render the Evaluation tab in place (counts changed), keeping the scroll. */
  function refreshEvalTab(){
    var on = $('#prTabs .on');
    if (!curProject || !on || on.dataset.prtab !== 'evaluation') return;
    var body = $('#prBody'), top = body.scrollTop;
    setProjectTab('evaluation');
    body.scrollTop = top;
  }

  // ---- evidence manager (one result) ---------------------------------------
  var _evKpi = null, _evMsg = null;
  function openEvidence(r){
    _evKpi = r; _evMsg = null;
    $('#evTitle').textContent = 'Evidence · ' + kpiLabelText(r);
    renderEvidence();
    $('#evOverlay').classList.add('on');
  }
  function closeEvidence(){
    $('#evOverlay').classList.remove('on');
    _evKpi = null;
    refreshEvalTab();
  }
  function renderEvidence(){
    var r = _evKpi, p = curProject; if (!r || !p) return;
    var body = $('#evBody'); body.innerHTML = '';
    var can = canAddEvidence(p), m = evMetrics(r), rows = evRows(p.id, r.kpiId);

    body.appendChild(elHTML('div', 'ev-ctx',
      '<span class="ev-st" style="--st:' + m.status.c + '">' + esc(m.status.label) + '</span>'
      + '<span>Achieved <b>' + esc(m.achieved) + '</b> of <b>' + esc(m.target) + '</b></span>'
      + '<span>Progress <b>' + esc(m.progress) + '</b></span>'
      + '<span>Performance <b>' + esc(m.performance) + '</b></span>'));

    if (can){
      var zone = el('label', 'ev-drop');
      zone.innerHTML = '<input type="file" multiple hidden>'
        + '<span class="ev-drop-ic">⭱</span>'
        + '<span class="ev-drop-t"><b>Drop files here</b> or click to choose</span>'
        + '<span class="ev-drop-s">Documents, photos, video and voice recordings · as many files as you need · up to 25 MB each</span>';
      var inp = zone.querySelector('input');
      inp.onchange = function (){ var fs = Array.prototype.slice.call(inp.files || []); inp.value = ''; evUpload(fs); };
      zone.addEventListener('dragover', function (e){ e.preventDefault(); zone.classList.add('over'); });
      zone.addEventListener('dragleave', function (){ zone.classList.remove('over'); });
      zone.addEventListener('drop', function (e){
        e.preventDefault(); zone.classList.remove('over');
        evUpload(Array.prototype.slice.call((e.dataTransfer && e.dataTransfer.files) || []));
      });
      body.appendChild(zone);
      var bar = el('div', 'ev-bar');
      var addLink = el('button', 'hbtn', '🔗 Add video / voice link'); addLink.type = 'button';
      addLink.onclick = function (){ openEvidenceItem(null); };
      bar.appendChild(addLink);
      var msg = el('span', 'ev-prog' + (_evMsg && _evMsg.ok ? ' ok' : ''), _evMsg ? _evMsg.text : '');
      bar.appendChild(msg);
      body.appendChild(bar);
    } else {
      body.appendChild(el('div', 'cp-note', 'Read-only - you can open this evidence, but uploading is limited to users who report on this project.'));
    }

    var wrap = el('div', 'ev-listwrap');
    var tbl = el('table', 'utbl ev-list');
    tbl.innerHTML = '<thead><tr><th>#</th><th>Evidence</th><th>Date</th><th>File / link</th><th>Uploaded by</th><th></th></tr></thead>';
    var tb = el('tbody');
    if (!rows.length) tb.innerHTML = '<tr><td colspan="6" class="re-empty">No evidence uploaded for this result yet.</td></tr>';
    rows.forEach(function (e, i){
      var tr = el('tr'), key = evTypeKey(e);
      tr.innerHTML = '<td class="umono">' + (i + 1) + '</td>'
        + '<td><div class="ev-item"><span class="ev-ic" title="' + esc(evTypeName(e)) + '">' + (EV_ICON[key] || '📄') + '</span><span class="ev-item-t"><span class="ev-item-n">' + esc(e.name) + '</span>'
        +   '<span class="ev-item-k">' + esc(evTypeName(e)) + '</span>'
        +   (e.description ? '<span class="ev-item-d">' + esc(e.description) + '</span>' : '') + '</span></div></td>'
        + '<td class="umono ev-date">' + esc(e.evidence_date ? shortDate(e.evidence_date) : '–') + '</td>'
        + (evIsFile(e)
            ? '<td data-notr><div class="ev-file"><span class="ev-fname" title="' + esc(e.file_name) + '">' + esc(e.file_name) + '</span><span class="ev-fsize">' + esc(evSize(e.size_bytes)) + '</span></div></td>'
            : '<td data-notr><div class="ev-file"><a href="' + esc(e.url || '#') + '" target="_blank" rel="noopener" title="' + esc(e.url || '') + '">' + esc(e.url || '') + '</a></div></td>')
        + '<td>' + esc(userName(e.created_by)) + '</td>';
      var act = el('td', 'uact');
      var op = el('button', 'cp-mini', 'Open'); op.type = 'button'; op.onclick = function (){ evOpen(e); };
      act.appendChild(op);
      if (ownsRow('evidence', e)){
        var ed = el('button', 'cp-mini', 'Edit'); ed.type = 'button'; ed.onclick = function (){ openEvidenceItem(e); };
        var del = el('button', 'cp-del', '🗑'); del.type = 'button'; del.title = 'Delete this evidence';
        del.onclick = function (){
          if (!ownsRow('evidence', e)) return;
          if (!confirm('Delete "' + e.name + '"? The uploaded file is removed too. This cannot be undone.')) return;
          evRemove([e]).then(function (){ _evMsg = { text: 'Deleted.', ok: true }; renderEvidence(); });
        };
        act.appendChild(ed); act.appendChild(del);
      }
      tr.appendChild(act); tb.appendChild(tr);
  ...
  function evSetMsg(text, ok){
    _evMsg = { text: text, ok: !!ok };
    var m = $('#evBody .ev-prog'); if (m){ m.textContent = text; m.classList.toggle('ok', !!ok); }
  }
  /** Upload files as evidence of the open result, one at a time (each can be
   *  large): the metadata row first, then its bytes into cold storage. A file
   *  whose bytes fail to store takes its row back out with it. */
  function evUpload(files){
    var r = _evKpi, p = curProject;
    if (!r || !p || !files.length || !canAddEvidence(p)) return;
    var ok = 0, bad = [];
    evSerial(files, function (file, i){
      if (!file.size){ bad.push(file.name + ' (empty)'); return null; }
      if (file.size > EV_MAX_BYTES){ bad.push(file.name + ' (over 25 MB)'); return null; }
      evSetMsg('Uploading ' + (i + 1) + ' of ' + files.length + ' · ' + file.name + '…');
      var row = null;
      return evReadBase64(file).then(function (b64){
        var fields = {
          project_id: p.id, indicator_id: r.kpiId,
          type_id: lkIdByKey('evidence_type', evGuessTypeKey(file.type, file.name)),
          name: evTitleFromFile(file.name), description: '',
          evidence_date: file.lastModified ? new Date(file.lastModified).toISOString().slice(0, 10) : TODAY.toISOString().slice(0, 10),
          file_name: file.name, size_bytes: file.size, uploaded: new Date().toISOString()
        };
        if (file.type) fields.mime_type = file.type;
        return DB.insert('evidence', fields).then(function (rows){
          row = rows[0];
          return DB.putCold('evidence_file', { evidence_id: row.id, data: b64 });
        });
      }).then(function (){ ok++; }, function (err){
        bad.push(file.name + ' (' + (err && err.message || 'failed') + ')');
        if (row && DB._idx && (DB.tables.evidence || []).indexOf(row) >= 0) return DB.remove('evidence', [row.id]).catch(function (){});
      });
    }).then(function (){
      _evMsg = bad.length
        ? { text: (ok ? fmt(ok) + ' uploaded · ' : '') + 'not uploaded: ' + bad.join(', '), ok: false }
        : { text: fmt(ok) + ' file' + (ok === 1 ? '' : 's') + ' uploaded.', ok: true };
      if (_evKpi === r) renderEvidence();
    });
  }

  // ---- evidence item add / edit (links, and the details of any item) -------
  function openEvidenceItem(e){
    var r = _evKpi, p = curProject; if (!r || !p) return;
    if (e && !ownsRow('evidence', e)) return;
    if (!e && !canAddEvidence(p)) return;
    var isFile = evIsFile(e);
    var curType = e ? e.type_id : lkIdByKey('evidence_type', 'video');
    $('#evItemTitle').textContent = e ? ('Edit evidence · ' + e.name) : '🔗 Add a video / voice link';
    var body = $('#evItemBody'); body.innerHTML = '';
    var f = el('div', 'uform');
    f.innerHTML =
      '<div class="ufgrid kpigrid">' +
      (isFile ? '' :
      '  <label class="kf-wide"><span>Link *</span><input class="evf-url" type="url" data-notr value="' + esc(e ? (e.url || '') : '') + '" placeholder="https://…"></label>') +
      '  <label class="kf-wide"><span>Title *</span><input class="evf-name" type="text" value="' + esc(e ? e.name : '') + '" placeholder="What this evidence is"></label>' +
      '  <label><span>Type</span><select class="evf-type">' + lookupOptions('evidence_type', curType) + '</select></label>' +
      '  <label><span>Date</span><input class="evf-date" type="date" value="' + esc(e ? (e.evidence_date || '') : TODAY.toISOString().slice(0, 10)) + '"></label>' +
      '  <label class="kf-wide"><span>Description</span><textarea class="evf-desc" rows="3" placeholder="What it shows, where and when it was captured">' + esc(e ? (e.description || '') : '') + '</textarea></label>' +
      (isFile ?
      '  <label class="kf-wide"><span>File</span><span class="ev-curfile" data-notr>' + esc(e.file_name) + ' · ' + esc(evSize(e.size_bytes)) + '</span>'
      + '<input class="evf-file" type="file"><small class="ev-hint">Choose a file only to replace the current one.</small></label>' : '') +
      '</div>' +
      '<div class="ufbtns"><span class="ufmsg"></span>' +
        '<button class="hbtn evf-cancel" type="button">Cancel</button>' +
        '<button class="hbtn primary evf-save" type="button">' + (e ? 'Save changes' : 'Add link') + '</button></div>';
    var urlIn = f.querySelector('.evf-url'), typeIn = f.querySelector('.evf-type');
    // a pasted link suggests its own kind (video / voice) until the user picks one
    var typeTouched = !!e;
    if (typeIn) typeIn.addEventListener('change', function (){ typeTouched = true; });
    if (urlIn) urlIn.addEventListener('input', function (){
      if (!typeTouched) typeIn.value = String(lkIdByKey('evidence_type', evGuessLinkKey(urlIn.value)));
    });
    f.querySelector('.evf-cancel').onclick = closeEvidenceItem;
    f.querySelector('.evf-save').onclick = function (){ saveEvidenceItem(e, f); };
    body.appendChild(f);
    $('#evItemOverlay').classList.add('on');
    trField($('#evItemOverlay'));
    var first = urlIn || f.querySelector('.evf-name'); if (first) first.focus();
  }
  function closeEvidenceItem(){ var o = $('#evItemOverlay'); if (o) o.classList.remove('on'); }
  function saveEvidenceItem(e, f){
    var r = _evKpi, p = curProject; if (!r || !p) return;
    var v = function (c){ return fv(f.querySelector(c)); };
    var msg = f.querySelector('.ufmsg');
    var name = v('.evf-name').trim(), url = f.querySelector('.evf-url') ? f.querySelector('.evf-url').value.trim() : null;
    if (e && !ownsRow('evidence', e)){ msg.textContent = 'Only the uploader can edit this evidence.'; return; }
    if (url !== null){
      if (!url){ msg.textContent = 'Paste the link to the video or recording.'; return; }
      if (!/^https?:\/\/\S+$/i.test(url)){ msg.textContent = 'The link must start with http:// or https://'; return; }
    }
    if (!name){
      if (url){ try { name = new URL(url).hostname.replace(/^www\./, '') + ' link'; } catch (x){ name = 'Link'; } }
      else { msg.textContent = 'A title is required.'; return; }
    }
    var fields = {
      name: name, type_id: v('.evf-type') ? +v('.evf-type') : null,
      evidence_date: v('.evf-date') || null, description: v('.evf-desc').trim()
    };
    if (url !== null) fields.url = url;
    var fileIn = f.querySelector('.evf-file'), file = fileIn && fileIn.files && fileIn.files[0];
    if (file && file.size > EV_MAX_BYTES){ msg.textContent = 'That file is over 25 MB.'; return; }
    msg.textContent = 'Saving…';
    var job;
    if (e){
      Object.keys(fields).forEach(function (k){ e[k] = fields[k]; });
      job = file
        ? evReadBase64(file).then(function (b64){
            e.file_name = file.name; e.size_bytes = file.size; e.uploaded = new Date().toISOString();
            if (file.type) e.mime_type = file.type; else delete e.mime_type;
            return DB.putCold('evidence_file', { evidence_id: e.id, data: b64 });
          }).then(function (){ return DB.persist('evidence', [e]); })
        : DB.persist('evidence', [e]);
    } else {
      fields.project_id = p.id; fields.indicator_id = r.kpiId; fields.uploaded = new Date().toISOString();
      job = DB.insert('evidence', fields);
    }
    Promise.resolve(job).then(function (){
      closeEvidenceItem();
      _evMsg = { text: e ? 'Saved.' : 'Link added.', ok: true };
      renderEvidence();
    }, function (err){ msg.textContent = 'Could not save: ' + (err && err.message || err); });
  }

  // =========================================================================
  //  EVALUATION REPORT PDF - one document holding a project's results and every
  //  piece of evidence behind them. Three stages, deliberately separate:
  //
  //    1. COLLECT  (evalCollect) - the evaluation as plain data: the project,
  //       its results grouped down the chain, each result's evidence with the
  //       file bytes loaded.
  //    2. RENDER   (evalRenderBase) - the report pages on the zero-dependency
  //       pdfWriter: cover, results index, one evidence sheet per result
  //       (QR codes for video / voice links drawn as vectors).
  //    3. ASSEMBLE (evalAssemble) - pdf-lib merges it all into ONE file: each
  //       result's photos and PDF pages follow its sheet, files that cannot be
  //       printed (Word, Excel, video, audio…) are embedded as attachments.
  //
  //  FUTURE - AI synthesis slots in between 1 and 2: a stage that reads the
  //  collected model (evidence bytes included), synthesises across every item
  //  of a result and of the whole project, and writes `k.synthesis` per result
  //  and `model.synthesis` for the project. The renderer already prints both
  //  when present; nothing calls a model today.
  //
  //  pdf-lib and qrcode-generator are loaded from the CDN on first use only.
  // =========================================================================
  var EV_LIBS = {
    pdflib: { url: 'https://unpkg.com/pdf-lib@1.17.1/dist/pdf-lib.min.js', global: 'PDFLib' },
    qrcode: { url: 'https://unpkg.com/qrcode-generator@1.4.4/qrcode.js', global: 'qrcode' }
  };
  var _evLibLoads = {};
  function evLoadLib(key){
    var lib = EV_LIBS[key];
    if (window[lib.global]) return Promise.resolve(window[lib.global]);
    if (_evLibLoads[key]) return _evLibLoads[key];
    _evLibLoads[key] = new Promise(function (resolve, reject){
      var s = document.createElement('script');
      s.src = lib.url; s.async = true;
      s.onload = function (){ window[lib.global] ? resolve(window[lib.global]) : reject(new Error(key + ' did not initialise')); };
      s.onerror = function (){ delete _evLibLoads[key]; s.remove(); reject(new Error('could not load the PDF tools - check the internet connection')); };
      document.head.appendChild(s);
    });
    return _evLibLoads[key];
  }

  function exportEvaluationReport(pr){
    if (!pr || pr.id == null){ alert('Save the project details first, then build its Evaluation Report.'); return; }
    if (!PROJECTSBYID[pr.id]){ alert('This project is not in the active plan - open it in its own plan to build the report.'); return; }
    var btn = $('#prEvalPdf'), msg = $('#prMsg'), label = btn ? btn.textContent : '';
    function status(t, ok){ if (msg){ msg.textContent = t; msg.classList.toggle('ok', !!ok); } }
    if (btn){ btn.disabled = true; btn.textContent = 'Compiling…'; }
    status('Loading the PDF tools…');
    var libs;
    Promise.all([evLoadLib('pdflib'), evLoadLib('qrcode')]).then(function (l){
      libs = l;
      status('Collecting evidence…');
      return evalCollect(pr);
    }).then(function (model){
      status('Preparing ' + fmt(model.evidenceN) + ' evidence item' + (model.evidenceN === 1 ? '' : 's') + '…');
      return evalPrepare(model, libs[0]).then(function (){ return model; });
    }).then(function (model){
      status('Compiling the report…');
      var base = evalRenderBase(model);
      return evalAssemble(model, base, libs[0]).then(function (bytes){
        var blob = new Blob([bytes], { type: 'application/pdf' });
        var a = document.createElement('a');
        a.href = URL.createObjectURL(blob); a.download = evalFileName(model);
        document.body.appendChild(a); a.click();
        setTimeout(function (){ URL.revokeObjectURL(a.href); a.remove(); }, 1500);
        status('Evaluation Report ready · ' + fmt(model.outPages) + ' pages.', true);
      });
    }).catch(function (err){
      console.error(err);
      status('Could not build the Evaluation Report: ' + (err && err.message || err));
    }).then(function (){
      if (btn){ btn.disabled = false; btn.textContent = label; }
    });
  }

  function evalFileName(model){
    var pr = model.project;
    var base = ((pr.code ? pr.code + '-' : '') + (model.P.name || 'project')).toLowerCase()
      .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60);
    var brand = (orgActive() ? (orgShort() || 'grassroots') : 'grassroots')
      .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 30) || 'grassroots';
    return brand + '-evaluation-report-' + (base || 'project') + '-' + TODAY.toISOString().slice(0, 10) + '.pdf';
  }

  /** Stage 1 · COLLECT. */
  function evalCollect(pr){
    var P = PROJECTSBYID[pr.id], groups = evalGroups(P), kpis = [], n = 0;
    groups.forEach(function (I){ I.outcomes.forEach(function (O){ O.outputs.forEach(function (Q){ Q.kpis.forEach(function (r){
      kpis.push({ r: r, impact: I, outcome: O, output: Q, synthesis: null,
        evidence: evRows(pr.id, r.kpiId).map(function (e){ return { row: e, n: ++n }; }) });
    }); }); }); });
    var model = { project: pr, P: P, groups: groups, kpis: kpis, evidenceN: n, synthesis: null };
    var files = [];
    kpis.forEach(function (k){ k.evidence.forEach(function (ev){ if (evIsFile(ev.row)) files.push(ev); }); });
    // serially: evidence files can be large, so only one decode is in flight
    return evSerial(files, function (ev){
      return DB.getCold('evidence_file', ev.row.id).then(function (f){ ev.bytes = f && f.data ? evB64ToBytes(f.data) : null; });
    }).then(function (){ return model; });
  }

  /** Decide how each item enters the report: an image page, merged PDF pages,
   *  an embedded attachment, or a QR code. Phone photos are redrawn through a
   *  canvas so their EXIF orientation is honoured and their size capped. */
  function evalPrepare(model, L){
    var all = [];
    model.kpis.forEach(function (k){ k.evidence.forEach(function (ev){ all.push(ev); }); });
    return evSerial(all, function (ev){
      var e = ev.row, key = evTypeKey(e), b = ev.bytes;
      ev.qr = null; ev.attach = false; ev.kind = 'file';
      if (!evIsFile(e)){ ev.kind = 'link'; ev.qr = e.url || null; return null; }
      if (!b){ ev.kind = 'missing'; return null; }
      var isPng = b.length > 8 && b[0] === 0x89 && b[1] === 0x50 && b[2] === 0x4E && b[3] === 0x47;
      var isPdf = b.length > 4 && b[0] === 0x25 && b[1] === 0x50 && b[2] === 0x44 && b[3] === 0x46;
      if (isPng){ ev.kind = 'png'; ev.img = b; return null; }
      if (/^image\//.test(e.mime_type || '') || key === 'image'){
        return evToJpeg(b, e.mime_type).then(function (jpg){
          if (jpg){ ev.kind = 'jpg'; ev.img = jpg; } else { ev.kind = 'file'; ev.attach = true; }
        });
      }
      if (isPdf){
        return L.PDFDocument.load(b, { ignoreEncryption: true, updateMetadata: false }).then(function (d){
          if (d.getPageCount() > 0){ ev.kind = 'pdf'; ev.pdf = d; } else { ev.attach = true; }
        }, function (){ ev.kind = 'file'; ev.attach = true; });
      }
      ev.attach = true;
      // an uploaded recording: in LOCAL mode the server has a URL for it - worth
      // a QR code when it is reachable from another device (not a loopback host)
      if (key === 'video' || key === 'audio'){
        var u = DB.coldUrl('evidence_file', e.id);
        if (u){ ev.localUrl = u; if (!/^https?:\/\/(localhost|127\.|\[::1\])/i.test(u)) ev.qr = u; }
      }
      return null;
    });
  }
  function evToJpeg(bytes, mime){
    return new Promise(function (resolve){
      var url = URL.createObjectURL(new Blob([bytes], { type: mime || 'image/jpeg' })), img = new Image();
      function done(v){ URL.revokeObjectURL(url); resolve(v); }
      img.onload = function (){
        try {
          var w = img.naturalWidth, h = img.naturalHeight, s = Math.min(1, 2400 / Math.max(w, h, 1));
          var c = document.createElement('canvas');
          c.width = Math.max(1, Math.round(w * s)); c.height = Math.max(1, Math.round(h * s));
          var g = c.getContext('2d');
          g.fillStyle = '#ffffff'; g.fillRect(0, 0, c.width, c.height);   // transparent GIF/WebP → white paper
          g.drawImage(img, 0, 0, c.width, c.height);
          c.toBlob(function (blob){
            if (!blob) return done(null);
            blob.arrayBuffer().then(function (ab){ done(new Uint8Array(ab)); }, function (){ done(null); });
          }, 'image/jpeg', 0.88);
        } catch (x){ done(null); }
      };
      img.onerror = function (){ done(null); };
  ...
  /** A QR code drawn as vector squares (runs of dark modules merged per row),
   *  with its quiet zone, bottom-left corner at (x, y). */
  function pdfQr(doc, text, x, y, size){
    var qr;
    try {
      if (qrcode.stringToBytesFuncs && qrcode.stringToBytesFuncs['UTF-8']) qrcode.stringToBytes = qrcode.stringToBytesFuncs['UTF-8'];
      qr = qrcode(0, 'M'); qr.addData(String(text)); qr.make();
    } catch (x){ return false; }
    var n = qr.getModuleCount(), quiet = 2, cell = size / (n + quiet * 2), black = [0, 0, 0];
    doc.rect(x, y, size, size, [1, 1, 1]);
    for (var row = 0; row < n; row++){
      var c = 0;
      while (c < n){
        if (!qr.isDark(row, c)){ c++; continue; }
        var s = c; while (c < n && qr.isDark(row, c)) c++;
        // a hair of overlap so viewers never show seams between rows
        doc.rect(x + (quiet + s) * cell, y + size - (quiet + row + 1) * cell - 0.05, (c - s) * cell, cell + 0.1, black);
      }
    }
    return true;
  }

  /** Stage 2 · RENDER the report's own pages. Returns the bytes plus the page
   *  ranges assembly interleaves evidence into, and the link areas to make live. */
  function evalRenderBase(model){
    var pr = model.project, P = model.P;
    ...
    var BOTTOM = M + 20, links = [];
    var co = P.country, don = P.donor, pt = P.partner;
    var st = P.statAll || {}, S0 = STATUS[st.code || 'nodata'] || STATUS.nodata;

    function fit(s, size, bold, maxW){
      s = String(s == null ? '' : s);
      if (doc.textW(s, size, bold) <= maxW) return s;
      while (s.length > 1 && doc.textW(s + '…', size, bold) > maxW) s = s.slice(0, -1);
      return s.replace(/\s+$/, '') + '…';
    }
    function opener(title, sub){
      doc.addPage();
      var rt = 'Evaluation Report  ·  ' + pdfDate(TODAY);
      pdfBrandHead(doc, M, PW - M - doc.textW(rt, 8.5, false) - 14);
      doc.text(rt, PW - M - doc.textW(rt, 8.5, false), PH - 26, { size: 8.5, color: doc.rgb('#d7e3f0') });
      var y = PH - 92, t = String(title).toUpperCase(), tf = 16;
      while (tf > 11 && doc.textW(t, tf, true) > AW) tf -= 0.5;
      doc.text(t, M, y, { size: tf, bold: true, color: INK });
      y -= 9; doc.roundRect(M, y - 3, 36, 3, 1.5, GOLD); y -= 13;
      if (sub){ doc.wrap(sub, AW, 10.5, true).slice(0, 2).forEach(function (ln){ doc.text(ln, M, y - 2, { size: 10.5, bold: true, color: SUB }); y -= 14; }); }
      return y - 6;
    }
    function section(y, label){
      if (y < M + 90){ doc.addPage(); y = PH - M; } else y -= 8;
      doc.text(String(label || '').toUpperCase(), M, y - 9, { size: 8.5, bold: true, color: NAVY });
      doc.hline(M, PW - M, y - 15, RULE, 0.8);
      return y - 25;
    }
    function para(y, text, size, color, italic){
      doc.wrap(text, AW, size, false).forEach(function (ln){
        if (y - size - 4 < BOTTOM){ doc.addPage(); y = PH - M; }
        doc.text(ln, M, y - size, { size: size, color: color || SUB, italic: !!italic }); y -= size + 4;
      });
      return y - 4;
    }
    function statCards(y, stats){
      var nS = stats.length, gap = 10, cardW = (AW - gap * (nS - 1)) / nS, cardH = 52;
      if (y - cardH < BOTTOM){ doc.addPage(); y = PH - M; }
      stats.forEach(function (s, i){
        var sx = M + i * (cardW + gap);
        ...
        var vF = 15; while (vF > 8 && doc.textW(s.value || '', vF, true) > cardW - 18) vF -= 0.5;
        ...
        if (s.sub){
          if (s.color){
            ...
            doc.text(fit(s.sub, 6.5, true, chW - 12), sx + 15, y - 44.5, { size: 6.5, bold: true, color: [1, 1, 1] });
          } else doc.text(fit(s.sub, 7.5, false, cardW - 18), sx + 9, y - 44.5, { size: 7.5, color: MUT });
        }
      });
      return y - cardH - 18;
    }
    function factGrid(y, facts){
      ...
      for (var i = 0; i < facts.length; i += 2){
        if (y - rowH < BOTTOM){ doc.addPage(); y = PH - M; }
        for (var c = 0; c < 2; c++){
          var fct = facts[i + c]; if (!fct) continue;
          var fx = M + c * (colW + gap);
          doc.text(String(fct[0]).toUpperCase(), fx, y - 10, { size: 6.5, bold: true, color: MUT });
          doc.text(fit(fct[1] == null || fct[1] === '' ? '-' : fct[1], 10.5, false, colW), fx, y - 23, { size: 10.5, color: INK });
        }
        y -= rowH;
      }
      return y;
    }
    function chainText(k){
      return [k.impact.secondary ? 'Project-specific results' : k.impact.code, k.outcome.code, k.output.code]
        .filter(Boolean).join('  ›  ');
    }
    function kdot(r){ return r.sdg ? PILLAR_COLORS[r.sdg] : (r.secondary ? '#27500a' : '#94a3b8'); }
    var files = 0, links_n = 0, attached = [];
    model.kpis.forEach(function (k){ k.evidence.forEach(function (ev){
      if (evIsFile(ev.row)) files++; else links_n++;
      if (ev.attach) attached.push({ ev: ev, k: k });
    }); });
    var covered = model.kpis.filter(function (k){ return k.evidence.length; }).length;

    // ---- COVER ------------------------------------------------------------
    var y = opener('Evaluation Report', P.name || 'Project');
    doc.text([pr.code, co ? co.name : pr.country_iso3, don ? don.name : ''].filter(Boolean).join('  ·  '), M, y - 2, { size: 9.5, color: SUB });
    y -= 20;
    ...
    doc.text(st.ratio != null ? 'Overall performance ' + Math.round(st.ratio * 100) + '%' : 'Not yet measured', M + bw + 10, y - 8.5, { size: 9, color: MUT });
    y -= 30;
    y = statCards(y, [
      { label: 'Results', value: fmt(model.kpis.length), sub: fmt(P.primary.length) + ' primary · ' + fmt(P.secondary.length) + ' secondary' },
      { label: 'With evidence', value: fmt(covered) + ' of ' + fmt(model.kpis.length), sub: (model.kpis.length ? Math.round(covered / model.kpis.length * 100) : 0) + '% evidenced' },
      { label: 'Evidence items', value: fmt(model.evidenceN), sub: fmt(files) + ' files · ' + fmt(links_n) + ' links' },
      { label: 'Performance', value: st.ratio != null ? Math.round(st.ratio * 100) + '%' : '-', sub: S0.label, color: S0.c }
    ]);
    y = section(y, 'Project profile');
    var _ap = activePlan();
    y = factGrid(y, [
      ['Project code', pr.code], ['Country', co ? co.name : pr.country_iso3],
      ['Donor', don ? don.name : ''], ['Delivery', pt ? (pt.name + (pt.acronym ? ' (' + pt.acronym + ')' : '')) : implLabel(P.implementation)],
      ['Lead', userName(projectLeadId(pr))], ['Budget (USD)', pr.budget_usd != null ? '$' + fmt(pr.budget_usd) : ''],
      ['Timeframe', (pr.start_date || '?') + '   to   ' + (pr.end_date || '?')], ['Plan', _ap ? _ap.name : '']
    ]);
    if (model.synthesis){ y = section(y, 'Evaluation synthesis'); y = para(y, model.synthesis, 9.5, INK); }
    y = section(y, 'About this report');
    y = para(y, 'This report brings together the results this project has achieved and the evidence uploaded against each of them. '
      + 'Results are grouped down the results chain - Impact, Outcome, Output. Each evidenced result has its own evidence sheet, '
      + 'followed by its photos and document pages. Video and voice links print as QR codes: scan one with a phone to watch or listen. '
      + 'Files that cannot be printed on a page (spreadsheets, word-processor documents, audio and video files) are embedded inside this PDF '
      + '- open the attachments panel of your PDF reader to view them.', 9, SUB);

    // ---- RESULTS INDEX ----------------------------------------------------
    y = section(y, 'Results and evidence');
    var idxRows = [];
    model.groups.forEach(function (I){ I.outcomes.forEach(function (O){ O.outputs.forEach(function (Q){
      var title = I.secondary ? 'Project-specific results'
        : [I.code, O.code, Q.code].filter(Boolean).join('  ›  ') + (Q.name ? '  ·  ' + Q.name : '');
      idxRows.push({ group: { title: fit(title, 9, true, AW - 16), color: I.color, count: null } });
      Q.kpis.forEach(function (r){
        var m = evMetrics(r), nEv = evRows(pr.id, r.kpiId).length;
        idxRows.push([
          { t: kpiLabelText(r), dot: kdot(r) },
          { t: m.achieved, align: 'right' }, { t: m.target, align: 'right' },
          { t: m.progress, align: 'right' },
          { t: m.status.label + (r.performance != null ? ' · ' + m.performance : ''), color: m.status.c },
          { t: nEv ? String(nEv) : '0', align: 'right', color: nEv ? '#0c447c' : null }
        ]);
      });
    }); }); });
    y = pdfTable(doc, [
      { t: 'Result', w: 236 }, { t: 'Achieved', w: 54, align: 'right' }, { t: 'Target', w: 54, align: 'right' },
      { t: 'Progress', w: 50, align: 'right' }, { t: 'Performance', w: 94 }, { t: 'Evidence', w: 48, align: 'right' }
    ], idxRows, M, AW, y);
    y -= 6;
    doc.text('Progress is achievement of target; Performance is the RAG rating - progress paced against the result’s own timeframe.', M, y - 8, { size: 7.5, italic: true, color: MUT });
    var introEnd = doc.pageCount();

    // ---- ONE EVIDENCE SHEET PER EVIDENCED RESULT ---------------------------
    var sections = [];
    model.kpis.forEach(function (k){
      if (!k.evidence.length) return;
      var r = k.r, m = evMetrics(r), from = doc.pageCount();
      doc.addPage();
      var y = PH - M;
      doc.text('RESULT EVIDENCE', M, y - 8, { size: 7, bold: true, color: NAVY });
      doc.text(fit(chainText(k), 8, false, AW - 90), M + 78, y - 8, { size: 8, color: MUT });
      y -= 16;
      doc.wrap(kpiLabelText(r), AW, 13, true).slice(0, 3).forEach(function (ln){ doc.text(ln, M, y - 12, { size: 13, bold: true, color: INK }); y -= 16; });
      y -= 3; doc.roundRect(M, y - 3, 34, 3, 1.5, GOLD); y -= 12;
      if (!k.impact.secondary && k.output.name){ y = para(y, k.output.code ? k.output.code + '  ·  ' + k.output.name : k.output.name, 8.5, SUB); }
      y = statCards(y, [
        { label: 'Achieved', value: m.achieved, sub: 'latest value' },
        { label: 'Target', value: m.target, sub: r.raw.target_date ? 'by ' + shortDate(r.raw.target_date) : '' },
        { label: 'Progress', value: m.progress, sub: 'of target' },
        { label: 'Performance', value: m.performance, sub: m.status.label, color: m.status.c }
      ]);
      if (k.synthesis){ y = section(y, 'Synthesis'); y = para(y, k.synthesis, 9.5, INK); }
      y = section(y, 'Evidence  ·  ' + k.evidence.length);
      y = pdfTable(doc, [
        { t: '#', w: 30, align: 'center' }, { t: 'Evidence', w: 190 }, { t: 'Type', w: 74 },
        { t: 'Date', w: 54, align: 'center' }, { t: 'Uploaded by', w: 78 }, { t: 'In this report', w: 94 }
      ], k.evidence.map(function (ev){
        var e = ev.row, where = {
          jpg: 'Photo page follows', png: 'Photo page follows', link: 'QR code below', missing: 'File missing',
          pdf: ev.pdf ? (ev.pdf.getPageCount() + ' page' + (ev.pdf.getPageCount() === 1 ? '' : 's') + ' follow') : '',
          file: 'Embedded file' + (ev.qr ? ' + QR code' : '')
        }[ev.kind] || 'Embedded file';
        var what = e.name + (e.description ? ' - ' + e.description : '') + (evIsFile(e) ? '  (' + e.file_name + ', ' + evSize(e.size_bytes) + ')' : '');
        return [
          { t: 'E' + ev.n }, { t: what }, { t: evTypeName(e) },
          { t: e.evidence_date ? shortDate(e.evidence_date) : '-' }, { t: userName(e.created_by) }, { t: where }
        ];
      }), M, AW, y);

      // QR cards: two per row - the code, the item, and the link printed in full
      var qrs = k.evidence.filter(function (ev){ return ev.qr; });
      if (qrs.length){
        y = section(y - 6, 'Scan to watch or listen');
        var gap = 12, cw = (AW - gap) / 2, ch = 104, qs = 80;
        qrs.forEach(function (ev, i){
          var col = i % 2;
          if (col === 0 && i > 0) y -= ch + gap;
          if (col === 0 && y - ch < BOTTOM){ doc.addPage(); y = PH - M; }
          var cx = M + col * (cw + gap), top = y;
          doc.roundRect(cx, top - ch, cw, ch, 6, SOFT);
          doc.roundRect(cx, top - ch, cw, ch, 6, CARD, 0.8);
          pdfQr(doc, ev.qr, cx + 12, top - ch + (ch - qs) / 2, qs);
          var tx = cx + 12 + qs + 12, tw = cw - (tx - cx) - 12, ty = top - 18;
          doc.text(('E' + ev.n + '  ·  ' + evTypeName(ev.row)).toUpperCase(), tx, ty, { size: 6.5, bold: true, color: NAVY });
          ty -= 13;
          doc.wrap(ev.row.name, tw, 9.5, true).slice(0, 2).forEach(function (ln){ doc.text(ln, tx, ty, { size: 9.5, bold: true, color: INK }); ty -= 12; });
          ty -= 2;
          var ul = doc.wrap(ev.qr, tw, 7, false).slice(0, 4);
          ul.forEach(function (ln){ doc.text(ln, tx, ty, { size: 7, color: doc.rgb('#2563eb') }); ty -= 9; });
          links.push({ page: doc.pageCount() - 1, url: ev.qr, rect: [tx, ty + 6, tx + tw, ty + 9 * ul.length + 8] });
          links.push({ page: doc.pageCount() - 1, url: ev.qr, rect: [cx + 12, top - ch + (ch - qs) / 2, cx + 12 + qs, top - ch + (ch - qs) / 2 + qs] });
        });
        y -= ch + gap;
      }
      // a recording kept only on this machine: say where it is, with no QR
      k.evidence.forEach(function (ev){
        if (ev.localUrl && !ev.qr){
          if (y - 24 < BOTTOM){ doc.addPage(); y = PH - M; }
          y = para(y, 'E' + ev.n + ' is embedded in this PDF; on the machine that holds the database it also plays from ' + ev.localUrl, 7.5, MUT, true);
        }
      });
      sections.push({ k: k, from: from, to: doc.pageCount() });
    });

    // ---- EMBEDDED FILES ---------------------------------------------------
    var closeFrom = doc.pageCount();
    if (attached.length){
      doc.addPage();
      y = PH - M;
      doc.text('EMBEDDED FILES', M, y - 12, { size: 13, bold: true, color: INK });
      y -= 18; doc.roundRect(M, y - 3, 34, 3, 1.5, GOLD); y -= 14;
      y = para(y, 'These files could not be printed on a page, so they travel inside this PDF. Open the attachments panel of your PDF reader (the paper-clip icon) to view or save them.', 9, SUB);
      y = pdfTable(doc, [
        { t: '#', w: 30, align: 'center' }, { t: 'File', w: 164 }, { t: 'Type', w: 74 }, { t: 'Size', w: 50, align: 'right' }, { t: 'Result', w: 202 }
      ], attached.map(function (a){
        return [ { t: 'E' + a.ev.n }, { t: a.ev.row.file_name }, { t: evTypeName(a.ev.row) },
                 { t: evSize(a.ev.row.size_bytes), align: 'right' }, { t: kpiLabelText(a.k.r), dot: kdot(a.k.r) } ];
      }), M, AW, y);
    }
    var raw = doc.build(), bytes = new Uint8Array(raw.length);
    for (var i = 0; i < raw.length; i++) bytes[i] = raw.charCodeAt(i) & 0xff;
    return { bytes: bytes, introEnd: introEnd, sections: sections, closeFrom: closeFrom, closeTo: doc.pageCount(), links: links };
  }

  /** Stage 3 · ASSEMBLE: the report pages with each result's evidence pages
   *  interleaved after its sheet, attachments embedded, link areas made live,
   *  and one footer ("Page n of N") stamped across the finished document. */
  function evalAssemble(model, base, L){
    var out, src, F1, F2, charset = null, labels = [], baseToOut = {};
    var A4 = [595.28, 841.89], M = 44;
    var brand = orgBrand(), projLabel = (model.P.name || 'Project') + (model.project.code ? '  ·  ' + model.project.code : '');
    function wa(s){
      s = String(s == null ? '' : s).replace(/[‘’]/g, "'").replace(/[“”]/g, '"').replace(/[→↳]/g, '-').replace(/[\r\n\t]+/g, ' ');
      if (!charset){ charset = {}; F1.getCharacterSet().forEach(function (c){ charset[c] = 1; }); }
      var o = '';
      for (var i = 0; i < s.length; i++){ var c = s.codePointAt(i); if (c > 0xffff) i++; o += charset[c] ? String.fromCodePoint(c) : '?'; }
      return o;
    }
    function wrapL(text, font, size, maxW){
      var words = wa(text).split(/\s+/), lines = [], line = '';
      words.forEach(function (w){
        if (!w) return;
        var t = line ? line + ' ' + w : w;
    ...
    function copyRange(from, to){
      ...
      return out.copyPages(src, idx).then(function (pages){
        pages.forEach(function (pg, j){ baseToOut[from + j] = out.getPageCount(); out.addPage(pg); labels.push(null); });
      });
    }
    function evLabel(ev, k){ return 'Evidence E' + ev.n + '  ·  ' + (k.r.raw.code || k.r.name) + '  ·  ' + (ev.row.file_name || ev.row.name); }
    function imagePage(ev, k){
      var embed = ev.kind === 'png' ? out.embedPng(ev.img) : out.embedJpg(ev.img);
      return embed.then(function (im){
        var page = out.addPage(A4), W = A4[0], H = A4[1], y = H - M;
        labels.push(evLabel(ev, k));
        var navy = L.rgb(0.047, 0.267, 0.486), ink = L.rgb(0.1, 0.13, 0.19), mut = L.rgb(0.53, 0.57, 0.64);
        page.drawText(wa('EVIDENCE E' + ev.n + '  ·  ' + evTypeName(ev.row).toUpperCase()), { x: M, y: y - 8, size: 7, font: F2, color: navy });
        y -= 22;
        wrapL(ev.row.name, F2, 13, W - 2 * M).slice(0, 2).forEach(function (ln){ page.drawText(ln, { x: M, y: y - 4, size: 13, font: F2, color: ink }); y -= 16; });
        wrapL(kpiLabelText(k.r), F1, 8.5, W - 2 * M).slice(0, 2).forEach(function (ln){ page.drawText(ln, { x: M, y: y - 4, size: 8.5, font: F1, color: mut }); y -= 11; });
        if (ev.row.description){
          y -= 2;
          wrapL(ev.row.description, F1, 9, W - 2 * M).slice(0, 4).forEach(function (ln){ page.drawText(ln, { x: M, y: y - 4, size: 9, font: F1, color: ink }); y -= 12; });
        }
        y -= 10;
        var boxW = W - 2 * M, boxH = Math.max(160, y - 50), s = Math.min(boxW / im.width, boxH / im.height, 1.5);
        var iw = im.width * s, ih = im.height * s;
        page.drawImage(im, { x: M + (boxW - iw) / 2, y: y - ih, width: iw, height: ih });
      });
    }
    function pdfPages(ev, k){
      return out.copyPages(ev.pdf, ev.pdf.getPageIndices()).then(function (pages){
        pages.forEach(function (pg){ out.addPage(pg); labels.push(evLabel(ev, k)); });
      }, function (){ ev.attach = true; });   // a PDF pdf-lib cannot copy still travels as an attachment
    }

    return L.PDFDocument.create().then(function (d){
      out = d;
      return L.PDFDocument.load(base.bytes);
    }).then(function (b){
      src = b;
      return Promise.all([out.embedFont(L.StandardFonts.Helvetica), out.embedFont(L.StandardFonts.HelveticaBold)]);
    }).then(function (fonts){
      F1 = fonts[0]; F2 = fonts[1];
      var chain = copyRange(0, base.introEnd);
      base.sections.forEach(function (sec){
        chain = chain.then(function (){ return copyRange(sec.from, sec.to); });
        sec.k.evidence.forEach(function (ev){
          chain = chain.then(function (){
            if (ev.kind === 'jpg' || ev.kind === 'png') return imagePage(ev, sec.k).catch(function (){ ev.attach = true; });
            if (ev.kind === 'pdf') return pdfPages(ev, sec.k);
            return null;
          });
        });
      });
      return chain.then(function (){ return copyRange(base.closeFrom, base.closeTo); });
    }).then(function (){
      // every file that could not be printed travels inside the PDF
      var jobs = [];
      model.kpis.forEach(function (k){ k.evidence.forEach(function (ev){
        if (!ev.attach || !ev.bytes) return;
        var when = ev.row.uploaded ? new Date(ev.row.uploaded) : new Date();
        jobs.push(out.attach(ev.bytes, 'E' + ev.n + '-' + ev.row.file_name, {
          mimeType: ev.row.mime_type || 'application/octet-stream',
          description: wa(ev.row.name + ' - ' + kpiLabelText(k.r)),
          creationDate: when, modificationDate: when
        }));
      }); });
      return Promise.all(jobs);
    }).then(function (){
      // live links over the printed URLs and QR codes
      base.links.forEach(function (lk){
        var pi = baseToOut[lk.page]; if (pi == null) return;
        try {
          var ctx = out.context;
          var annot = ctx.register(ctx.obj({
            Type: 'Annot', Subtype: 'Link', Rect: lk.rect, Border: [0, 0, 0],
            A: { Type: 'Action', S: 'URI', URI: L.PDFString.of(lk.url) }
          }));
          out.getPage(pi).node.addAnnot(annot);
        } catch (x){ /* a link that will not register still prints as text */ }
      });
      // footer across every page, report and evidence alike
      var pages = out.getPages(), total = pages.length, fcol = L.rgb(0.6, 0.64, 0.7), rule = L.rgb(0.89, 0.91, 0.93);
      pages.forEach(function (pg, i){
        var sz = pg.getSize(), left = wa(brand + '  ·  ' + (labels[i] || ('Evaluation Report  ·  ' + projLabel)));
        var pn = 'Page ' + (i + 1) + ' of ' + total, pw = F1.widthOfTextAtSize(pn, 7);
        while (left.length > 8 && F1.widthOfTextAtSize(left, 7) > sz.width - 2 * M - pw - 20) left = left.slice(0, -2);
        pg.drawLine({ start: { x: M, y: 31 }, end: { x: sz.width - M, y: 31 }, thickness: 0.6, color: rule });
        pg.drawText(left, { x: M, y: 21, size: 7, font: F1, color: fcol });
        pg.drawText(pn, { x: sz.width - M - pw, y: 21, size: 7, font: F1, color: fcol });
      });
      model.outPages = total;
      out.setTitle(wa('Evaluation Report - ' + (model.P.name || 'Project')));
      out.setSubject(wa('Results and evidence - ' + projLabel));
      out.setAuthor(wa(brand));
      out.setCreator('The Grassroots');
      out.setProducer('The Grassroots');
  ...
  function exportProjectPDF(pr){
    ...
    y = factGrid(y, [
      ...
      ['Donor', don ? don.name : ''],
      ['Delivery', implLabel(P.implementation)],
    ...
    // Code + name are ONE label, so they share one column here as on screen.
    var resCols = [
      { t: 'KPI', w: 262 },
    ...
    if (!acts.length){
      ...
      y = pdfTable(doc, [
        { t: '#', align: 'center', w: 26 }, { t: 'KPI', w: 170 }, { t: 'Activity', w: 120 },
        { t: 'Location', w: 84 }, { t: 'Value', w: 52, align: 'right' }, { t: 'Date', w: 58, align: 'center' },
        { t: 'Reported by', w: 84 }
      ], acts.map(function (m, i){
        var ind = useOfMeas(m), u = (ind && ind.unit === '%') ? '%' : '';
        return [
          { t: String(i + 1) },
          { t: kpiLabelText(ind), dot: ind ? kdot(ind) : '#94a3b8' },
          { t: m.narrative || '-' },
          { t: m.place_name || '-' },
