...
# Each region carries its identity colour as DATA (region.color), the same way a
# donor or a partner does - the app reads the colour off the row instead of
# keeping a name->colour map in JavaScript.
REGIONS = [
 (1, "Africa",        1, "#F2934A"),
 (2, "Asia",          2, "#5399EA"),
 (3, "Europe",        3, "#A97FDD"),
 (4, "North America", 4, "#EC7BA6"),
 (5, "South America", 5, "#2CC4A0"),
 (6, "Oceania",       6, "#6FBF73"),
]
regions = [{"id": rid, "name": name, "color": color, "seq": seq} for (rid, name, seq, color) in REGIONS]
region_id_by_name = {name: rid for (rid, name, seq, color) in REGIONS}

# Taxonomies that used to be CHECK (...) enums in schema.sql and literal arrays
# in app.js. They are lists, so they are tables - seeded here, referenced by key.
result_levels = [{"id": i + 1, "key": k, "name": n, "seq": i + 1} for i, (k, n) in enumerate(
    [("impact", "Impact"), ("outcome", "Outcome"), ("output", "Output"), ("activity", "Activity")])]
risk_levels = [{"id": i + 1, "key": k, "name": n, "seq": i + 1} for i, (k, n) in enumerate(
    [("low", "Low"), ("medium", "Medium"), ("high", "High")])]
implementation_modes = [{"id": i + 1, "key": k, "name": n, "seq": i + 1} for i, (k, n) in enumerate(
    [("direct", "Directly by the organisation"), ("partner", "Through an implementing partner")])]
...
payload = {
    ...
    "result_level":result_levels,
    "risk_level":risk_levels,
    "implementation_mode":implementation_modes,
    "country":countries,
    "user":users,
    "donor":donors,
    "partner":partners,
