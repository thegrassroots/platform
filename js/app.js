...
(function () {
  ...
  function buildLocations(projs) {
    ...
    MEAS_NOPROJ.forEach(function (m) {
      ...
      var loc = locs[key] || (locs[key] = { lat: +m.place_lat, lng: +m.place_lng, name: m.place_name, acts: 0, projIds: {}, actsByProj: {} });
      loc.acts++;
      (loc.freeInds = loc.freeInds || []).push(INDBYID[m.indicator_id]);
      if (!loc.name && m.place_name) loc.name = m.place_name;
    });
  ...
  /** How many distinct KPIs a pool of activities reports against. */
  function kpiCountOf(acts){
    var seen = {}, n = 0;
    (acts || []).forEach(function (m){ if (!seen[m.indicator_id]){ seen[m.indicator_id] = 1; n++; } });
    return n;
  }
  ...
  function openLocationKpiActivities(c, indId, push){
    ...
    var r = INDBYID[indId];
    var mine = function (m){ return m.indicator_id === indId; };
  ...
  function beneficiaryHeatmap(acts, opts){
    ...
    var units = opts.groups || acts.map(function (m){
      var pr = PROJECTSBYID[m.project_id], bInd = INDBYID[m.indicator_id];
      return { acts: [m],
  ...
  function oneKpiPool(acts){
    ...
    for (var i = 0; i < (acts || []).length; i++){
      var l = kpiLabelText(INDBYID[acts[i].indicator_id]);
  ...
  function kpiRollupBlock(acts, opts){
    ...
    (acts || []).forEach(function (m){
      ...
      if (!g){
        g = byInd[m.indicator_id] = { id: m.indicator_id, r: INDBYID[m.indicator_id],
    ...
    if (!order.length){
      ...
      html += '<div class="cp-note">One row per KPI. <b>Click a KPI</b> to read the activities logged against it.</div>'
        ...
        + '<th class="col-eq col-r" title="The project that reported this KPI - the results chain is instantiated per project, so the same KPI recurs once per project">Project</th>'
  ...
  function openActivityEdit(r, m){
    ...
    var details = '<div class="aform">'
      ...
      + '<div class="afgrid">'
      + '  <label class="af full"><span>Project</span><select id="naProject">'
      +      activityProjectOptionsFor(r, m.project_id != null ? +m.project_id : null) + '</select></label>'
      + activityFieldsHtml({
  ...
  function renderResultsEditor(r){
    ...
    if (editable) {
      ...
      add.onclick = function (){ openActivityAdd(null, r.id); };
  ...
  /** Primary KPIs: SEARCH the whole inventory and add; the tree below lists only
   *  what this project actually reports against, grouped by Impact. */
  function projectPrimaryEditor(){
    var p = curProject, ro = !canEditProjectContainer(p);
    // `picked` is the working set - edited by add/remove here, committed on Save.
    var linked = {}; (DB._idx.projectKpiByProject[p.id] || []).forEach(function (pk){ linked[pk.indicator_id] = 1; });
    var picked = {}; Object.keys(linked).forEach(function (iid){ picked[iid] = 1; });

    // Candidates are the ENTIRE framework inventory, not just this country's -
    // a project may report against a KPI defined anywhere in the results chain.
    var all = IND.filter(function (r){ return !r.secondary; });
    var coName = function (iso){ var c = DB._idx.countryByIso[iso]; return c ? c.name : (iso || ''); };

    var box = el('div', 'prtab');
    box.appendChild(elHTML('div', 'cp-note', 'Primary KPIs are drawn from the <b>KPI inventory</b>. Search the inventory below and add the KPIs this project reports against; their activities aggregate into the project. Secondary (project-local) KPIs are managed on the next tab.'));

    var list = el('div', 'prkpi-list');

    // ---- search-and-add -------------------------------------------------
    var search = el('div', 'prkpi-search');
    var inp = el('input', 'prkpi-q');
    inp.setAttribute('data-notr', '');   // a typed query is the user's own words
    inp.type = 'search';
    inp.placeholder = 'Search all KPIs by code, name or country…';
    inp.disabled = ro;
    var results = el('div', 'prkpi-res hide');
    search.appendChild(inp); search.appendChild(results);
    if (!ro) box.appendChild(search);

    function closeResults(){ results.classList.add('hide'); results.innerHTML = ''; }
    ...
    function runSearch(){
      ...
      var hits = all.filter(function (r){
        if (picked[r.id]) return false;                       // already on the project
        var hay = ((r.raw.code || '') + ' ' + r.name + ' ' + coName(r.iso) + ' ' + pillarLabel(r.sdg || 0)).toLowerCase();
        return terms.every(function (t){ return hay.indexOf(t) >= 0; });
      }).slice(0, 40);
      results.innerHTML = '';
      results.classList.remove('hide');
      if (!hits.length){ results.appendChild(el('div', 'prkpi-none', 'No KPI matches "' + inp.value.trim() + '".')); return; }
      hits.forEach(function (r){
        var row = el('button', 'prkpi-hit'); row.type = 'button';
        row.innerHTML = '<span class="prkpi-hit-code">' + esc(r.raw.code || '') + '</span>'
          + '<span class="prkpi-hit-nm">' + esc(r.name) + '</span>'
          + '<span class="prkpi-hit-meta">' + esc(coName(r.iso)) + ' · ' + esc(pillarLabel(r.sdg || 0)) + '</span>'
          + '<span class="prkpi-hit-add">＋ Add</span>';
        row.onclick = function (){ picked[r.id] = 1; inp.value = ''; closeResults(); paint(); inp.focus(); };
        results.appendChild(row);
      });
    }
    inp.oninput = runSearch;
    inp.onfocus = runSearch;
    inp.onkeydown = function (e){
      if (e.key === 'Escape'){ e.stopPropagation(); inp.value = ''; closeResults(); }
      // Enter adds the single remaining match - the quick path once a code is typed
      if (e.key === 'Enter'){
        e.preventDefault();
        var first = results.querySelector('.prkpi-hit');
        if (first) first.click();
      }
    };

    // ---- the tree: ONLY the KPIs on this project, grouped by Impact -------
    function paint(){
      list.innerHTML = '';
      var chosen = all.filter(function (r){ return picked[r.id]; })
        .sort(function (a, b){ return (a.sdg || 99) - (b.sdg || 99) || (a.raw.code < b.raw.code ? -1 : 1); });
      if (!chosen.length){
        list.appendChild(el('div', 'empty', ro
          ? 'This project reports against no primary KPIs.'
          : 'No primary KPIs yet - search above to add the first one.'));
        return;
      }
      var byImpact = {};
      chosen.forEach(function (r){ (byImpact[r.sdg || 0] = byImpact[r.sdg || 0] || []).push(r); });
      Object.keys(byImpact).sort(function (a, b){ return (+a || 99) - (+b || 99); }).forEach(function (sdg){
        ...
        byImpact[sdg].forEach(function (r){
          var row = el('div', 'prkpi-row');
          var txt = el('span', 'prkpi-txt');
          txt.innerHTML = '<b>' + esc(r.raw.code || '') + '</b> · ' + esc(r.name)
            + ' <span class="prkpi-st" style="color:' + STATUS[r.status].c + '">'
            + (r.frac != null ? Math.round(r.frac * 100) + '%' : 'no data') + '</span>';
          row.appendChild(txt);
          if (!ro){
            // A KPI already linked AND carrying activities under this project is
            // locked: detaching it would strand those activity reports.
            var acts = linked[r.id] ? projKpiActivityCount(r.id, p.id) : 0;
            var del = el('button', 'prkpi-del'); del.type = 'button';
            del.innerHTML = '🗑';
            if (acts){
              del.disabled = true;
              del.classList.add('locked');
              del.title = acts + ' activit' + (acts === 1 ? 'y is' : 'ies are') + ' logged against this KPI in this project. '
                + 'Remove those activities first (Activities tab), then this KPI can be removed.';
            } else {
              del.title = 'Remove this KPI from the project';
              del.onclick = function (){ delete picked[r.id]; paint(); };
            }
            row.appendChild(del);
          }
    ...
    // Clicking outside the results closes them (Esc is handled on the input).
    box.addEventListener('click', function (e){
      if (!search.contains(e.target)) closeResults();
    });

    // Save is driven from the persistent modal footer (configProjectFoot).
    if (!ro) { box.__saveLabel = 'Save primary KPIs'; box.__save = function (){ savePrimaryKpis(picked, linked); }; }
    return box;
  }
  function savePrimaryKpis(picked, linked){
    if (!canEditProjectContainer(curProject)) return;   // creator (or Admin) only
    var pid = curProject.id;
    var adds = [], delIds = [], removed = {};
    Object.keys(picked).forEach(function (iid){ if (!linked[iid]) adds.push(+iid); });
    (DB._idx.projectKpiByProject[pid] || []).forEach(function (pk){
      if (!picked[pk.indicator_id]){ delIds.push(pk.id); removed[pk.indicator_id] = 1; }
    });
    var jobs = [];
    ...
    // A removed KPI can only reach here with no activities on this project (the
    // bin is disabled otherwise), so this detaches nothing that still exists.
    Object.keys(removed).forEach(function (iid){
      (DB._idx.measByIndicator[+iid] || []).forEach(function (m){ if (m.project_id === pid){ m.project_id = null; mChanged.push(m); } });
    });
  ...
  function projectKpiOptions(p, selId){
    ...
    function opt(r, tag){
      return '<option value="' + r.id + '"' + (selId != null && r.id === selId ? ' selected' : '') + '>'
        + (r.raw.code ? esc(r.raw.code) + ' · ' : '') + esc(r.name) + ' [' + tag + ']</option>';
    }
    var html = '';
  ...
  function projectActivitiesEditor(){
    ...
    var drillR = _prActKpi != null ? INDBYID[_prActKpi] : null;
  ...
  function projActKpiTable(p, all, editable){
    ...
    all.forEach(function (m){
      var g = byInd[m.indicator_id];
      if (!g){ g = byInd[m.indicator_id] = { id: m.indicator_id, r: INDBYID[m.indicator_id], acts: [], sum: 0, latest: null, ben: 0 }; order.push(g); }
  ...
  //
  //  The country is never chosen by hand: it is the project's country, or the
  //  country programme the KPI belongs to. That is what scopes the settlement
  //  search, so a location can never contradict the record it hangs off.

  /** Projects an activity may be attributed to, with an explicit empty option. */
  function activityProjectOptions(selId){
    var list = (DB.tables.project || []).filter(function (p){
      return p.plan_id === S.plan && canReportProject(p);
    }).sort(function (a, b){
  ...
  /** With no project in the picture, the KPIs to report against are every
   *  framework KPI of the active plan the user is allowed to report on. */
  function planKpiOptions(selId){
    // The same framework KPI exists once per country programme, so a flat list
    // repeats every name dozens of times. Grouped by country, the picker reads
    // as "where, then what" - which is how anyone logging field work thinks.
    var byCo = {};
    IND.filter(function (r){ return !r.secondary && canReport(r); }).forEach(function (r){
      var co = countryLabel(r.iso);
      (byCo[co] = byCo[co] || []).push(r);
    });
    return Object.keys(byCo).sort().map(function (co){
      var opts = byCo[co].sort(function (a, b){ return kpiLabelText(a).localeCompare(kpiLabelText(b)); })
        .map(function (r){
          return '<option value="' + r.id + '"' + (selId != null && r.id === selId ? ' selected' : '') + '>'
            + esc(kpiLabelText(r)) + '</option>';
        }).join('');
      return '<optgroup label="' + esc(co) + '">' + opts + '</optgroup>';
    }).join('');
  }
  ...
  function activityProjectOptionsFor(r, selId){
    var ids = {};
    (DB._idx.projectKpiByIndicator[r.id] || []).forEach(function (pk){ ids[pk.project_id] = 1; });
    if (r.secondary && r.raw && r.raw.project_id != null) ids[r.raw.project_id] = 1;
  ...
  /** The country an activity sits in: the project's, else its KPI's programme. */
  function activityIso(p, r){
    if (p && p.country_iso3) return p.country_iso3;
    return r ? r.iso : null;
  }
  ...
  // Add-activity popup - over the project modal, or standalone from a KPI's
  // Activities tab. Reuses the New Activity overlay.
  function openProjectActivityAdd(p){ openActivityAdd(p, null); }
  function openActivityAdd(p, presetKpiId){
    var host = $('#naBody'); host.innerHTML = '';
    ...
    var details = '<div class="aform"><div class="afgrid">' +
      '  <label class="af"><span>Project</span><select id="paProject">' + activityProjectOptions(p ? p.id : null) + '</select></label>' +
      '  <label class="af"><span>Country</span><input class="af-ro" id="paCountry" type="text" readonly value="" title="Taken from the project, or from the KPI’s country programme"></label>' +
      ...
      '</div><div class="cp-note"><b>Project is optional.</b> Leave it empty and the activity is still logged against its KPI, ' +
      'counted and plotted - it simply carries no donor or project attribution. Locations are searched live from the ' +
    ...
    function selKpi(){ return indById(+$('#paKpi').value); }
    function clearPlace(){
    ...
    function syncContext(){
      var r = selKpi();
      var iso = activityIso(selProject(), r);
      $('#paCountry').value = countryLabel(iso);
    ...
    $('#paProject').onchange = function (){
      var pp = selProject();
      var keep = selKpi();
      $('#paKpi').innerHTML = activityKpiOptions(pp, keep ? keep.id : null);
      syncContext();
    };
    ...
    // the search reads the country at query time, so switching project or KPI
  ...
  function submitActivity(){
    ...
    if (!activityIso(proj, r)){ msg.textContent = 'This KPI has no country programme - pick a project for it instead.'; return; }
    ...
    var fromProject = $('#projectModal').classList.contains('on');
    msg.textContent = 'Saving…';
    insertProjectActivity(r, projId, title, valRaw, date, (fv($('#paNote')) || '').trim(), place)
      .then(function (measRow){ return saveBeneficiaries(measRow.id); })
      // land on the KPI the activity was logged against, so the new row is in view
      .then(function (){
        enrich(); renderTicker(); renderAll(); closeNewActivity();
        if (fromProject && projId != null && curProject && projId === curProject.id){ _prActKpi = r.id; setProjectTab('activities'); }
        else { var nr = indById(r.id) || r; openDetail(nr); setModalTab('results'); }
      });
  }
  ...
  // otherwise a first-class activity. Beneficiaries hang off it. Returns the
  // measurement row (id assigned) so beneficiaries can be attached.
  ...
  function kpiInventoryEditor(){
    ...
    tbl.innerHTML = '<thead><tr><th>KPI</th><th>Impact</th><th>Unit</th><th>Target</th><th>Instances</th><th></th></tr></thead>';
    var tb = el('tbody');
