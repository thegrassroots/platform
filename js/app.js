...
(function () {
  ...
  var S = {
    ...
    mapMode: 'flat',        // map projection: 'flat' (Web-Mercator) | 'globe' (3D sphere)
  ...
  function renderKpiFacet() {
    ...
    var names = Object.keys(pillarOf).filter(function (n) {
      return !q || kpiFacetLabel(n).toLowerCase().indexOf(q) >= 0;
    });
  ...
  function submitProjectActivity(p){
    ...
    insertProjectActivity(r, p.id, title, valRaw, date, ($('#paNote').value || '').trim(), place)
      ...
      .then(function (){ enrich(); renderTicker(); renderAll(); closeNewActivity(); _prActKpi = r ? r.id : null; setProjectTab('activities'); });
  }
  ...
  function loadPrefs() {
    ...
    try {
      ...
      S.mapMode = (p.mapMode === 'globe') ? 'globe' : 'flat';   // projection is sticky across reloads
