...
(function () {
  ...
  var mem = {};
  // Cold rows when there is no persistent backend at all (memory-only session).
  var COLD_MEM = {};

  function tableNames() { return MODEL.order; }
  function pkOf(t) { var m = MODEL.tables[t]; return (m && m.pk) || 'id'; }
  /** COLD STORAGE: a table that declares a BLOB column holds uploaded file bytes.
   *  It is never hydrated into `mem` - not at boot, not on resync - and is read
   *  one row at a time through DB.getCold, so files of any size cost nothing
   *  until someone opens one. Derived from the schema, like everything else. */
  function isCold(t) { var m = MODEL.tables[t]; return !!(m && m.cold); }
  ...
  function readTableBody(name, body) {
    ...
    splitTopLevel(body).forEach(function (piece) {
      ...
      // COLD STORAGE: a BLOB column holds file bytes (see isCold)
      if (/^"?\w+"?\s+BLOB\b/i.test(def)) meta.cold = true;
  ...
  /** Load js/seed.js on demand.
   *
   *  The seed is 7 MB of DEMO DATA and is needed only in STATIC mode. In LOCAL
   *  mode every row comes from the database, so downloading and parsing it there
   *  was pure startup cost. It is fetched as a script rather than by fetch() so
   *  it still works from file://, where fetch is blocked. The URL comes from
   *  window.SEED_SRC, declared in index.html, so the cache-buster stays in the
   *  one place every other asset's version lives. */
  function loadSeed() {
    if (window.SEED) return Promise.resolve(true);
    if (typeof document === 'undefined') return Promise.resolve(false);
    return new Promise(function (resolve) {
      var s = document.createElement('script');
      s.src = window.SEED_SRC || (ROOT + 'js/seed.js');
      s.async = false;
      s.onload = function () { resolve(!!window.SEED); };
      s.onerror = function () {
        console.warn('DB: could not load the demo seed (' + s.src + ')');
        resolve(false);
      };
  ...
  function apiFetch(path, opts) {
    ...
    return fetch(API + path, o).then(function (r) {
      clearTimeout(timer);
      // an error page (403/404 from the static handler) is HTML, not JSON -
      // keep its status rather than surfacing a JSON parse error
      return r.json().catch(function () { return { ok: false, error: path + ' -> ' + r.status }; }).then(function (body) {
  ...
  function openIDB() {
    return new Promise(function (resolve, reject) {
      ...
      req.onsuccess = function () {
        ...
        up.onsuccess = function () { yieldOnVersionChange(up.result); resolve(up.result); };
  ...
  /** Another tab needs to upgrade or reseed the store: step aside rather than
   *  block it (a blocked upgrade drops that tab to a memory-only session).
   *  This tab reloads to pick up the new store. */
  function yieldOnVersionChange(db) {
    db.onversionchange = function () {
      try { db.close(); } catch (e) {}
      if (DB._db === db) DB._db = null;
  ...
  /** `fresh` marks NEW rows: in SQLite they go as a plain INSERT, so a key that
   *  another tab or user took in the meantime is refused (and memory resyncs)
   *  instead of silently overwriting their row. */
  function putRows(table, rows, fresh) {
    if (DB.mode === 'sqlite') return enqueue({ op: fresh ? 'ins' : 'put', table: table, rows: rows.map(clone) });
  ...
  var DB = {
    ...
    apiBase: API,        // absolute-ish API root, valid from / and /mobile/ alike
    ...
    _loadModel: function () {
      ...
      return apiFetch('meta', { timeout: API_TIMEOUT }).then(function (body) {
        ...
        return fetchText(SCHEMA_URL).then(function (sql) {
          ...
          // neither the API nor schema.sql is reachable (file://) - the seed's
          // own shape is the only description of the model left
    ...
    _initBrowser: function (source) {
      return loadSeed().then(function () { return DB._seedBrowser(source); });
    },
    _seedBrowser: function (source) {
      ...
      function memoryOnly() {
        // a half-opened store must not keep receiving writes the session cannot read back
      ...
      return wipe.then(openIDB).then(function (db) {
        ...
        return serial(tableNames(), function (t) {
          if (isCold(t)) { mem[t] = []; return null; }   // read on demand, never hydrated
    ...
    resync: function () {
      ...
      if (DB.mode === 'idb' && DB._db) {
        return Promise.all(tableNames().filter(function (t) { return !isCold(t); }).map(function (t) {
    ...
    insert: function (table, rows) {
      ...
      return putRows(table, rows, true).then(function () { return rows; });
    },
    ...
    // ---- cold storage (file bytes) -----------------------------------------
    // Rows of a COLD table (see isCold) bypass the in-memory mirror entirely:
    // written straight to the backend, read back one row at a time.
    isCold: isCold,
    /** Write (insert or replace) cold rows. They must carry their primary key. */
    putCold: function (table, rows) {
      if (!Array.isArray(rows)) rows = [rows];
      if (DB.mode === 'memory' || (!DB._db && DB.mode !== 'sqlite')) {
        COLD_MEM[table] = COLD_MEM[table] || {};
        rows.forEach(function (r) { COLD_MEM[table][r[pkOf(table)]] = r; });
        return Promise.resolve(rows);
      }
      return putRows(table, rows).then(function () { return rows; });
    },
    /** Read one cold row by primary key; resolves null when it does not exist. */
    getCold: function (table, key) {
      if (DB.mode === 'sqlite') {
        return apiFetch('row/' + encodeURIComponent(table) + '/' + encodeURIComponent(key))
          .then(function (b) { return b.row || null; }, function (e) { if (e.status === 404) return null; throw e; });
      }
      if (!DB._db) return Promise.resolve(((COLD_MEM[table] || {})[key]) || null);
      return new Promise(function (resolve, reject) {
        var r = DB._db.transaction(table, 'readonly').objectStore(table).get(key);
        r.onsuccess = function () { resolve(r.result || null); };
        r.onerror = function () { reject(r.error); };
      });
    },
    /** Delete cold rows by primary key. */
    removeCold: function (table, keys) {
      if (!Array.isArray(keys)) keys = [keys];
      if (!keys.length) return Promise.resolve();
      if (COLD_MEM[table]) keys.forEach(function (k) { delete COLD_MEM[table][k]; });
      if (DB.mode === 'memory' || (!DB._db && DB.mode !== 'sqlite')) return Promise.resolve();
      return delRows(table, keys);
    },
    /** A URL the file behind a cold row is served from, when the backend serves
     *  one (LOCAL mode). The browser store has no URL - callers fall back to a
     *  blob built from getCold. */
    coldUrl: function (table, key) {
      if (DB.mode !== 'sqlite') return null;
      return new URL(API + 'blob/' + encodeURIComponent(table) + '/' + encodeURIComponent(key), location.href).href;
    },
    ...
    /** Every measurement, oldest first (built once per index rebuild). */
    measurementsByDate: function () {
      var idx = DB._idx;
      if (!idx.measByDate) {
        idx.measByDate = mem.measurement.slice().sort(function (a, b) {
          return (a.date < b.date) ? -1 : (a.date > b.date ? 1 : a.id - b.id);
        });
      }
      return idx.measByDate;
    },
